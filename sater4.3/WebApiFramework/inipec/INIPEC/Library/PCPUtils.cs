...
using static INIPEC.Library.AggiudicazioneA3_6Type;

namespace INIPEC.Library
{
    //Test per formazione sui rami git
    ...
    public class GeneralUtils
    {
        ...
        /// <summary>
        /// Funzione di utilità base, permette di ottenere un valore/parametro dalla tabella ctl_parametri
        /// </summary>
        /// <param name="Contesto">valore in match sulla colonna Contesto</param>
        /// <param name="Oggetto">valore in match sulla colonna Oggetto</param>
        /// <param name="Proprieta">valore in match sulla colonna Proprieta</param>
        /// <param name="DefaultValue">(opz.)valore di default da ritornare in caso di assenza del parametro db richiesto. default stringa vuota</param>
        /// <param name="IdPfu">(opz.)per ora passare sempre -1 o omettere del tutto il parametro. default -1</param>
        /// <returns></returns>
        public string getDbParam(string Contesto, string Oggetto, string Proprieta, string DefaultValue = "", int IdPfu = -1)
        {
            const string strSql = "select dbo.PARAMETRI(@Contesto, @Oggetto, @Proprieta, @DefaultValue, @IdPfu) as pValue";

            cmd.Parameters.Clear();

            cmd.Parameters.AddWithValue("@Contesto", Contesto);
            cmd.Parameters.AddWithValue("@Oggetto", Oggetto);
            cmd.Parameters.AddWithValue("@Proprieta", Proprieta);
            ...
            try
            {
                string pValue = (string)cmd.ExecuteScalar();
                return pValue;
            }
            finally
            {
    ...
    public class PDNDUtils
    {
        ...
        public int AggiornaIdAvviso(int iddoc, string idAvviso)
        {
            cmd.Parameters.Clear();
            cmd.Parameters.AddWithValue("@idDoc", iddoc);
            cmd.Parameters.AddWithValue("@idAvviso", idAvviso);
        ...
        public string recuperaTipoDocGara(int iddoc)
        {
            //RECUPERO TIPODOC DALLA CTL_DOC
            string strSQL = "select TipoDoc from CTL_DOC with(nolock) where ID = " + iddoc;
            cmd.Parameters.Clear();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = strSQL;
            conn.Open();
            ...
            object ObjTipoDoc = (string)cmd.ExecuteScalar();
            conn.Close();

            if (ObjTipoDoc != null)
            {
                TipoDoc = ObjTipoDoc.ToString();
            }
            conn.Close();
        ...
        public string getStatoSchedaBando(int idGara)
        {
            try
            {
                conn.Open();

                const string strSql = @"select scheda.statoScheda 
	                                from ctl_doc a with(nolock)
			                                inner join Document_PCP_Appalto b with(nolock) on b.idHeader = a.Id
			                                inner join Document_PCP_Appalto_Schede scheda with(nolock) on scheda.idHeader = a.Id and scheda.tipoScheda = b.pcp_TipoScheda and scheda.bDeleted = 0
	                                where a.Id = @idGara";

                cmd.Parameters.Clear();
                cmd.Parameters.AddWithValue("@idGara", idGara);
                cmd.CommandText = strSql;
                cmd.CommandType = CommandType.Text;
        ...
        public TipoScheda enumTipoScheda(string tipoScheda)
        {
            switch (tipoScheda)
            {
                ...
                case ("NAG"):
                    return TipoScheda.NAG;
                case ("P2_20"):
                    return TipoScheda.P2_20;
                case ("P1_20"):
                    return TipoScheda.P1_20;
                case ("A2_32"):
                    return TipoScheda.A2_32;
                case ("A3_6"):
                    return TipoScheda.A3_6;
                case ("A1_32"):
                    return TipoScheda.A1_32;
                case ("A2_33"):
                    return TipoScheda.A2_33;
                case ("A1_33"):
                    return TipoScheda.A1_33;
                default:
                    return TipoScheda.P1_16;
            }
        }

        public void RecuperaEFormXml64EspdXml64(int idDoc, ref string eformXml64, ref string espdXml64, string tipoEForm = "CN16")
        {
            ...
            switch (tipoScheda)
            {
                ...
                case TipoScheda.P1_19:
                    ...
                    string eform = recuperaEFormXml(idDoc, tipoEForm);
                ...
                case TipoScheda.P2_20:
                    string espdP2_20 = recuperaESPDXml(idDoc);
                    if (String.IsNullOrEmpty(espdP2_20))
                    {
                        //esito = "0#XML ESPD vuoto";
                        throw new ApplicationException("XML ESPD vuoto");
                    }

                    espdXml64 = Convert.ToBase64String(Encoding.UTF8.GetBytes(espdP2_20));
                    break;
                case TipoScheda.P1_20:
                    string espdP1_20 = recuperaESPDXml(idDoc);
                    string eformP1_20 = recuperaEFormXml(idDoc, tipoEForm);

                    if (string.IsNullOrEmpty(espdP1_20))
                    {
                        //esito = "0#XML ESPD vuoto";
                        throw new ApplicationException("XML ESPD vuoto");
                    }

                    if (string.IsNullOrEmpty(eformP1_20))
                    {
                        //esito = "0#XML eForm vuoto";
                        throw new ApplicationException("XML eForm vuoto");
                    }
                    eformXml64 = Convert.ToBase64String(Encoding.UTF8.GetBytes(eformP1_20));
                    espdXml64 = Convert.ToBase64String(Encoding.UTF8.GetBytes(espdP1_20));
                    break;
                default:
                    break;
            }
        ...
        /// <summary>
        /// Serve recuperare la scheda pcp entrando con l'idRow della tabella document_pcp_appalto_schede
        /// </summary>
        /// <param name="idRow">ID tabellare ( chiave primaria ) della tabella document_pcp_appalto_schede</param>
        /// <returns></returns>
        ...
        {
            string strumentiSvolgimentoProcedure = string.Empty;
            ...
            if (dt.Rows.Count > 0)
            {
                /* Recupero le stazioni Appaltanti */
                a.stazioniAppaltanti = RecuperoStazioniAppaltanti(dt);
                ...
                strumentiSvolgimentoProcedure = dt.Rows[0]["strumentiSvolgimentoProcedure"].ToString();
                if (!string.IsNullOrEmpty(strumentiSvolgimentoProcedure))
                {
                    StrumentiSvolgimentoProcedure ss = new StrumentiSvolgimentoProcedure() { codice = strumentiSvolgimentoProcedure, idTipologica = "strumentiSvolgimentoProcedure" };
        ...
        private List<LottiP2_16> recuperaLottiP2_16(int idDoc, AppaltoP2_16 appalto)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    bool afferenteInvestimentiPNRR = Convert.ToBoolean(Convert.ToInt16(dt.Rows[i]["afferenteInvestimentiPNRR"].ToString()));
                    l.afferenteInvestimentiPNRR = afferenteInvestimentiPNRR;

                    //Se afferenteInvestimentiPNRR è true allora procedo con la composizione del nodo ParitaDiGenereGenerazionale
                    if (afferenteInvestimentiPNRR == true)
                    {
                        l.paritaDiGenereGenerazionale = RecuperaParitaDiGenereGenerazionale(dt1);
                    }
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (lottoP7_1_2 != null)
                {
                    LottiP7_1_3 lottoP7_1_3 = new LottiP7_1_3()
                    {
                        ...
                        paritaDiGenereGenerazionale = lottoP7_1_2.paritaDiGenereGenerazionale,
        ...
        public AnacFormAD2_25 recuperaAnacFormAD2_25(int idDoc, Dati_PCP dati)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                DatiBaseProceduraA2_29 dbaseProc = new DatiBaseProceduraA2_29();
                ...
                {
                    if (!string.IsNullOrEmpty(dt.Rows[0]["giustificazioniAggiudicazioneDiretta"].ToString()))
                    {
                        ...
                        dbaseProc.giustificazioniAggiudicazioneDiretta = listGiustificazioniAggiudicazioneDiretta;
                    }
                }
                ...
                for (int j = 0; j < dt2.Rows.Count; j++)
                {
                    ...
                    bool afferenteInvestimentiPNRR = Convert.ToBoolean(Convert.ToInt16(dt2.Rows[j]["afferenteInvestimentiPNRR"].ToString()));
                    aggiudicazioneAD2_25Type.afferenteInvestimentiPNRR = afferenteInvestimentiPNRR;

                    //Se afferenteInvestimentiPNRR è true allora procedo con la composizione del nodo ParitaDiGenereGenerazionale
                    if (afferenteInvestimentiPNRR == true)
                    {
                        aggiudicazioneAD2_25Type.paritaDiGenereGenerazionale = RecuperaParitaDiGenereGenerazionale(dt);
                        aggiudicazioneAD2_25Type.acquisizioneCup = UtilsConvert.ToBool(dt2.Rows[j]["acquisizioneCup"].ToString());
                    }
        ...
        public AnacFormAD3 recuperaAnacFormAD3(int idDoc, Dati_PCP dati)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for (int j = 0; j < dt2.Rows.Count; j++)
                {
                    ...
                    //Recupero il campo PNRR e se true inseriesco ParitaDiGenereGenerazionale
                    if (!string.IsNullOrEmpty(dt2.Rows[j]["afferenteInvestimentiPNRR"].ToString()))
                    {
                        bool afferenteInvestimentiPNRR = Convert.ToBoolean(Convert.ToInt16(dt2.Rows[j]["afferenteInvestimentiPNRR"].ToString()));

                        aggiudicazioneAD3Type.afferenteInvestimentiPNRR = afferenteInvestimentiPNRR;

                        //Se afferenteInvestimentiPNRR è true allora procedo con la composizione del nodo ParitaDiGenereGenerazionale
                        if (afferenteInvestimentiPNRR == true)
                        {
                            aggiudicazioneAD3Type.paritaDiGenereGenerazionale = RecuperaParitaDiGenereGenerazionale(dt);
                            aggiudicazioneAD3Type.acquisizioneCup = UtilsConvert.ToBool(dt2.Rows[j]["acquisizioneCup"].ToString());
                        }
                    }
        ...
        public AnacFormAD4 recuperaAnacFormAD4(int idDoc, Dati_PCP dati)
        {
            AnacFormAD4 a = new AnacFormAD4();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                AppaltoAD4 appalto = new AppaltoAD4();

                appalto.codiceAppalto = recuperaCodiceAppalto(idDoc);

                appalto.cig = dt.Rows[0]["cig"].ToString();

                appalto.dataAdesione = dt.Rows[0]["dataAdesione"].ToString();
                ...
                for (int j = 0; j < dt2.Rows.Count; j++)
                {
                    ...
                    aggiudicazioneAD3Type.afferenteInvestimentiPNRR = Convert.ToBoolean(Convert.ToInt16(dt2.Rows[j]["afferenteInvestimentiPNRR"].ToString()));
        ...
        public AnacFormP1_19 recuperaAnacFormP1_19(int idDoc, Dati_PCP dati)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dt2.Rows.Count; i++)
                {
                    ...
                    bool afferenteInvestimentiPNRR = Convert.ToBoolean(Convert.ToInt16(dt2.Rows[i]["afferenteInvestimentiPNRR"].ToString()));
                    l.afferenteInvestimentiPNRR = afferenteInvestimentiPNRR;

                    //Se afferenteInvestimentiPNRR è true allora procedo con la composizione del nodo ParitaDiGenereGenerazionale
                    if (afferenteInvestimentiPNRR == true)
                    {
                        l.paritaDiGenereGenerazionale = RecuperaParitaDiGenereGenerazionale(dt);
                    }
        ...
        /* INIZIO P1_20 */
        public AnacFormP1_20 recuperaAnacFormP1_20(int idDoc, Dati_PCP dati)
        {
            AnacFormP1_20 a = new AnacFormP1_20();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                AppaltoP1_20 appalto = new AppaltoP1_20();

                // if()

                if (dt.Rows[0]["pcp_MotivoUrgenza"] != null && !String.IsNullOrEmpty(dt.Rows[0]["pcp_MotivoUrgenza"].ToString()))
                {
                    MotivoUrgenza mu = new MotivoUrgenza();
                    mu.idTipologica = "motivoUrgenza";
                    mu.codice = dt.Rows[0]["pcp_MotivoUrgenza"].ToString();
                    appalto.motivoUrgenza = mu;
                }
                else
                {
                    appalto.motivoUrgenza = null;
                ...
                DatiBaseProcedura d = new DatiBaseProcedura();
                TipoProcedura t = new TipoProcedura();
                t.idTipologica = "tipoProcedura";
                t.codice = dt.Rows[0]["tipoProcedura"].ToString();

                d.tipoProcedura = t;

                appalto.datiBaseProcedura = d;

                List<LottiP1_20> lotti = new List<LottiP1_20>();
                //List<Finanziamenti> listaFinanziamenti = new List<Finanziamenti>();


                for (int i = 0; i < dt2.Rows.Count; i++)
                {
                    LottiP1_20 l = new LottiP1_20();
                    List<Finanziamenti> listaFinanziamenti = new List<Finanziamenti>();
                    //l.finanziamenti = listaFinanziamenti;
                    string numLotto = "0000" + dt2.Rows[i]["NumeroLotto"].ToString();
                    l.lotIdentifier = "LOT-" + numLotto.Substring(numLotto.Length - 4);
                    ...
                    if (dt2.Rows[i]["pcp_PrevedeRipetizioniCompl"].ToString() != null && !string.IsNullOrEmpty(dt2.Rows[i]["pcp_PrevedeRipetizioniCompl"].ToString()))
                    {
                        l.ripetizioniEConsegneComplementari = UtilsConvert.ToBool(dt2.Rows[i]["pcp_PrevedeRipetizioniCompl"].ToString());
                    }
                    ...
                    {
                        quadroEconomicoStandard.impTotaleSicurezza = UtilsConvert.ToDecimal(dt2.Rows[i]["impTotaleSicurezza"].ToString());
                    }
                    else
                    {
                        quadroEconomicoStandard.impTotaleSicurezza = 0;
                    ...
                    {
                        quadroEconomicoStandard.ulterioriSommeNoRibasso = UtilsConvert.ToDecimal(dt2.Rows[i]["ulterioriSommeNoRibasso"].ToString());
                    }
                    else
                    {
                        quadroEconomicoStandard.ulterioriSommeNoRibasso = 0;
                    ...
                    {
                        quadroEconomicoStandard.impProgettazione = UtilsConvert.ToDecimal(dt2.Rows[i]["impProgettazione"].ToString());
                    }
                    else
                    {
                        quadroEconomicoStandard.impProgettazione = 0;
                    ...
                    {
                        quadroEconomicoStandard.sommeOpzioniRinnovi = UtilsConvert.ToDecimal(dt2.Rows[i]["sommeOpzioniRinnovi"].ToString());
                    }
                    else
                    {
                        quadroEconomicoStandard.sommeOpzioniRinnovi = 0;
                    ...
                    {
                        quadroEconomicoStandard.sommeRipetizioni = UtilsConvert.ToDecimal(dt2.Rows[i]["sommeRipetizioni"].ToString());
                    }
                    else
                    {
                        quadroEconomicoStandard.sommeRipetizioni = 0;
                    ...
                    {
                        quadroEconomicoStandard.sommeADisposizione = UtilsConvert.ToDecimal(dt2.Rows[i]["sommeADisposizione"].ToString());
                    }
                    else
                    {
                        quadroEconomicoStandard.sommeADisposizione = 0;
                    ...
                    if (dt2.Rows[i]["pcp_ModalitaAcquisizione"] != null && !string.IsNullOrEmpty(dt2.Rows[i]["pcp_ModalitaAcquisizione"].ToString()) && dt2.Rows[i]["oggettoPrincipaleContratto"].ToString().ToUpper() != "L")
                    {
                        ModalitaAcquisizione m = new ModalitaAcquisizione();
                        m.codice = dt2.Rows[i]["pcp_ModalitaAcquisizione"].ToString();
                        m.idTipologica = "modalitaAcquisizione";
                    ...
                    l.datiBaseDocumenti = new DatiBaseDocumenti()
                    {
                        url = dt.Rows[0]["cn16_CallForTendersDocumentReference_ExternalRef"].ToString()
                    };
                    ...
                    l.datiBaseTerminiInvio = new DatiBaseTerminiInvio2()
                    {
                        ...
                        scadenzaPresentazioneInvito = Convert.ToDateTime(dt.Rows[0]["oraScadenzaPresentazioneOfferte"].ToString())
                    };
        ...
        /* FINE P1_20 */
        public AnacFormP2_20 recuperaAnacFormP2_20(int idDoc, Dati_PCP dati)
        {
            AnacFormP2_20 a = new AnacFormP2_20();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                AppaltoP2_20 appalto = new AppaltoP2_20();
                ...
                List<LottiP2_20> lotti = new List<LottiP2_20>();



                for (int i = 0; i < dt2.Rows.Count; i++)
                {
                    LottiP2_20 l = new LottiP2_20();
                    List<Finanziamenti> listaFinanziamenti = new List<Finanziamenti>();
                    //l.finanziamenti = listaFinanziamenti;
                    string numLotto = "0000" + dt2.Rows[i]["NumeroLotto"].ToString();
                    l.lotIdentifier = "LOT-" + numLotto.Substring(numLotto.Length - 4);
                    ...
                    if (!string.IsNullOrEmpty(dt.Rows[0]["scadenzaPresentazioneInvito"].ToString()))
                    {
                        datiBaseTerminiInvio.scadenzaPresentazioneInvito = Convert.ToDateTime(dt.Rows[0]["oraScadenzaPresentazioneOfferte"].ToString());
                    }
                    else
                    {
                        if (!string.IsNullOrEmpty(dt.Rows[0]["oraScadenzaPresentazioneOfferte"].ToString()))
                        {
                            datiBaseTerminiInvio.oraScadenzaPresentazioneOfferte = Convert.ToDateTime(dt.Rows[0]["oraScadenzaPresentazioneOfferte"].ToString());
                        }
                    }
        ...
        public AnacFormP2_19 recuperaAnacFormP2_19(int idDoc, Dati_PCP dati)
        {
            AnacFormP2_19 a = new AnacFormP2_19();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                AppaltoP2_19 appalto = new AppaltoP2_19();
                ...
                List<LottiP2_19> lotti = new List<LottiP2_19>();
                List<Finanziamenti> listaFinanziamenti = new List<Finanziamenti>();

                for (int i = 0; i < dt2.Rows.Count; i++)
                {
                    LottiP2_19 l = new LottiP2_19();
                    l.finanziamenti = listaFinanziamenti;
                    string numLotto = "0000" + dt2.Rows[i]["NumeroLotto"].ToString();
                    l.lotIdentifier = "LOT-" + numLotto.Substring(numLotto.Length - 4);
                    ...
                    l.datiBaseDocumenti = new DatiBaseDocumenti()
                    {
                        //url = dt.Rows[0]["datiBaseDocumenti_url"].ToString()
                        url = dt.Rows[0]["cn16_CallForTendersDocumentReference_ExternalRef"].ToString()
                    };

                    string tipoProcedura = UtilsConvert.ToString(dt.Rows[0]["tipoProcedura"]);


                    DatiBaseTerminiInvio2 datiBaseTerminiInvio2 = new DatiBaseTerminiInvio2();
                    if (tipoProcedura == "open")
                    {
                        datiBaseTerminiInvio2.oraScadenzaPresentazioneOfferte = Convert.ToDateTime(dt.Rows[0]["oraScadenzaPresentazioneOfferte"].ToString());
                    }
                    else
                    {
                        datiBaseTerminiInvio2.scadenzaPresentazioneInvito = Convert.ToDateTime(dt.Rows[0]["oraScadenzaPresentazioneOfferte"].ToString());
                    }

                    l.datiBaseTerminiInvio = datiBaseTerminiInvio2;
        ...
        public AnacFormA1_29 recuperaAnacFormA1_29NonAgg(int idDoc, Dati_PCP dati, int IdDoc_Scheda, string ListaCIG)
        {
            AnacFormA1_29 a = new AnacFormA1_29();
            ...
            cmd.Parameters.AddWithValue("@IdGara", idDoc);
            cmd.Parameters.AddWithValue("@Contesto", "LOTTI");
            string strSql_2 = "GET_DATI_SCHEDA_PCP_A1_29_NON_AGG";
            cmd.CommandText = strSql_2;
            cmd.CommandType = CommandType.StoredProcedure;
            DataTable dt2 = new DataTable();
            SqlDataAdapter da2 = new SqlDataAdapter();
            ...
            cmd.Parameters.Clear();
            cmd.Parameters.AddWithValue("@IdGara", idDoc);
            cmd.Parameters.AddWithValue("@Contesto", "OFFERTE");
            string strSql_3 = "GET_DATI_SCHEDA_PCP_A1_29_NON_AGG";
            cmd.CommandText = strSql_3;
            cmd.CommandType = CommandType.StoredProcedure;
            DataTable dt3 = new DataTable();
            SqlDataAdapter da3 = new SqlDataAdapter();
            ...
            for (int i = 0; i < dt2.Rows.Count; i++)
            {
                if (ListaCIG.Contains(UtilsConvert.ToString(dt2.Rows[i]["cig"])))
                {
                    var aggiudicazione = new AggiudicazioneA1_29();
            ...
            for (int i = 0; i < dt3.Rows.Count; i++)
            {
                OffertePresentate offertePresentate = new OffertePresentate()
                {
                    idPartecipante = UtilsConvert.ToString(dt3.Rows[i]["idPartecipante"]),
                    importo = UtilsConvert.ToDecimal(dt3.Rows[i]["importo"]),
                    aggiudicatario = false,
                    ccnl = UtilsConvert.ToString(dt3.Rows[i]["ccnl"])
                };
        ...
        public AnacFormA2_29 recuperaAnacFormA2_29(int idDoc, Dati_PCP dati, int IdDoc_Scheda)
        {
            ...
            for (int i = 0; i < dt3.Rows.Count; i++)
            {
                ...
                if (dt3.Columns.Contains("posizioneGraduatoria") && !string.IsNullOrEmpty(dt3.Rows[i]["posizioneGraduatoria"].ToString()))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                {
                    offertePresentate.posizioneGraduatoria = Convert.ToInt32(dt3.Rows[i]["posizioneGraduatoria"].ToString());
                }
                if (dt3.Columns.Contains("offertaEconomica") && !string.IsNullOrEmpty(dt3.Rows[i]["offertaEconomica"].ToString()))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                {
                    offertePresentate.offertaEconomica = UtilsConvert.ToDecimal(dt3.Rows[i]["offertaEconomica"]);
                }
                if (dt3.Columns.Contains("offertaQualitativa") && !string.IsNullOrEmpty(dt3.Rows[i]["offertaQualitativa"].ToString()))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                {
                    offertePresentate.offertaQualitativa = UtilsConvert.ToDecimal(dt3.Rows[i]["offertaQualitativa"]);
                }

                if (dt3.Columns.Contains("offertaInAumento") && !string.IsNullOrEmpty(dt3.Rows[i]["offertaInAumento"].ToString()))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                {
                    offertePresentate.offertaInAumento = UtilsConvert.ToDecimal(dt3.Rows[i]["offertaInAumento"]);
                }

                if (dt3.Columns.Contains("offertaMaggioreSogliaAnomalia") && !string.IsNullOrEmpty(dt3.Rows[i]["offertaMaggioreSogliaAnomalia"].ToString()))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                {
                    offertePresentate.offertaMaggioreSogliaAnomalia = UtilsConvert.ToBool(dt3.Rows[i]["offertaMaggioreSogliaAnomalia"]);
                }

                if (dt3.Columns.Contains("impresaEsclusaAutomaticamente") && !string.IsNullOrEmpty(dt3.Rows[i]["impresaEsclusaAutomaticamente"].ToString()))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                {
                    offertePresentate.impresaEsclusaAutomaticamente = UtilsConvert.ToBool(dt3.Rows[i]["impresaEsclusaAutomaticamente"]);
                }

                if (dt3.Columns.Contains("offertaAnomala") && !string.IsNullOrEmpty(dt3.Rows[i]["offertaAnomala"].ToString()))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                {
                    offertePresentate.offertaAnomala = UtilsConvert.ToBool(dt3.Rows[i]["offertaAnomala"]);
                }
        ...
        public AnacFormP7_2 recuperaAnacFormP7_2(int idDoc, Dati_PCP dati)
        {
            ...
            cmd.Parameters.Clear();
            cmd.Parameters.AddWithValue("@idDoc", idDoc);
            string strSql_3 = "select Concessione from document_bando with (nolock) where idHeader = @idDoc ";
            cmd.CommandText = strSql_3;
            cmd.CommandType = CommandType.Text;
            conn.Open();
            ...
            {
                /* Recupero le stazioni Appaltanti */
                stazioni = RecuperoStazioniAppaltanti(dt);
                a.stazioniAppaltanti = stazioni;
                ...
                for (int i = 0; i < dt2.Rows.Count; i++)
                {
                    ...
                    lotto.afferenteInvestimentiPNRR = afferenteInvestimentiPNRR;

                    //Se afferenteInvestimentiPNRR è true allora procedo con la composizione del nodo ParitaDiGenereGenerazionale
                    if (afferenteInvestimentiPNRR == true)
                    {
                        lotto.paritaDiGenereGenerazionale = RecuperaParitaDiGenereGenerazionale(dt);
                    }
        ...
        public AnacFormA2_32 recuperaAnacFormA2_32(int idDoc, Dati_PCP dati, int IdDoc_Scheda)
        {
            AnacFormA2_32 a = new AnacFormA2_32();
            ...
            //il recupero dei dati va fatto passando alle stored l'id del contratto, NON l'id della gara
            //in quanto la scheda si costruisce dal contratto
            //difatti per una gara possiamo avere n contratti, quindi n scheda a2_32
            ...
            #region anacForm.appalto
            var appalto = new AppaltoA2_32();


            DatiBaseProceduraA2_32 datiBaseProcedura = new DatiBaseProceduraA2_32();
            TipoProcedura tipoProcedura = new TipoProcedura();
            tipoProcedura.idTipologica = "tipoProcedura";
            tipoProcedura.codice = UtilsConvert.ToString(dt.Rows[0]["tipoProcedura"]);
            datiBaseProcedura.tipoProcedura = tipoProcedura;
            ...
            #region anacForm.aggiudicazioni
            var aggiudicazioni = new List<AggiudicazioneA2_32>();
            for (int i = 0; i < dt2.Rows.Count; i++)
            {
                var aggiudicazione = new AggiudicazioneA2_32();
                ...
                if (dt2.Columns.Contains("sommeADisposizione"))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                {
                    QuadroEconomicoConcessioni qes = new QuadroEconomicoConcessioni();
                    qes.impForniture = UtilsConvert.ToDecimal(dt2.Rows[i]["sommeADisposizione"]);

                    if (dt2.Columns.Contains("impLavori") && !string.IsNullOrEmpty(dt2.Rows[i]["impLavori"].ToString()))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                    {
                        qes.impLavori = UtilsConvert.ToDecimal(dt2.Rows[i]["impLavori"]);
                    }

                    if (dt2.Columns.Contains("impServizi") && !string.IsNullOrEmpty(dt2.Rows[i]["impServizi"].ToString()))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                    {
                        qes.impServizi = UtilsConvert.ToDecimal(dt2.Rows[i]["impServizi"]);
                    }

                    if (dt2.Columns.Contains("impForniture") && !string.IsNullOrEmpty(dt2.Rows[i]["impForniture"].ToString()))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                    {
                        qes.impForniture = UtilsConvert.ToDecimal(dt2.Rows[i]["impForniture"]);
                    }

                    if (dt2.Columns.Contains("impTotaleSicurezza") && !string.IsNullOrEmpty(dt2.Rows[i]["impTotaleSicurezza"].ToString()))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                    {
                        qes.impTotaleSicurezza = UtilsConvert.ToDecimal(dt2.Rows[i]["impTotaleSicurezza"]);
                    }

                    if (dt2.Columns.Contains("ulterioriSommeNoRibasso") && !string.IsNullOrEmpty(dt2.Rows[i]["ulterioriSommeNoRibasso"].ToString()))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                    {
                        qes.ulterioriSommeRibasso = UtilsConvert.ToDecimal(dt2.Rows[i]["ulterioriSommeNoRibasso"]);
                    }

                    if (dt2.Columns.Contains("sommeOpzioniRinnovi") && !string.IsNullOrEmpty(dt2.Rows[i]["sommeOpzioniRinnovi"].ToString()))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                    {
                        qes.sommeOpzioniRinnovi = UtilsConvert.ToDecimal(dt2.Rows[i]["sommeOpzioniRinnovi"]);
                    }

                    if (dt2.Columns.Contains("sommeADisposizione") && !string.IsNullOrEmpty(dt2.Rows[i]["sommeADisposizione"].ToString()))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                    {
                        qes.sommeADisposizione = UtilsConvert.ToDecimal(dt2.Rows[i]["sommeADisposizione"]);
                    }

                    aggiudicazione.quadroEconomicoConcessioni = qes;
                }
            ...
            for (int i = 0; i < dt3.Rows.Count; i++)
            {
                ...
                offertePresentate.importo = UtilsConvert.ToDecimal(dt3.Rows[i]["importo"]);
                offertePresentate.aggiudicatario = UtilsConvert.ToBool(dt3.Rows[i]["aggiudicatario"]);
                offertePresentate.ccnl = "non applicabile";
        ...
        public AnacFormA7_1_2 recuperaAnacFormA7_1_2(int idDoc, Dati_PCP dati, int idDoc_Scheda)
        {

            AnacFormA7_1_2 a = new AnacFormA7_1_2();
            ...
            cmd.Parameters.Clear();
            cmd.Parameters.AddWithValue("@IdContratto", idDoc_Scheda);
            cmd.Parameters.AddWithValue("@Contesto", "AGGIUDICATARIO");
            string strSql_3 = "GET_DATI_SCHEDA_PCP_FROM_CONTRATTO";
            cmd.CommandText = strSql_3;
            cmd.CommandType = CommandType.StoredProcedure;
            da3.SelectCommand = cmd;
            ...
            foreach (DataRow dr1 in dt.Rows)
            {
                ...
                var appalto = new AppaltoA7_1_2();

                appalto.motivoUrgenza = new MotivoUrgenza()
                {
                    codice = dr1["pcp_MotivoUrgenza"].ToString(),
                    idTipologica = "motivoUrgenza"
                };

                if (!string.IsNullOrEmpty(dr1["pcp_LinkDocumenti"].ToString()))
                {
                    appalto.linkDocumenti = dr1["pcp_LinkDocumenti"].ToString();
                }

                if (!string.IsNullOrEmpty(dr1["pcp_RelazioneUnicaSulleProcedure"].ToString()))
                {
                    appalto.relazioneUnicaSulleProcedure = Convert.ToBoolean(dr1["pcp_RelazioneUnicaSulleProcedure"].ToString());
                }

                if (!string.IsNullOrEmpty(dr1["pcp_OpereUrbanizzateScomputo"].ToString()))
                {
                    appalto.opereUrbanizzazioneScomputo = Convert.ToBoolean(dr1["pcp_OpereUrbanizzateScomputo"].ToString());

                }


                var tipoProcedura = new TipoProcedura()
                {
                    idTipologica = "tipoProcedura",
                    codice = dr1["tipoProcedura"].ToString()
                };

                appalto.datiBaseProcedura = new DatiBaseProcedura()
                {
                ...
                if (dt.Columns.Contains("accordoQuadro"))
                {
                    accordoQuadro.codice = UtilsConvert.ToString(dr1["accordoQuadro"]);
                    datiBaseStrumentiProcedura.accordoQuadro = accordoQuadro;
                }
                SistemaDinamicoAcquisizione sistemaDinamicoAcquisizione = new SistemaDinamicoAcquisizione();
                sistemaDinamicoAcquisizione.idTipologica = "sistemaDinamicoAcquisizione";
                if (dt.Columns.Contains("sistemaDinamicoAcquisizione"))
                {
                    sistemaDinamicoAcquisizione.codice = UtilsConvert.ToString(dr1["sistemaDinamicoAcquisizione"]);
                    datiBaseStrumentiProcedura.sistemaDinamicoAcquisizione = sistemaDinamicoAcquisizione;
                }
                if (dt.Columns.Contains("astaElettronica"))
                {
                    datiBaseStrumentiProcedura.astaElettronica = UtilsConvert.ToBool(dr1["astaElettronica"]);
                }
                if (dt.Columns.Contains("accordoQuadro") || dt.Columns.Contains("sistemaDinamicoAcquisizione") || dt.Columns.Contains("astaElettronica"))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                {
                    appalto.datiBaseStrumentiProcedura = datiBaseStrumentiProcedura;
                ...
                #region AGGIUDICAZIONI

                List<AggiudicazioneA7_1_2> aggiudicazioni = new List<AggiudicazioneA7_1_2>();

                //LOTTI
                foreach (DataRow dr2 in dt2.Rows)
                {
                    var agg = new AggiudicazioneA7_1_2();

                    agg.cig = dr2["CIG"].ToString();

                    QuadroEconomicoStandard qs = new QuadroEconomicoStandard();

                    qs.ulterioriSommeNoRibasso = 0;
                    if (dt2.Columns.Contains("UlterioriSommeNoRibasso"))
                    {
                        qs.ulterioriSommeNoRibasso = UtilsConvert.ToDecimal(dr2["UlterioriSommeNoRibasso"].ToString());
                    }

                    qs.sommeOpzioniRinnovi = 0;
                    if (dt2.Columns.Contains("sommeOpzioniRinnovi"))
                    {
                        qs.sommeOpzioniRinnovi = UtilsConvert.ToDecimal(dr2["sommeOpzioniRinnovi"].ToString());
                    }

                    qs.sommeADisposizione = 0;
                    if (dt2.Columns.Contains("sommeADisposizione"))
                    {
                        qs.sommeADisposizione = UtilsConvert.ToDecimal(dr2["sommeADisposizione"].ToString());
                    }

                    qs.impProgettazione = 0;
                    if (dt2.Columns.Contains("impProgettazione"))
                    {
                        qs.impProgettazione = UtilsConvert.ToDecimal(dr2["impProgettazione"].ToString());
                    }

                    qs.impTotaleSicurezza = 0;
                    if (dt2.Columns.Contains("impTotaleSicurezza"))
                    {
                        qs.impTotaleSicurezza = UtilsConvert.ToDecimal(dr2["impTotaleSicurezza"].ToString());
                    }

                    qs.sommeRipetizioni = 0;
                    if (dt2.Columns.Contains("sommeRipetizioni"))
                    {
                        qs.sommeRipetizioni = UtilsConvert.ToDecimal(dr2["sommeRipetizioni"].ToString());
                    }

                    qs.impLavori = 0;
                    if (dt2.Columns.Contains("impLavori"))
                    {
                        qs.impLavori = UtilsConvert.ToDecimal(dr2["impLavori"].ToString());
                    }

                    qs.impServizi = 0;
                    if (dt2.Columns.Contains("impServizi"))
                    {
                        qs.impServizi = UtilsConvert.ToDecimal(dr2["impServizi"].ToString());
                    }

                    qs.impForniture = 0;
                    if (dt2.Columns.Contains("impForniture"))
                    {
                        qs.impForniture = UtilsConvert.ToDecimal(dr2["impForniture"].ToString());
                    }

                    agg.quadroEconomicoStandard = qs;

                    agg.afferenteInvestimentiPNRR = UtilsConvert.ToBool(dr2["afferenteInvestimentiPNRR"].ToString());


                    List<string> cupList = new List<string>();

                    if (dt2.Columns.Contains("cup") && !string.IsNullOrEmpty(dr2["cup"].ToString()))
                    {

                        string cups = dr2["CUP"].ToString();
                    ...
                    agg.cupLotto = cupList;

                    agg.acquisizioneCup = UtilsConvert.ToBool(dr2["acquisizioneCup"].ToString());


                    List<Finanziamenti> listaFin = new List<Finanziamenti>();

                    var Tipof = new TipoFinanziamento()
                    {
                        idTipologica = "tipoFinanziamento",
                        codice = dr2["TIPO_FINANZIAMENTO"].ToString()
                    };
                    var fin = new Finanziamenti()
                    {
                        importo = !dr2["pcp_ImportoFinanziamento"].ToString().IsNullOrEmpty() ? Convert.ToDecimal(dr2["pcp_ImportoFinanziamento"].ToString()) : 0,
                        tipoFinanziamento = Tipof
                    };
                    listaFin.Add(fin);

                    agg.finanziamenti = listaFin;
                    ...
                    if (dt2.Columns.Contains("accessibilita") || dt2.Columns.Contains("giustificazione"))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                    {
                        ...
                        Accessibilita accessibilita = new Accessibilita();
                        accessibilita.idTipologica = "accessibilita";
                        accessibilita.codice = UtilsConvert.ToString(dr2["accessibilita"]);
                        datiBaseAccessibilita.accessibilita = accessibilita;
                        datiBaseAccessibilita.giustificazione = UtilsConvert.ToString(dr2["giustificazione"]);
                        agg.datiBaseAccessibilita = datiBaseAccessibilita;
                    }

                    var partecipanti = new List<PartecipanteAD>();

                    //CERCO LE INFROMAZIONI DELL AGGIUDICATARIO
                    foreach (DataRow dr3 in dt3.Rows)
                    {
                        if (agg.cig == dr3["CIG"].ToString())
                        {
                            var tipoOE = new Categoria()
                            {
                                idTipologica = "tipoOE",
                                codice = dr3["tipoOE"].ToString()
                            };

                            var ruoloOE = new Categoria()
                            {
                                idTipologica = "ruoloOE",
                                codice = dr3["ruoloOE"].ToString()
                            };

                            partecipanti.Add(new PartecipanteAD()
                            {
                                codiceFiscale = dr3["codiceFiscale"].ToString(),
                                denominazione = dr3["Denominazione"].ToString(),
                                ruoloOE = ruoloOE,
                                tipoOE = tipoOE,
                                idPartecipante = dr3["idPartecipante"].ToString(),
                                paeseOperatoreEconomico = dr3["paeseOperatoreEconomico"].ToString(),
                                avvalimento = UtilsConvert.ToBool(dr3["idPartecipante"].ToString()),
                                importo = UtilsConvert.ToDecimal(dr3["importo"].ToString())
                            });

                            if (dt2.Columns.Contains("dataAggiudicazione") && !string.IsNullOrEmpty(dr2["dataAggiudicazione"].ToString()))
                            {
                                agg.DatiBaseAggiudicazioneAppalto = new DatiBaseAggiudicazioneAppalto()
                                {
                                    dataAggiudicazione = dr2["dataAggiudicazione"].ToString()
                                };
                            }
                    ...
                    agg.partecipanti = partecipanti;



                    aggiudicazioni.Add(agg);
                }
        ...
        public AnacFormA3_6 recuperaAnacFormA3_6(int idDoc, Dati_PCP dati)
        {
            AnacFormA3_6 a = new AnacFormA3_6();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                AppaltoA3_6 appalto = new AppaltoA3_6();
                ...
                List<AggiudicazioneA3_6Type> aggiudicazioni = new List<AggiudicazioneA3_6Type>();

                for (int j = 0; j < dt2.Rows.Count; j++)
                {

                    AggiudicazioneA3_6Type aggiudicazioneA3_6Type = new AggiudicazioneA3_6Type();

                    aggiudicazioneA3_6Type.lotIdentifier = dt2.Rows[j]["lotIdentifier"].ToString();

                    bool afferenteInvestimentiPNRR = Convert.ToBoolean(Convert.ToInt16(dt2.Rows[j]["afferenteInvestimentiPNRR"].ToString()));
                    aggiudicazioneA3_6Type.afferenteInvestimentiPNRR = afferenteInvestimentiPNRR;
                    ...
                    aggiudicazioneA3_6Type.cupLotto = cupLottoList;

                    aggiudicazioneA3_6Type.ccnl = dt2.Rows[j]["ccnl"].ToString();

                    Categoria cat2 = new Categoria();
                    cat2.idTipologica = "categoria";
                    cat2.codice = dt2.Rows[j]["pcp_Categoria"].ToString();
                    aggiudicazioneA3_6Type.categoria = cat2;

                    CodIstat istat = new CodIstat();
                    istat.idTipologica = "codIstat";
                    istat.codice = dt2.Rows[j]["codIstat"].ToString();
                    aggiudicazioneA3_6Type.codIstat = istat;


                    //Se afferenteInvestimentiPNRR è true allora procedo con la composizione del nodo ParitaDiGenereGenerazionale
                    if (afferenteInvestimentiPNRR == true)
                    {
                        aggiudicazioneA3_6Type.paritaDiGenereGenerazionale = RecuperaParitaDiGenereGenerazionale(dt);
                        aggiudicazioneA3_6Type.acquisizioneCup = UtilsConvert.ToBool(dt2.Rows[j]["acquisizioneCup"].ToString());
                    }

                    aggiudicazioneA3_6Type.ccnl = "non applicabile";



                    #region QuadroEconomicoStandard A3_6
                    QuadroEconomicoStandard qs = new QuadroEconomicoStandard();
                    if (!string.IsNullOrEmpty(dt2.Rows[j]["UlterioriSommeNoRibasso"].ToString()))
                    {
                        qs.ulterioriSommeNoRibasso = UtilsConvert.ToDecimal(dt2.Rows[j]["UlterioriSommeNoRibasso"].ToString());
                    ...
                    aggiudicazioneA3_6Type.quadroEconomicoStandard = qs;
                    List<PartecipanteA3_6> partecipantiA3_6 = new List<PartecipanteA3_6>();
                    string dataAggiudicazione = "";
                    string oggettoContratto = "";
                    for (int i = 0; i < dt3.Rows.Count; i++)
                    {
                        ...
                        if (dt2.Rows[j]["NumeroLotto"].ToString() == dt3.Rows[i]["NumeroLotto"].ToString())
                        {
                            PartecipanteA3_6 partecipanteA3_6 = new PartecipanteA3_6();
                            partecipanteA3_6.codiceFiscale = dt3.Rows[i]["codiceFiscale"].ToString();
                            partecipanteA3_6.denominazione = dt3.Rows[i]["Denominazione"].ToString();
                            partecipanteA3_6.idPartecipante = dt3.Rows[i]["idPartecipante"].ToString();
                            partecipanteA3_6.importo = UtilsConvert.ToDecimal(dt3.Rows[i]["importo"].ToString());
                            dataAggiudicazione = dt3.Rows[i]["dataAggiudicazione"].ToString();
                            oggettoContratto = dt3.Rows[i]["oggettoContratto"].ToString();
                            partecipantiA3_6.Add(partecipanteA3_6);
                        }

                    }

                    aggiudicazioneA3_6Type.partecipanti = partecipantiA3_6;

                    DatiBaseA3_6 datiBase1 = new DatiBaseA3_6();
                    datiBase1.oggetto = dt.Rows[0]["Oggetto"] != null ? dt.Rows[0]["Oggetto"].ToString() : string.Empty; //TODO: Body da tab Testata?
                    Categoria cat5 = new Categoria();
                    cat5.idTipologica = "oggettoContratto";
                    cat5.codice = oggettoContratto;
                    datiBase1.oggettoContratto = cat5;
                    aggiudicazioneA3_6Type.datiBase = datiBase1;

                    DatiBaseAggiudicazioneAppalto datiBaseAggiudicazioneAppalto = new DatiBaseAggiudicazioneAppalto();
                    datiBaseAggiudicazioneAppalto.dataAggiudicazione = dataAggiudicazione;
                    aggiudicazioneA3_6Type.datiBaseAggiudicazioneAppalto = datiBaseAggiudicazioneAppalto;

                    aggiudicazioni.Add(aggiudicazioneA3_6Type);
        ...
        public AnacFormA1_32 recuperaAnacFormA1_32(int idDoc, Dati_PCP dati, int idDoc_Scheda)
        {
            //GET_DATI_SCHEDA_PCP_HEADER
            cmd.Parameters.Clear();
            cmd.Parameters.AddWithValue("@idGara", idDoc);
            ...
            AnacFormA1_32 anacForm = new AnacFormA1_32();
            AppaltoA1_32 appalto = new AppaltoA1_32();
            List<AggiudicazioneA1_32> aggiudicazioni = new List<AggiudicazioneA1_32>();
            ...
            foreach (DataRow dr1 in dt.Rows)
            {
                ...
                foreach (DataRow dr2 in dt2.Rows)
                {
                    AggiudicazioneA1_32 agg = new AggiudicazioneA1_32();

                    agg.cig = dr2["CIG"].ToString();

                    QuadroEconomicoStandard qc = new QuadroEconomicoStandard();

                    qc.impServizi = 0;
                    if (dt2.Columns.Contains("impServizi"))
                    {
                        qc.impServizi = UtilsConvert.ToDecimal(dr2["impServizi"].ToString());
                    }

                    qc.impLavori = 0;
                    if (dt2.Columns.Contains("impLavori"))
                    {
                        qc.impLavori = UtilsConvert.ToDecimal(dr2["impLavori"].ToString());
                    }

                    qc.impForniture = 0;
                    if (dt2.Columns.Contains("impForniture"))
                    {
                        qc.impForniture = UtilsConvert.ToDecimal(dr2["impForniture"].ToString());
                    }

                    qc.impTotaleSicurezza = 0;
                    if (dt2.Columns.Contains("impTotaleSicurezza"))
                    {
                        qc.impTotaleSicurezza = UtilsConvert.ToDecimal(dr2["impTotaleSicurezza"].ToString());
                    }

                    qc.ulterioriSommeNoRibasso = 0;
                    if (dt2.Columns.Contains("UlterioriSommeNoRibasso"))
                    {
                        qc.ulterioriSommeNoRibasso = UtilsConvert.ToDecimal(dr2["UlterioriSommeNoRibasso"].ToString());
                    }

                    qc.impProgettazione = 0;
                    if (dt2.Columns.Contains("impProgettazione"))
                    {
                        qc.impProgettazione = UtilsConvert.ToDecimal(dr2["impProgettazione"].ToString());
                    }

                    qc.sommeOpzioniRinnovi = 0;
                    if (dt2.Columns.Contains("sommeOpzioniRinnovi"))
                    {
                        qc.sommeOpzioniRinnovi = UtilsConvert.ToDecimal(dr2["sommeOpzioniRinnovi"].ToString());
                    }

                    qc.sommeRipetizioni = 0;
                    if (dt2.Columns.Contains("sommeRipetizioni"))
                    {
                        qc.sommeRipetizioni = UtilsConvert.ToDecimal(dr2["sommeRipetizioni"].ToString());
                    }

                    qc.sommeADisposizione = 0;
                    if (dt2.Columns.Contains("sommeADisposizione"))
                    {
                        qc.sommeADisposizione = UtilsConvert.ToDecimal(dr2["sommeADisposizione"].ToString());
                    }

                    agg.quadroEconomicoConcessioni = qc;

                    agg.numeroOfferteAmmesse = Convert.ToInt32(dr2["numeroOfferteAmmesse"]);

                    if (dt2.Columns.Contains("dataAggiudicazione"))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                    {
                        DatiBaseAggiudicazioneAppalto datiBaseAggiudicazioneAppalto = new DatiBaseAggiudicazioneAppalto();
                        datiBaseAggiudicazioneAppalto.dataAggiudicazione = UtilsConvert.ToString(dr2["dataAggiudicazione"].ToString());
                        agg.datiBaseAggiudicazioneAppalto = datiBaseAggiudicazioneAppalto;
                    }

                    List<OffertaPresentata> offerte = new List<OffertaPresentata>();

                    //CERCO PARTECIPANTI MAPPANDOLI PER CIG CON I LOTTI
                    foreach (DataRow dr3 in dt3.Rows)
                    {
                        if (dr2["CIG"].ToString() == dr3["CIG"].ToString())
                        {
                            offerte.Add(new OffertaPresentata()
                            {
                                idPartecipante = dr3["idPartecipante"].ToString(),
                                importo = Convert.ToDecimal(dr3["importo"].ToString()),
                                aggiudicatario = UtilsConvert.ToBool(dr3["aggiudicatario"].ToString()),
                                ccnl = dr3["ccnl"].ToString()
                            });
                        }
                    }

                    agg.offertePresentate = offerte;
            ...
            anacForm.appalto = appalto;
            anacForm.aggiudicazioni = aggiudicazioni;


            return anacForm;
        }

        public AnacFormA1_33 recuperaAnacFormA1_33(int idDoc, Dati_PCP dati, int idDoc_Scheda)
        {
            AnacFormA1_33 a = new AnacFormA1_33();
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                var appalto = new AppaltoA1_33();


                appalto.motivoUrgenza = new MotivoUrgenza()
                {
                    idTipologica = "motivoUrgenza",
                    codice = dr["pcp_MotivoUrgenza"].ToString()
                };

                appalto.linkDocumenti = dr["pcp_LinkDocumenti"].ToString();

                appalto.relazioneUnicaSulleProcedure = false;
                if (dt.Columns.Contains("pcp_RelazioneUnicaSulleProcedure") && !string.IsNullOrEmpty(dr["pcp_RelazioneUnicaSulleProcedure"].ToString()))
                {
                    appalto.relazioneUnicaSulleProcedure = UtilsConvert.ToBool(dr["pcp_RelazioneUnicaSulleProcedure"].ToString());
                }

                var aggiudicazioni = new List<AggiudicazioneA1_33>();

                foreach (DataRow dr2 in dt2.Rows)
                {
                    var agg = new AggiudicazioneA1_33();
                    ...
                    qes.impLavori = 0;
                    if (dt2.Columns.Contains("impLavori") && !string.IsNullOrEmpty(dr2["impLavori"].ToString()))
                    {
                        qes.impLavori = UtilsConvert.ToDecimal(dr2["impLavori"].ToString());
                    }

                    qes.impServizi = 0;
                    if (dt2.Columns.Contains("impServizi") && !string.IsNullOrEmpty(dr2["impServizi"].ToString()))
                    {
                        qes.impServizi = UtilsConvert.ToDecimal(dr2["impServizi"].ToString());
                    }

                    qes.impForniture = 0;
                    if (dt2.Columns.Contains("impForniture") && !string.IsNullOrEmpty(dr2["impForniture"].ToString()))
                    {
                        qes.impForniture = UtilsConvert.ToDecimal(dr2["impForniture"].ToString());
                    }

                    qes.impTotaleSicurezza = 0;
                    if (dt2.Columns.Contains("impTotaleSicurezza") && !string.IsNullOrEmpty(dr2["impTotaleSicurezza"].ToString()))
                    {
                        qes.impTotaleSicurezza = UtilsConvert.ToDecimal(dr2["impTotaleSicurezza"].ToString());
                    }

                    qes.ulterioriSommeNoRibasso = 0;
                    if (dt2.Columns.Contains("ulterioriSommeNoRibasso") && !string.IsNullOrEmpty(dr2["ulterioriSommeNoRibasso"].ToString()))
                    {
                        qes.ulterioriSommeNoRibasso = UtilsConvert.ToDecimal(dr2["ulterioriSommeNoRibasso"].ToString());
                    }

                    qes.impProgettazione = 0;
                    if (dt2.Columns.Contains("impProgettazione") && !string.IsNullOrEmpty(dr2["impProgettazione"].ToString()))
                    {
                        qes.impProgettazione = UtilsConvert.ToDecimal(dr2["impProgettazione"].ToString());
                    }

                    qes.sommeOpzioniRinnovi = 0;
                    if (dt2.Columns.Contains("sommeOpzioniRinnovi") && !string.IsNullOrEmpty(dr2["sommeOpzioniRinnovi"].ToString()))
                    {
                        qes.sommeOpzioniRinnovi = UtilsConvert.ToDecimal(dr2["sommeOpzioniRinnovi"].ToString());
                    }

                    qes.sommeRipetizioni = 0;
                    if (dt2.Columns.Contains("sommeRipetizioni") && !string.IsNullOrEmpty(dr2["sommeRipetizioni"].ToString()))
                    {
                        qes.sommeRipetizioni = UtilsConvert.ToDecimal(dr2["sommeRipetizioni"].ToString());
                    }

                    qes.sommeADisposizione = 0;
                    if (dt2.Columns.Contains("sommeADisposizione") && !string.IsNullOrEmpty(dr2["sommeADisposizione"].ToString()))
                    {
                        qes.sommeADisposizione = UtilsConvert.ToDecimal(dr2["sommeADisposizione"].ToString());
                    }

                    agg.quadroEconomicoStandard = qes;


                    agg.numeroOfferteAmmesse = Convert.ToInt32(dr2["numeroOfferteAmmesse"].ToString());

                    var offerte = new List<OffertaPresentata>();
        ...
        public AnacFormA2_33 recuperaAnacFormA2_33(int idDoc, Dati_PCP dati, int IdDoc_Scheda)
        {
            AnacFormA2_33 a = new AnacFormA2_33();
            ...
            #region anacForm.appalto
            var appalto = new AppaltoA2_33();


            DatiBaseProceduraA2_33 datiBaseProcedura = new DatiBaseProceduraA2_33();
            TipoProcedura tipoProcedura = new TipoProcedura();
            tipoProcedura.idTipologica = "tipoProcedura";
            tipoProcedura.codice = UtilsConvert.ToString(dt.Rows[0]["tipoProcedura"]);
            datiBaseProcedura.tipoProcedura = tipoProcedura;
            ...
            #region anacForm.aggiudicazioni
            var aggiudicazioni = new List<AggiudicazioneA2_33>();
            for (int i = 0; i < dt2.Rows.Count; i++)
            {
                var aggiudicazione = new AggiudicazioneA2_33();
                ...
                if (dt2.Columns.Contains("sommeADisposizione"))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                {
                    ...
                    if (dt2.Columns.Contains("ulterioriSommeNoRibasso") && !string.IsNullOrEmpty(dt2.Rows[i]["ulterioriSommeNoRibasso"].ToString()))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                    {
                        qes.ulterioriSommeNoRibasso = UtilsConvert.ToDecimal(dt2.Rows[i]["ulterioriSommeNoRibasso"]);
                    }

                    if (dt2.Columns.Contains("impProgettazione") && !string.IsNullOrEmpty(dt2.Rows[i]["impProgettazione"].ToString()))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                    {
                        qes.impProgettazione = UtilsConvert.ToDecimal(dt2.Rows[i]["impProgettazione"]);
                    }
                    ...
                    if (dt2.Columns.Contains("sommeRipetizioni") && !string.IsNullOrEmpty(dt2.Rows[i]["sommeRipetizioni"].ToString()))//non è obbligatorio se non ho un valore da db non lo inserisco nel json
                    {
                        qes.sommeRipetizioni = UtilsConvert.ToDecimal(dt2.Rows[i]["sommeRipetizioni"]);
                    }
        ...
        private List<LottiP7_1_2> recuperaLottiP7_1_2(int idDoc)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (afferenteInvestimentiPNRR == true)
                    {
                        l.paritaDiGenereGenerazionale = RecuperaParitaDiGenereGenerazionale(dt2);
                    }
        ...
        public string recuperaLottiCIG(int idGara, string CIG)
        {
            string strToReturn = "";
            cmd.Parameters.Clear();
            cmd.Parameters.AddWithValue("@IDGARA", idGara);

            //gestisco i CIG UNO PER UNO
            var listaCig = CIG.Split(',').ToList();
            var parameterNames = new List<string>();
            int j = 0;
            foreach (var item in listaCig)
            {               
                parameterNames.Add($"@param{j}");
                cmd.Parameters.AddWithValue($"@param{j}", item);
                j++;
            }

            var inClause = string.Join(", ", parameterNames);
            string strSql_2 = $"SELECT numeroLotto FROM Document_MicroLotti_Dettagli WITH(NOLOCK) WHERE IdHeader = @IDGARA AND CIG IN ({inClause})";
            cmd.CommandText = strSql_2;
            cmd.CommandType = CommandType.Text;
            DataTable dt2 = new DataTable();
            SqlDataAdapter da2 = new SqlDataAdapter();
        ...
        public string recuperaCigMonolotto(int idGara)
        {
            cmd.Parameters.Clear();
            cmd.Parameters.AddWithValue("@idDoc", idGara);
            string strSql = "select CIG from Document_Bando where idHeader = @idDoc";
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = strSql;
            conn.Open();
            string CIG = Convert.ToString(cmd.ExecuteScalar());
            conn.Close();
        ...
        public string getPayloadA1_29(int idGara, string idAppalto, int IdDoc_Scheda, string ListaCIG = "", string DatiElaborazione = "")
        {
            ...
            //Se provengo da una scheda di non aggiudicazione
            if (DatiElaborazione == "NON_AGG")
            {
                if (isMultiLotto)
                {
                    string listaLotti = pu.recuperaLottiCIG(idGara, ListaCIG);
                    respCan29 = cAN29Controller.GenerateXml(idGara.ToString(), operation: "NON_AGG", lotto: listaLotti, idContrattoConv: IdDoc_Scheda, tipoScheda: "A1_29").Content.ReadAsStringAsync().Result;
                }
                else
                {
                    respCan29 = cAN29Controller.GenerateXml(idGara.ToString(), operation: "NON_AGG", idContrattoConv: IdDoc_Scheda, tipoScheda: "A1_29").Content.ReadAsStringAsync().Result;
                }
            }
            else
            {
                if (isMultiLotto)
                {
                    ...
                    respCan29 = cAN29Controller.GenerateXml(idPDA.ToString(), operation: "", lotto: listaLotti, idContrattoConv: IdDoc_Scheda, tipoScheda: "A1_29").Content.ReadAsStringAsync().Result;
                }
                else
                {
                    respCan29 = cAN29Controller.GenerateXml(idPDA.ToString(), operation: "monolotto", idContrattoConv: IdDoc_Scheda, tipoScheda: "A1_29").Content.ReadAsStringAsync().Result;
                }
            }
            ...
            object s = pu.compilaScheda(idGara, eformXml64, "", null, TipoScheda.A1_29, IdDoc_Scheda, ListaCIG, DatiElaborazione);
        ...
        public string getPayloadA1_32(int idGara, string idAppalto, int IdDoc_Scheda)
        {
            ...
            {
                string listaLotti = pu.recuperaLottiContratto(idContratto: IdDoc_Scheda);
                respCan29 = cAN29Controller.GenerateXml(idPDA.ToString(), operation: "", lotto: listaLotti, idContrattoConv: IdDoc_Scheda, tipoScheda: "A1_32").Content.ReadAsStringAsync().Result;
            }
            else
            {
                respCan29 = cAN29Controller.GenerateXml(idPDA.ToString(), operation: "monolotto", idContrattoConv: IdDoc_Scheda, tipoScheda: "A1_32").Content.ReadAsStringAsync().Result;
            }
            ...
            string eform = pu.recuperaEFormXml(idGara, "CAN29");
            string eformXml64 = Convert.ToBase64String(Encoding.UTF8.GetBytes(eform));
            object s = pu.compilaScheda(idGara, eformXml64, "", null, TipoScheda.A1_32, IdDoc_Scheda);

            var baseModelA1_32 = new BaseModelGeneric<SchedaGeneric<BodyA1_32>>();
            baseModelA1_32.scheda = (SchedaGeneric<BodyA1_32>)s;
            baseModelA1_32.idAppalto = idAppalto;

            string json = AnacFormUtils.getJsonWithOptAttrib(baseModelA1_32);

            return json;
        }

        public string getPayloadA1_33(int idGara, string idAppalto, int IdDoc_Scheda)
        {
            ...
            //string espd = pu.recuperaESPDXml(idGara);
            //string espdXml64 = Convert.ToBase64String(Encoding.UTF8.GetBytes(espd));


            object s = pu.compilaScheda(idGara, eformXml64, "", null, TipoScheda.A1_33, IdDoc_Scheda);

            var baseModelA1_33 = new BaseModelGeneric<SchedaGeneric<BodyA1_33>>();
            baseModelA1_33.scheda = (SchedaGeneric<BodyA1_33>)s;
            baseModelA1_33.idAppalto = idAppalto;

            string json = AnacFormUtils.getJsonWithOptAttrib(baseModelA1_33);
        ...
        public string getPayloadA7_1_2(int idGara, string idAppalto, int IdDoc_Scheda)
        {
            PDNDUtils pu = new PDNDUtils();
            Dati_PCP dati = pu.recuperaDatiPerVoucher(idGara);
            object s = pu.compilaScheda(idGara, "", "", dati, TipoScheda.A7_1_2, IdDoc_Scheda);

            var baseModel = new BaseModelGeneric<SchedaGeneric<BodyA7_1_2>>();
            baseModel.scheda = (SchedaGeneric<BodyA7_1_2>)s;
            baseModel.idAppalto = idAppalto;

            string json = AnacFormUtils.getJsonWithOptAttrib(baseModel);

            return json;

        }

        public string getPayloadA2_32(int idGara, string idAppalto, int IdDoc_Scheda)
        {
            PDNDUtils pu = new PDNDUtils();
            object s = pu.compilaScheda(idGara, "", "", null, TipoScheda.A2_32, IdDoc_Scheda);
            var baseModelA2_32 = new BaseModelGeneric<SchedaGeneric<BodyA2_32>>();
            baseModelA2_32.scheda = (SchedaGeneric<BodyA2_32>)s;
            baseModelA2_32.idAppalto = idAppalto;

            string json = AnacFormUtils.getJsonWithOptAttrib(baseModelA2_32);

            return json;
        }

        public string getPayloadA2_33(int idGara, string idAppalto, int IdDoc_Scheda)
        {
            PDNDUtils pu = new PDNDUtils();
            object s = pu.compilaScheda(idGara, "", "", null, TipoScheda.A2_33, IdDoc_Scheda);
            var baseModelA2_33 = new BaseModelGeneric<SchedaGeneric<BodyA2_33>>();
            baseModelA2_33.scheda = (SchedaGeneric<BodyA2_33>)s;
            baseModelA2_33.idAppalto = idAppalto;

            string json = AnacFormUtils.getJsonWithOptAttrib(baseModelA2_33);
        ...
        public object compilaScheda(int idDoc, string eformXml64, string espdXml64, Dati_PCP dati, TipoScheda tipoScheda, int IdDoc_Scheda = 0, string CIG = "", string DatiEaborazione = "")
        {
            ...
            switch (tipoScheda)
            {
                ...
                case TipoScheda.P1_19:
                    ...
                    return schedaP1_19;
                case TipoScheda.P2_19:
                    SchedaP2_19 schedaP2_19 = new SchedaP2_19();
                    AppaltoP2_19 appaltoP2_19 = new AppaltoP2_19();
                    BodyP2_19 bodyP2_19 = new BodyP2_19();
                    AnacFormP2_19 anacFormP2_19 = new AnacFormP2_19();
                    anacFormP2_19 = recuperaAnacFormP2_19(idDoc, dati);
                    bodyP2_19.anacForm = anacFormP2_19;

                    Codice cP2_19 = new Codice();
                    cP2_19.idTipologica = "codiceScheda";
                    cP2_19.codice = "P2_19";
                    schedaP2_19.codice = cP2_19;
                    schedaP2_19.versione = "1.0";
                    schedaP2_19.body = bodyP2_19;
                    //schedaP2_19.body.eform = eformXml64;
                    schedaP2_19.body.espd = espdXml64;
                    return schedaP2_19;
                case TipoScheda.P2_20:
                    SchedaP2_20 schedaP2_20 = new SchedaP2_20();
                    AppaltoP2_20 appaltoP2_20 = new AppaltoP2_20();
                    BodyP2_20 bodyP2_20 = new BodyP2_20();
                    AnacFormP2_20 anacFormP2_20 = new AnacFormP2_20();
                    anacFormP2_20 = recuperaAnacFormP2_20(idDoc, dati);
                    bodyP2_20.anacForm = anacFormP2_20;

                    Codice cP2_20 = new Codice();
                    cP2_20.idTipologica = "codiceScheda";
                    cP2_20.codice = "P2_20";
                    schedaP2_20.codice = cP2_20;
                    schedaP2_20.versione = "1.0";
                    schedaP2_20.body = bodyP2_20;
                    //schedaP2_19.body.eform = eformXml64;
                    schedaP2_20.body.espd = espdXml64;
                    return schedaP2_20;
                case TipoScheda.P1_20:
                    SchedaP1_20 schedaP1_20 = new SchedaP1_20();
                    AppaltoP1_20 appaltoP1_20 = new AppaltoP1_20();
                    BodyP1_20 bodyP1_20 = new BodyP1_20();
                    AnacFormP1_20 anacFormP1_20 = new AnacFormP1_20();
                    anacFormP1_20 = recuperaAnacFormP1_20(idDoc, dati);
                    bodyP1_20.anacForm = anacFormP1_20;

                    Codice cP1_20 = new Codice();
                    cP1_20.idTipologica = "codiceScheda";
                    cP1_20.codice = "P1_20";
                    schedaP1_20.codice = cP1_20;
                    schedaP1_20.versione = "1.0";
                    schedaP1_20.body = bodyP1_20;
                    schedaP1_20.body.eform = eformXml64;
                    schedaP1_20.body.espd = espdXml64;
                    return schedaP1_20;
                case TipoScheda.P2_16:
                ...
                case TipoScheda.A1_29:
                    ...
                    AnacFormA1_29 aFormA1_29 = new AnacFormA1_29();

                    //SE VENGO DA UNA NON AGGIUDICAZIONE
                    if (DatiEaborazione == "NON_AGG")
                    {
                        aFormA1_29 = recuperaAnacFormA1_29NonAgg(idDoc, dati, IdDoc_Scheda, CIG);
                    }
                    else
                    {
                        aFormA1_29 = recuperaAnacFormA1_29(idDoc, dati, IdDoc_Scheda);
                    }
                ...
                case TipoScheda.AD_4:
                    SchedaAD4 sAD4 = new SchedaAD4();
                    AppaltoAD4 appaltoAD_4 = new AppaltoAD4();
                    BodyAD4 bodyAD_4 = new BodyAD4();
                    AnacFormAD4 aFormAD_4 = new AnacFormAD4();
                    aFormAD_4 = recuperaAnacFormAD4(idDoc, dati);

                    Codice cAD_4 = new Codice();
                    cAD_4.idTipologica = "codiceScheda";
                    cAD_4.codice = "AD4";
                    sAD4.codice = cAD_4;
                    sAD4.versione = "1.0";
                    sAD4.body = bodyAD_4;
                    sAD4.body.espd = !string.IsNullOrEmpty(espdXml64) ? espdXml64 : ""; ;
                    sAD4.body.anacForm = aFormAD_4;
                    return sAD4;


                case TipoScheda.A7_1_2:
                    SchedaGeneric<BodyA7_1_2> schedaA7_1_2 = new SchedaGeneric<BodyA7_1_2>();
                    var bodyA7_1_2 = new BodyA7_1_2();

                    AnacFormA7_1_2 anacFormA7_1_2 = recuperaAnacFormA7_1_2(idDoc, dati, IdDoc_Scheda);

                    bodyA7_1_2.anacForm = anacFormA7_1_2;

                    schedaA7_1_2.codice = new Codice()
                    {
                        codice = "A7_1_2",
                        idTipologica = "codiceScheda"
                    };

                    schedaA7_1_2.versione = "1.0";

                    schedaA7_1_2.body = bodyA7_1_2;

                    return schedaA7_1_2;
                case TipoScheda.A1_32:
                    SchedaGeneric<BodyA1_32> schedaA1_32 = new SchedaGeneric<BodyA1_32>();
                    var bodyA1_32 = new BodyA1_32();

                    AnacFormA1_32 anacFormA1_32 = recuperaAnacFormA1_32(idDoc, dati, IdDoc_Scheda);

                    bodyA1_32.anacForm = anacFormA1_32;

                    schedaA1_32.codice = new Codice()
                    {
                        codice = "A1_32",
                        idTipologica = "codiceScheda"
                    };

                    schedaA1_32.versione = "1.0";

                    schedaA1_32.body = bodyA1_32;

                    schedaA1_32.body.eform = eformXml64;

                    return schedaA1_32;

                case TipoScheda.A2_32:
                    SchedaGeneric<BodyA2_32> schedaA2_32 = new SchedaGeneric<BodyA2_32>();
                    var bodyA2_32 = new BodyA2_32();

                    AnacFormA2_32 aFormA2_32 = recuperaAnacFormA2_32(idDoc, dati, IdDoc_Scheda);

                    var cA2_32 = new Codice();
                    cA2_32.idTipologica = "codiceScheda";
                    cA2_32.codice = "A2_32";

                    schedaA2_32.codice = cA2_32;
                    schedaA2_32.versione = "1.0";

                    schedaA2_32.body = bodyA2_32;
                    schedaA2_32.body.anacForm = aFormA2_32;

                    return schedaA2_32;

                case TipoScheda.A3_6:
                    SchedaA3_6 schedaA3_6 = new SchedaA3_6();
                    AppaltoA3_6 appaltoA3_6 = new AppaltoA3_6();
                    BodyA3_6 bodyA3_6 = new BodyA3_6();
                    AnacFormA3_6 anacFormA3_6 = new AnacFormA3_6();
                    bodyA3_6.anacForm = recuperaAnacFormA3_6(idDoc, dati);
                    Codice cA3_6 = new Codice();
                    cA3_6.idTipologica = "codiceScheda";
                    cA3_6.codice = "A3_6";
                    schedaA3_6.codice = cA3_6;
                    schedaA3_6.versione = "1.0";
                    schedaA3_6.body = bodyA3_6;
                    return schedaA3_6;

                case TipoScheda.A1_33:
                    SchedaGeneric<BodyA1_33> schedaA1_33 = new SchedaGeneric<BodyA1_33>();
                    BodyA1_33 bodyA1_33 = new BodyA1_33();
                    AnacFormA1_33 aFormA1_33 = recuperaAnacFormA1_33(idDoc, dati, IdDoc_Scheda);

                    schedaA1_33.codice = new Codice()
                    {
                        codice = "A1_33",
                        idTipologica = "codiceScheda"
                    };

                    schedaA1_33.versione = "1.0";

                    bodyA1_33.anacForm = aFormA1_33;

                    bodyA1_33.eform = eformXml64;

                    schedaA1_33.body = bodyA1_33;


                    return schedaA1_33;
                case TipoScheda.A2_33:
                    SchedaGeneric<BodyA2_33> schedaA2_33 = new SchedaGeneric<BodyA2_33>();
                    var bodyA2_33 = new BodyA2_33();

                    AnacFormA2_33 aFormA2_33 = recuperaAnacFormA2_33(idDoc, dati, IdDoc_Scheda);

                    var cA2_33 = new Codice();
                    cA2_33.idTipologica = "codiceScheda";
                    cA2_33.codice = "A2_32";

                    schedaA2_33.codice = cA2_33;
                    schedaA2_33.versione = "1.0";

                    schedaA2_33.body = bodyA2_33;
                    schedaA2_33.body.anacForm = aFormA2_33;
        ...
        public void salvaIdScheda(int idDoc, string idScheda)
        {
            ...
            cmd.Parameters.Clear();
            cmd.Parameters.AddWithValue("@idScheda", idScheda);
            cmd.Parameters.AddWithValue("@idDoc", idDoc);
        ...
        public void salvaIdContratto(int idRow, string idContratto)
        {
            string strSQL = "update Document_PCP_Appalto_Schede set idContratto = @idContratto where idrow = @idRow";
        ...
        public void salvaDatiPubblicazioneIT(int idGara, DateTime? dataPubblicazione, string idAvvisoPVL)
        {
            cmd.Parameters.Clear();
            cmd.Parameters.AddWithValue("@idGara", idGara);
            cmd.Parameters.AddWithValue("@dataPubblicazione", dataPubblicazione);
            cmd.Parameters.AddWithValue("@idAvvisoPVL", idAvvisoPVL);

            cmd.CommandType = CommandType.StoredProcedure;
            cmd.CommandText = "PCP_AGGIORNA_DATI_PUBBLICAZIONE_IT";
            conn.Open();
        ...
        public void salvaDatiPubblicazioneEU(int idGara, DateTime? dataPubblicazione, string publicationId, string publicationUrl)
        {
            cmd.Parameters.Clear();
            cmd.Parameters.AddWithValue("@idGara", idGara);
            cmd.Parameters.AddWithValue("@dataPubblicazione", dataPubblicazione);
            cmd.Parameters.AddWithValue("@publicationId", publicationId);
            cmd.Parameters.AddWithValue("@publicationUrl", publicationUrl);

            cmd.CommandType = CommandType.StoredProcedure;
            cmd.CommandText = "PCP_AGGIORNA_DATI_PUBBLICAZIONE_EU";
            conn.Open();
        ...
        public void aggiornaCigP1_16(int idDoc, string CIGtoInsert, int NumeroLotto, string TipoDoc)
        {

            string strSQL = "update Document_MicroLotti_Dettagli set CIG = '" + CIGtoInsert + "' where idHeader = " + idDoc + " and NumeroLotto = " + NumeroLotto + " and TipoDoc ='" + TipoDoc + "'";
            cmd.Parameters.Clear();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = strSQL;
            conn.Open();
        ...
        public void UpdateServiceIntegration(int idRowSIC, string statoRichiesta, int numRetry = -1, string msgError = "", string inputWS = "", string outputWS = "", string datoRichiesto = "")
        {
            cmd.Parameters.Clear();
            ...
            if (!string.IsNullOrEmpty(inputWS))
            {
                strSQL += ", inputWS = @inputWS ";
                cmd.Parameters.AddWithValue("@inputWS", inputWS);
            }

            if (!string.IsNullOrEmpty(outputWS))
            {
                strSQL += ", outputWS = @outputWS ";
                cmd.Parameters.AddWithValue("@outputWS", outputWS);
            }

            if (!string.IsNullOrEmpty(datoRichiesto))
            {
                strSQL += ", datoRichiesto = @datoRichiesto ";
                cmd.Parameters.AddWithValue("@datoRichiesto", datoRichiesto);
            }
        ...
        public void aggiornaCigMonolotto(int idDoc, string CIGtoInsert, string TipoDocGara)
        {
            //TO DO da spostare in una stored per velocizzare la manutenzione
            string strSQL = "update Document_Bando set CIG = @CIGtoInsert where idHeader = @idDoc";

            //NEL CASO DI ODC/ODA IL CIG STA SU UN'ALTRA TABELLA 
            if (TipoDocGara.ToUpper() == "ODC")
            {
                strSQL = "update Document_ODC set CIG = @CIGtoInsert where rda_ID = @idDoc";
            }
            if (TipoDocGara.ToUpper() == "ODA")
            {
                strSQL = "update Document_ODA set CIG = @CIGtoInsert where idHeader = @idDoc";
            }

            cmd.Parameters.Clear();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = strSQL;
            cmd.Parameters.AddWithValue("@CIGtoInsert", CIGtoInsert);
            cmd.Parameters.AddWithValue("@idDoc", idDoc);
        ...
        public void insertServiceRequest(int iddoc, int idpfu, string operazioneRichiesta, string integrazione)
        {
            ...
            cmd.Parameters.AddWithValue("@operazioneRichiesta", operazioneRichiesta);
            cmd.Parameters.AddWithValue("@idPfu", idpfu);
            cmd.Parameters.AddWithValue("@integrazione", integrazione);
        ...
        public string recuperaCodiceFiscale(string idAzi)
        {
            string strSql = "select vatValore_FT as codiceFiscale from DM_Attributi with(nolock) where dztNome = 'codicefiscale' and lnk = @idAzi";
            cmd.Parameters.Clear();
            cmd.Parameters.AddWithValue("@idAzi", idAzi);
            cmd.CommandText = strSql;
            cmd.CommandType = CommandType.Text;
            conn.Open();
            string codiceFiscale = cmd.ExecuteScalar().ToString();
            conn.Close();

            return codiceFiscale;
        }

        public string recuperaCodiceAusa(string codiceFiscale)
        {
            string strSql = "select codice_ausa from PCP_CodiciAUSA with(nolock) where codice_fiscale = @codiceFiscale";
            cmd.Parameters.Clear();
            cmd.Parameters.AddWithValue("@codiceFiscale", codiceFiscale);
            cmd.CommandText = strSql;
            cmd.CommandType = CommandType.Text;
            conn.Open();
            string codiceAusa = cmd.ExecuteScalar().ToString();
            conn.Close();
        ...
        public int GetNextPage_For_RecuperaCIg(int idDoc, int PerPage, string TipoDoc)
        {
            string strSql = "select count(*) as NumLotti from Document_microlotti_Dettagli with(nolock) where idheader = @iddoc and Tipodoc = @TipoDoc and isnull(voce,0) = 0 ";
            cmd.Parameters.Clear();
            cmd.Parameters.AddWithValue("@iddoc", idDoc);
            ...
            string strSql1 = "select count(*) as NumCigAssenti from Document_microlotti_Dettagli with(nolock) where idheader = @iddoc and Tipodoc = @Tipodoc and isnull(voce,0) = 0 and isnull(cig,'') = ''  ";
            cmd.CommandText = strSql1;
        ...
        public int SvuotaCigGara(int iddoc, string TipoDoc)
        {
            cmd.Parameters.Clear();
            cmd.Parameters.AddWithValue("@idDoc", iddoc);
            cmd.Parameters.AddWithValue("@TipoDoc", TipoDoc);

            string strSql = "UPDATE Document_microlotti_Dettagli set CIG = '' where idHeader = @idDoc and Tipodoc = @TipoDoc AND VOCE=0";
            ...
            //NEL CASO DI ODC/ODA IL CIG E' SU UN'ALTRA TABELLA 
            if (TipoDoc.ToUpper() == "ODC")
            {
                strSql1 = "update Document_ODC set CIG = '' where rda_ID = @idDoc";
            }
            if (TipoDoc.ToUpper() == "ODA")
            {
                strSql1 = "update Document_ODA set CIG = '' where idHeader = @idDoc";
            }
        ...
        public List<StazioniAppaltanti> RecuperoStazioniAppaltanti(DataTable dtIn)
        {
            try
            {
                ...
                /* Ricavo le funzioni svolte */
                string funzioniSvolte = dtIn.Rows[0]["pcp_FunzioniSvolte"].ToString();
                ...
                /* Recupero i codici fiscali di ente Proponente ed Appaltante */
                string codiceFiscaleAppaltante = dtIn.Rows[0]["codiceFiscaleAppaltante"].ToString();
                string codiceFiscaleProponente = dtIn.Rows[0]["codiceFiscaleProponente"].ToString();

                /* Se CF Proponente non valorizzato o uguale all'appaltante allora non si considera l'ente Proponente */
                bool isEnteProponente = !string.IsNullOrEmpty(codiceFiscaleProponente) && !codiceFiscaleProponente.Equals(codiceFiscaleAppaltante, StringComparison.OrdinalIgnoreCase);


                /* Aggiungo le informazioni relative all'ente Appaltante */
                StazioniAppaltanti sA = new StazioniAppaltanti();

                sA.codiceCentroCosto = dtIn.Rows[0]["centroDiCostoAppaltante"].ToString();
                sA.codiceAusa = dtIn.Rows[0]["codiceAusaAppaltante"].ToString();
                sA.codiceFiscale = codiceFiscaleAppaltante.ToUpper().StartsWith("IT") ? codiceFiscaleAppaltante.Substring(2) : codiceFiscaleAppaltante;

                sA.funzioniSvolte = fsvolte;

                /* se ho solo l'ente Appaltante */
                if (!isEnteProponente)
                {
                    sA.saTitolare = true;
                }
                /* se ho solo entrambi gli enti */
                else
                {
                    /* Compongo i dati dell'ente Proponente */
                    StazioniAppaltanti sP = new StazioniAppaltanti();

                    sP.codiceCentroCosto = dtIn.Rows[0]["centroDiCostoProponente"].ToString();
                    sP.codiceAusa = dtIn.Rows[0]["codiceAusaProponente"].ToString();
                    sP.codiceFiscale = codiceFiscaleProponente.ToUpper().StartsWith("IT") ? codiceFiscaleProponente.Substring(2) : codiceFiscaleProponente;


                    //sP.funzioniSvolte = fsvolte;

                    /* In questo caso l'ente Proponente diventa sATitolare che deve essere false quindi nel caso dell'Appaltante */
                    sP.saTitolare = true;
                    sA.saTitolare = false;
            ...
            {
                throw new Exception($"Eccezione di runtime dal metodo RecuperoStazioniAppaltanti() - {e.Message}", e);
            }
        }

        private ParitaDiGenereGenerazionale RecuperaParitaDiGenereGenerazionale(DataTable dtIn)
        {
            var pg = new ParitaDiGenereGenerazionale();

            string quota30PerCento;
            string motivoDeroga;
            string misurePremiali;
            string quotaInferioreOccupFemminile;
            string quotaInferioreOccupGiovaninle;

            try
            {
                //derogaAQ non lo mettiamo per adesso. Mancanza in testata, da verificare.

                //quota30PerCento
                quota30PerCento = dtIn.Rows[0]["FLAG_PREVISIONE_QUOTA"].ToString();
                switch (quota30PerCento)
                {
                    case "S":
                        pg.quota30PerCento = "SI";
                        break;
                    case "N":
                        pg.quota30PerCento = "NO";
                        break;
                    case "Q":
                        pg.quota30PerCento = "SI_QUOTA_INFERIORE";
                        break;
                    default:
                        pg.quota30PerCento = "NO"; //Fallback
                        break;
                }

                //quotaInferioreOccupFemminile
                quotaInferioreOccupFemminile = dtIn.Rows[0]["QUOTA_FEMMINILE"].ToString();

                if (!string.IsNullOrEmpty(quotaInferioreOccupFemminile))
                {
                    pg.quotaInferioreOccupFemminile = UtilsConvert.ToDecimal(quotaInferioreOccupFemminile);
                }


                //quotaInferioreOccupGiovaninle
                quotaInferioreOccupGiovaninle = dtIn.Rows[0]["QUOTA_GIOVANILE"].ToString();

                if (!string.IsNullOrEmpty(quotaInferioreOccupGiovaninle))
                {
                    pg.quotaInferioreOccupGiovanile = UtilsConvert.ToDecimal(quotaInferioreOccupGiovaninle);
                }

                //motivoDeroga
                motivoDeroga = dtIn.Rows[0]["ID_MOTIVO_DEROGA"].ToString();

                if (!string.IsNullOrEmpty(motivoDeroga))
                {
                    pg.motivoDeroga = new List<MotivoDeroga>();

                    MotivoDeroga _motivoDeroga = new MotivoDeroga
                    {
                        idTipologica = "motivoDeroga",
                        codice = motivoDeroga
                    };

                    pg.motivoDeroga.Add(_motivoDeroga);
                }


                //misurePremiali
                misurePremiali = dtIn.Rows[0]["ID_MISURA_PREMIALE"].ToString();
                if (!string.IsNullOrEmpty(misurePremiali))
                {
                    pg.misurePremiali = new List<MisurePremiali>();

                    MisurePremiali _misurePremiali = new MisurePremiali
                    {
                        idTipologica = "misurePremiali",
                        codice = misurePremiali
                    };

                    pg.misurePremiali.Add(_misurePremiali);
                }
                return pg;
            ...
            {
                throw new Exception($"Eccezione di runtime dal metodo RecuperaParitaDiGenereGenerazionale() - {e.Message}", e);
            }
        }
    }
    ...
    /* FINE DTO per scheda NAG */


    /* INIZIO DTO per scheda A7_1_2 */
    public class BodyA7_1_2
    {
        public AnacFormA7_1_2 anacForm { get; set; }
    }

    public class AnacFormA7_1_2
    {
        public AppaltoA7_1_2 appalto { get; set; }
        public List<AggiudicazioneA7_1_2> aggiudicazioni { get; set; }
    }

    public class AppaltoA7_1_2
    {
    ...
    public class AggiudicazioneA7_1_2
    {
        public string cig { get; set; }
        public QuadroEconomicoStandard quadroEconomicoStandard { get; set; }
        public bool afferenteInvestimentiPNRR { get; set; }
        public bool acquisizioneCup { get; set; } //agg
        public List<string> cupLotto { get; set; } //agg
        public QuadroEconomicoConcessioni quadroEconomicoConcessioni { get; set; }
        public List<PartecipanteAD> partecipanti { get; set; }
        public List<Finanziamenti> finanziamenti { get; set; } //agg
    ...
    public class OffertaPresentata
    {
        public string idPartecipante { get; set; }
        public decimal importo { get; set; }
    ...
    public class ParitaDiGenereGenerazionale
    {
        public bool derogaAQ { get; set; }
        public string quota30PerCento { get; set; }
        public decimal quotaInferioreOccupFemminile { get; set; }
        public decimal quotaInferioreOccupGiovanile { get; set; }
        public List<MotivoDeroga> motivoDeroga { get; set; }
        public List<MisurePremiali> misurePremiali { get; set; }
    }

    public class MotivoDeroga
    {
        public string idTipologica { get; set; }
        public string codice { get; set; }
    }

    public class MisurePremiali
    {
        public string idTipologica { get; set; }
        public string codice { get; set; }
    }

    /* FINE DTO per scheda A7_1_2 */


    /* INIZIO DTO per scheda A1_32 */

    public class BodyA1_32
    {
        public AnacFormA1_32 anacForm { get; set; }
        public string eform { get; set; }
    }

    public class AnacFormA1_32
    {
        public AppaltoA1_32 appalto { get; set; }
        public List<AggiudicazioneA1_32> aggiudicazioni { get; set; }
    }

    public class AppaltoA1_32
    {
        public DatiBaseProcedura datiBaseProcedura { get; set; }
    }

    public class AggiudicazioneA1_32
    {
        public string cig { get; set; }
        public List<OffertaPresentata> offertePresentate { get; set; }
        public int numeroOfferteAmmesse { get; set; }
        public QuadroEconomicoStandard quadroEconomicoConcessioni { get; set; }
        public DatiBaseAggiudicazioneAppalto datiBaseAggiudicazioneAppalto { get; set; }
    }

    /* FINE DTO per scheda A1_32 */


    /* INIZIO DTO per scheda A1_33 */
    public class BodyA1_33
    {
        public AnacFormA1_33 anacForm { get; set; }
        public string eform { get; set; }
    }

    public class AnacFormA1_33
    {
        public AppaltoA1_33 appalto { get; set; }
        public List<AggiudicazioneA1_33> aggiudicazioni { get; set; }
    }

    public class AppaltoA1_33
    {
        public MotivoUrgenza motivoUrgenza { get; set; }
        public string linkDocumenti { get; set; }
        public bool relazioneUnicaSulleProcedure { get; set; }
    ...
    public class AggiudicazioneA1_33
    {
        public string cig { get; set; }
        public QuadroEconomicoStandard quadroEconomicoStandard { get; set; }
        public List<OffertaPresentata> offertePresentate { get; set; }
    ...
    /* FINE DTO per scheda A1_33 */

    public class AnacFormA2_33
    {
        public AppaltoA2_33 appalto { get; set; }

        public List<AggiudicazioneA2_33> aggiudicazioni { get; set; }
    }

    public class AppaltoA2_33
    {
        public string codiceAppalto { get; set; }
        public DatiBaseProceduraA2_33 datiBaseProcedura { get; set; }

    }

    public class DatiBaseProceduraA2_33
    {
        public TipoProcedura tipoProcedura { get; set; }
    }

    public class AggiudicazioneA2_33
    {
        public string cig { get; set; }
    ...
    public class BodyA2_33
    {
        public AnacFormA2_33 anacForm { get; set; }
    }


    /* FINE DTO per scheda A2_33 */
    ...
    /* INIZIO DTO PER SCHEDA A2_32 */

    public class BodyA2_32
    {
        public AnacFormA2_32 anacForm { get; set; }
    }

    public class AnacFormA2_32
    {
        public AppaltoA2_32 appalto { get; set; }

        public List<AggiudicazioneA2_32> aggiudicazioni { get; set; }
    }

    public class AppaltoA2_32
    {
        public string codiceAppalto { get; set; }
        public DatiBaseProceduraA2_32 datiBaseProcedura { get; set; }

    }

    public class DatiBaseProceduraA2_32
    {
        public TipoProcedura tipoProcedura { get; set; }
    }

    public class AggiudicazioneA2_32
    {
        public string cig { get; set; }
        public List<OffertePresentate> offertePresentate { get; set; }
        public decimal numeroOfferteAmmesse { get; set; }
    ...
    /* Inizio DTO per la scheda P2_19 */

    public class BaseModelP2_19
    {
        public SchedaP2_19 scheda { get; set; }
    }

    public class SchedaP2_19
    {
        public Codice codice { get; set; }
        public string versione { get; set; }
        public BodyP2_19 body { get; set; }
        public string espd { get; set; }
        public string eform { get; set; }
    }

    public class BodyP2_19
    {
        public AnacFormP2_19 anacForm { get; set; }
        public string espd { get; set; }
        public string eform { get; set; }
    }

    public class AnacFormP2_19
    {
        public List<StazioniAppaltanti> stazioniAppaltanti { get; set; }
        public AppaltoP2_19 appalto { get; set; }
        public List<LottiP2_19> lotti { get; set; }

    }

    public class AppaltoP2_19
    {
        public string codiceAppalto { get; set; }
        public List<CategorieMerceologiche> categorieMerceologiche { get; set; }
    ...
    public class LottiP2_19
    {
        ...
        public IpotesiCollegamento ipotesiCollegamento { get; set; }
        public Categoria categoria { get; set; }
        public DatiBaseP2_19 datiBase { get; set; }
        public DatiBaseTerminiInvio2 datiBaseTerminiInvio { get; set; }

        public DatiBaseDocumenti datiBaseDocumenti { get; set; }
        public ParitaDiGenereGenerazionale paritaDiGenereGenerazionale { get; set; }
    }

    /* FINE DTO PER SCHEDA P2_19 */

    /* INIZIO DTO PER SCHEDA P1_20 */

    public class BaseModelP1_20
    {
        public SchedaP1_20 scheda { get; set; }
    }

    public class SchedaP1_20
    {
        public Codice codice { get; set; }
        public string versione { get; set; }
        public BodyP1_20 body { get; set; }
        public string espd { get; set; }
        public string eform { get; set; }
    }

    public class BodyP1_20
    {
        public AnacFormP1_20 anacForm { get; set; }
        public string espd { get; set; }
        public string eform { get; set; }
    }

    public class AnacFormP1_20
    {
        public List<StazioniAppaltanti> stazioniAppaltanti { get; set; }
        public AppaltoP1_20 appalto { get; set; }
        public List<LottiP1_20> lotti { get; set; }

    }

    public class AppaltoP1_20
    {
        public string codiceAppalto { get; set; }
        public List<CategorieMerceologiche> categorieMerceologiche { get; set; }
        //public MotivazioneCIG motivazioneCIG { get; set; }
    ...
    public class LottiP1_20
    {
        ...
        public IpotesiCollegamento ipotesiCollegamento { get; set; }
        public Categoria categoria { get; set; }
        public DatiBaseP1_20 datiBase { get; set; }
        public DatiBaseTerminiInvio2 datiBaseTerminiInvio { get; set; }
    ...
    /* FINE DTO PER SCHEDA P1_20 */

    /* Inizio DTO per la scheda P2_20 */

    public class BaseModelP2_20
    {
        public SchedaP2_20 scheda { get; set; }
    }

    public class SchedaP2_20
    {
        public Codice codice { get; set; }
        public string versione { get; set; }
        public BodyP2_20 body { get; set; }
        public string espd { get; set; }
        public string eform { get; set; }
    }

    public class BodyP2_20
    {
        public AnacFormP2_20 anacForm { get; set; }
        public string espd { get; set; }
        public string eform { get; set; }
    }

    public class AnacFormP2_20
    {
        public List<StazioniAppaltanti> stazioniAppaltanti { get; set; }
        public AppaltoP2_20 appalto { get; set; }
        public List<LottiP2_20> lotti { get; set; }

    }

    public class AppaltoP2_20
    {
        public string codiceAppalto { get; set; }
        public List<CategorieMerceologiche> categorieMerceologiche { get; set; }
        //public MotivazioneCIG motivazioneCIG { get; set; }
    ...
    public class LottiP2_20
    {
        ...
        public IpotesiCollegamento ipotesiCollegamento { get; set; }
        public Categoria categoria { get; set; }
        public DatiBaseP2_20 datiBase { get; set; }
        public DatiBaseTerminiInvio2 datiBaseTerminiInvio { get; set; }
    ...
    public class AnacFormAD4
    {
        public List<StazioniAppaltanti> stazioniAppaltanti { get; set; }
        public AppaltoAD4 appalto { get; set; }
        public List<AggiudicazioneAD3Type> aggiudicazioni { get; set; }
    }


    public class AppaltoAD3 : Appalto_AD
    {
        public DatiBase datiBase { get; set; }
    }

    public class AppaltoAD4
    {
        public string codiceAppalto { get; set; }
        public string cig { get; set; }
        public string dataAdesione { get; set; }
        public bool relazioneUnicaSulleProcedure { get; set; }
        public bool opereUrbanizzazioneScomputo { get; set; }
        public DatiBase datiBase { get; set; }
        public DatiBaseProcedura datiBaseSubappalti { get; set; }
    ...
    public class BodyAD4
    {
        public AnacFormAD4 anacForm { get; set; }
        public string espd { get; set; }
    }
    ...
    */
    ...
    public class BaseModelAD4
    {
        public SchedaAD4 scheda { get; set; }
    }
    ...
    public class SchedaAD4
    {
        public Codice codice { get; set; }
        public string versione { get; set; }
        public BodyAD4 body { get; set; }
    }
    ...
    public class BaseModelA3_6
    {
        public SchedaA3_6 scheda { get; set; }
    }

    public class SchedaA3_6
    {
        public Codice codice { get; set; }
        public string versione { get; set; }
        public BodyA3_6 body { get; set; }
    }

    /* Inizio DTO per la scheda A3_6 */

    public class BodyA3_6
    {
        public AnacFormA3_6 anacForm { get; set; }
    }

    public class AnacFormA3_6
    {
        public List<StazioniAppaltanti> stazioniAppaltanti { get; set; }
        public AppaltoA3_6 appalto { get; set; }
        public List<AggiudicazioneA3_6Type> aggiudicazioni { get; set; }
    }

    public class AppaltoA3_6
    {
        public string codiceAppalto { get; set; }
        public bool relazioneUnicaSulleProcedure { get; set; }
        public DatiBase datiBase { get; set; }
    ...
    public class AggiudicazioneA3_6Type
    {
        public string lotIdentifier { get; set; }
        public bool afferenteInvestimentiPNRR { get; set; }
        public bool acquisizioneCup { get; set; }
        ...
        public ParitaDiGenereGenerazionale paritaDiGenereGenerazionale { get; set; }
        public QuadroEconomicoStandard quadroEconomicoStandard { get; set; }
        public List<PartecipanteA3_6> partecipanti { get; set; }
        public DatiBaseA3_6 datiBase { get; set; }
        public DatiBaseCPV CPV { get; set; }
        public DatiBaseContratto datiBaseContratto { get; set; }
        public DatiBaseAccessibilita datiBaseAccessibilita { get; set; }
        public DatiBaseAggiudicazioneAppalto datiBaseAggiudicazioneAppalto { get; set; }


        public class PartecipanteA3_6
        {
            public string idPartecipante { get; set; }
            public string codiceFiscale { get; set; }
            public string denominazione { get; set; }
    ...
    public class RispostaServizi : RispostaBase
    {
        ...
        public class ListaEsiti
        {
            ...
            public string idNuovoAvviso { get; set; }
            public string idNuovaScheda { get; set; }
            public string codiceNuovaScheda { get; set; }
