...
namespace INIPEC.Controllers
{
    public class CAN29Controller : ApiController
    {
        ...
        /// <param name="tipoScheda">parametro opzionale. indica il tiposcheda in questione (utilizzato per indicarlo nel tracciamento della cronologia PCP</param>
        ...
        [HttpGet]
        public HttpResponseMessage GenerateXml(string id = "", int idpfu = -20, string lotto = "", string operation = "", int idContrattoConv = 0, string tipoScheda = "")
        {
            var esito = "";
            var strCause = "";
            ...
            try
            {
                ...
                using (var connection = new SqlConnection(connectionString))
                {
                    ...
                    if (!operation.ToUpper().Equals("DESERTA") && !operation.ToUpper().Equals("NON_AGG"))
                    {
                    ...
                    {
                        //Se il chiamante vuole un numero lotto specifico ( o una lista )
                        if (operation.ToUpper().Equals("NON_AGG") && !string.IsNullOrEmpty(lotto))
                        {
                            numeroLotto = lotto; 
                        }
                        idProc = idDoc;
                    ...
                    var lotti = string.Empty;
                    if (operation.ToUpper().Equals("NON_AGG"))
                        lotti= GetLotti(connection, idProc, numeroLotto, xmlLotti, "INCLUDI_REVOCATI");
                    else
                        lotti = GetLotti(connection, idProc, numeroLotto, xmlLotti);
                    ...
                    //Inserisco la traccia nella Services_Integration_Request per alimentare la cronologia PCP
                    PDNDUtils pu = new PDNDUtils();
                    TipoScheda tipoSchedaEnum;
                     
                    if(tipoScheda == "")
                    {
                        tipoSchedaEnum = pu.recuperaTipoSchedaGara(idProc);
                    }
                    else
                    {
                        tipoSchedaEnum = pu.enumTipoScheda(tipoScheda);
                    }

                    pu.inserisciLogIntegrazione(idProc, "genera-xml-can", "Elaborato", tipoSchedaEnum, "XML_CAN", "", "", xmlbase, DateTime.Now, DateTime.Now, DateTime.Now, idpfu, 0, "OUT");
        ...
        private string GetLotti(SqlConnection connection, int idProc, string numeroLotto, string xmlLotti, string extraParams = "")
        {
            var output = ""; //l'elenco dei blocchi XML dei lotti
            const string strSql = "EXEC E_FORMS_CN16_DATI_LOTTI @idProc,-20,@extraParams,@numeroLotto";
            
            var cmd1 = new SqlCommand(strSql, connection);
            cmd1.Parameters.AddWithValue("@idProc", idProc);
            cmd1.Parameters.AddWithValue("@numeroLotto", numeroLotto);

            cmd1.Parameters.AddWithValue("@extraParams", extraParams);
