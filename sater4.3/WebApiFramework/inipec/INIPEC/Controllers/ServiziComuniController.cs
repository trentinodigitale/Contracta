...
namespace INIPEC.Controllers
{
    public class ServiziComuniController : ApiController
    {
        ...
        {       
            ...
            string idNuovoAvviso = string.Empty;
            ...
            try
            {
                ...
                strCause = "verifica di coerenza stato scheda";
                string statoScheda = pu.getStatoSchedaBando(iddoc);

                //Se si sta chiedendo un esito operazione rispetto ad una pubblicazione avviso e se siamo giù su uno stato terminale di "avviso in pubblicazione" interrompiamo la catena di verifiche
                //  ( questo perchè l'utente potrebbe aver richiesto un esito sincrono tramite l'intefaccia senza aspettare il giro della sentinella
                //      rendendo inutile l'ultimo test che si andrebbe a fare in modo asincrono, sporcando anche la cronologia pcp )
                if ( tipoOperazione == "AV_PUBB" && (statoScheda == "AV_PUBB" || statoScheda == "AV_RETT" || statoScheda == "AV_N_RETT"  || statoScheda == "AV_RICHIESTA_RETT_IN_CORSO") )
                {
                    ...
                    foreach (var item in risposta.listaEsiti)
                    {
                        ...
                        if ((tipoOperazione.ToUpper() != "SC_CONF" && idAppalto == item.idAppalto) || (tipoOperazione.ToUpper() == "SC_CONF" && idScheda == item.idScheda))
                        {
                            ...
                            else
                            {
                                ...
                                messaggioEsito = "1#OK";
                                statoEsitoOperazione = item.dettaglio.codice;
                                idContr = item.idContratto; //Presente solo nel ritorno di un esito-operazione di tipo SC_CONF a fronte di una scheda SC1. gestito il salvataggio in un blocco di codice successivo
                                idNuovoAvviso= item.idNuovoAvviso; //Presente solo nel ritorno di un esito-operazione di tipo AV_RETT
                            }
                            break;
                        }
                    }
                ...
                if (risposta.status == 200 && messaggioEsito.StartsWith("1#") && !( statoEsitoOperazione == "AP_IN_CONF" || statoEsitoOperazione == "SC_IN_CONF" || statoEsitoOperazione == "AV_IN_PUBB" || statoEsitoOperazione == "AV_RICHIESTA_PUBB_IN_CORSO" || statoEsitoOperazione == "AV_RICHIESTA_RETT_IN_CORSO") )
                {
                    ...
                    switch (tipoOperazione)
                    {
                        case ("AP_CONF"):
                            ...
                            //PER LE SCHEDE CHE NON PREVEDONO LA PUBBLICAZIONE IDAVVISO NON RITORNATO OPPURE NON VALORIZZATO
                            if ( !string.IsNullOrEmpty(risposta.listaEsiti[0].idAvviso) )
							{
                            ...
                            if (
								...
								tipoSchedaGara == TipoScheda.A3_6
								)
                            {
                        ...
                        case ("SC_CONF"):
                            ...
                            //Se sono nell'esito-operazione di una scheda SC1 devo recuperare e salvare l'id del contratto ( utile ai fini della creazione del flusso di esecuzione )
                            if (pu.enumTipoScheda(datiScheda.tipoScheda) == TipoScheda.SC1 && !string.IsNullOrEmpty(idContr))
                            {
                                pu.salvaIdContratto(idRow, idContr);
                            }
                            ...
                            try
                            {
                                ...
                                if (flesService.getListaSchede().Contains(datiScheda.tipoScheda) &&
                                    (datiScheda.TipoDoc_IdDoc_Scheda.ToUpper() == "SCRITTURA_PRIVATA" ||
                                     datiScheda.TipoDoc_IdDoc_Scheda.ToUpper() == "CONTRATTO_GARA_FORN")
                                   )
                                {
                                    flesService.updateStato(iddoc, datiScheda.tipoScheda, statoRichiestaLog);
                                }
                            } catch { }
                        ...
                        case ("AV_PUBB"):
                            ...
                            //Se lo stato è passato ad AV_PUBB ( Avviso pubblicato ) richiedo una sentinella di consulta-avviso per poter recuperare i dati di pubblicazione
                            if (statoEsitoOperazione == "AV_PUBB")
                            {

                                var utils = new GeneralUtils();
                                long delay = Convert.ToInt64(utils.getDbParam("PCP", "consulta-avviso", "DelayPostPub", "24"));

                                //La data di pubblicazione per la PVL può essere ottenuta fino a 48 ore dopo la pubblicazione dell'avviso
                        ...
                        case ("AV_RETT"):
                            ...
                            //In caso l'avviso venga rettificato inserisco il nuovo id restituito
                            //Nel caso non venga restituito IdNuovoAvviso
                            if (!string.IsNullOrEmpty(idNuovoAvviso))
                            {
                                pu.AggiornaIdAvviso(iddoc, idNuovoAvviso);
                            }

                            //Se lo stato dell’esito operazione restituisce AV_RETT (rettifica eseguita con successo) dovrà partire un consulta-avviso
                            if (statoEsitoOperazione == "AV_RETT")
                            {
                                var utils = new GeneralUtils();
                                long delay = Convert.ToInt64(utils.getDbParam("PCP", "consulta-avviso", "DelayPostPub", "24"));
        ...
        private bool manageErrorAndRetry(int idRowSIC, int iddoc, int idpfu, int idRow, string statoEsitoOperazione, string tipoOperazione, string messaggioEsito, string endpointWithQS, string rispostaJson, Dati_PCP dati, TipoScheda? tipoScheda)
        {
            ...
            if (statoEsitoOperazione == "AP_IN_CONF" || statoEsitoOperazione == "SC_IN_CONF" ||
                statoEsitoOperazione == "AV_IN_PUBB" || statoEsitoOperazione == "AV_RICHIESTA_PUBB_IN_CORSO" || statoEsitoOperazione == "AV_RICHIESTA_RETT_IN_CORSO")
            {
            ...
            var utils = new GeneralUtils();
            long delay = Convert.ToInt64(utils.getDbParam("PCP", "DelayEsitoOperazione", tipoOperazione,"-1"));
            long numRetryLimit = Convert.ToInt64(utils.getDbParam("PCP", "NumRetryEsitoOperazione", tipoOperazione,"-1"));
            ...
            switch (tipoOperazione)
            {
                ...
                case ("AV_PUBB"):
					...
					{
						//pubblica avviso -> default: ogni 2 ore per 20 volte ( i parametri a DB verranno portati avanti per lavorare ogni 10 ore per un max di 20 volte )
						numRetryLimit = numRetryLimit == -1 ? 20 : numRetryLimit;

                        //Se lo stato lato ANAC/PCP è ancora AV_IN_PUBB allora incremento il numero massimo di tentativi di 20.
                        //Così da avere un totale di 40 (ma solo se l'anac risponde senza errori e ritorna AV_IN_PUBB )
                        if (statoEsitoOperazione == "AV_IN_PUBB" || statoEsitoOperazione == "AV_RICHIESTA_PUBB_IN_CORSO")
                        {
                            numRetryLimit += 20;
                        }
					...
					break;
                //case ("AV_RICHIESTA_RETT_IN_CORSO"):
                case ("AV_RETT"):
                    ...
                    //se AVVISO NON RETTIFICATO NON RIPROVO
                    if (statoEsitoOperazione != "AV_N_RETT")
                    {
                        //pubblica avviso -> default: ogni 2 ore per 20 volte ( i parametri a DB verranno portati avanti per lavorare ogni 10 ore per un max di 50 volte )
                        numRetryLimit = numRetryLimit == -1 ? 20 : numRetryLimit;

                        //Se lo stato lato ANAC/PCP è ancora AV_RICHIESTA_RETT_IN_CORSO allora incremento il numero massimo di tentativi di 20.
                        //Così da avere un totale di 40 (ma solo se l'anac risponde senza errori e ritorna AV_IN_PUBB )
                        if (statoEsitoOperazione == "AV_RICHIESTA_RETT_IN_CORSO")
                        {
                            numRetryLimit += 20;
                        }
