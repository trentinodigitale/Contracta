...
namespace ProtocolloPiTre
{
    public partial class ProtocolloPiTre : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (isAttivo == 1 && idDocumento != "" && ingressoUscita != "" && oggettoProtocollo != "")
                {
                    ...
                    using (rs = cmd1.ExecuteReader())
                    {
                        ...
                        else
                        {
                            ...
                            setTraceAndMail(tab_id, "Nessun allegato trovato - Protocollazione non possibile - idProtGen=" + tab_id);
                    ...
                    if (allegatiMancanti == false)
                    {
                        ...
                        else
                        {
							...
							setTraceAndMail(tab_id, ErroreGestito + " - idProtGen=" + tab_id);
							Response.StatusCode = 500;
                ...
                else
                {
					...
					setTraceAndMail(tab_id,  "Dati di protocollazione obbligatori mancanti - idProtGen=" + tab_id);
					Response.StatusCode = 500;
            ...
            catch (Exception ex)
            {
				...
				setTraceAndMail(tab_id, ex.ToString() + " - idProtGen=" + tab_id);
		...
		private void setTraceAndMail(string tab_id, string strErrore)
		{
			SqlConnection sqlConn1 = null;
			...
			try
			{
                ...
                // vede se ha già inserito una trace per evitare di mandare tante mail
				bool bFound = false;

                string strSql = "select top 1 datediff(hh,[data],getdate()) as delta from CTL_TRACE where iddoc = " + tab_id + " and contesto = 'PROTOCOLLO' order by data desc";
				...
				using (rs = cmd1.ExecuteReader())
				{
					...
					if (rs.Read())
					{
						delta = rs.GetInt32(rs.GetOrdinal("delta"));
						bFound = true;
					}
				}
                ...
                if (delta > 12 || !bFound) 
                { 
                    strSql = "INSERT INTO CTL_TRACE(data,contesto,idDoc,descrizione) VALUES ( getdate(),'PROTOCOLLO'," + tab_id + " ,'" + strErrore.Replace("'", "''") + "');  SELECT SCOPE_IDENTITY();";
					cmd1 = new SqlCommand(strSql, sqlConn1);
					//cmd1.ExecuteNonQuery();
				    int idRow = Convert.ToInt32(cmd1.ExecuteScalar());

                    strSql = "INSERT INTO CTL_Schedule_Process(IdDoc,DPR_DOC_ID,DPR_ID) VALUES (" + idRow.ToString()  +  ",'ALERT_PROTOCOLLAZIONE','INVIO_MAIL' )";
				    cmd1 = new SqlCommand(strSql, sqlConn1);
				    cmd1.ExecuteNonQuery();
				}
