    ...
    public class WidgetController : TsControllerBase
    {
        ...
        public async Task<IActionResult> Get()
        {
            ...
            foreach (var item in allWidgetsInColl)
            {
                ...
                if (!CBool(item["Deleted"]) && (!item.Contains("Pos_permission") || IsEnabled(permessi, CInt(item["Pos_permission"]))))
                {
                ...
                if (counterInfo != null)
                {
                    ...
                    if(rs != null && !rs.EOF && IsEnabled(_session[SessionProperty.Funzionalita], CInt(rs["LFN_PosPermission"])))
                    {
                        ...
                        if (!hasCounterStored)
                        {
                            ...
                            DashBoardMod.GetRSGridCount(mp_OWNER, CLng(idpfu), mp_strTable, strGlobalFilter, mp_FilterHide.Replace("\\'", "'"), mp_RSConnectionString, ref mp_numRec, mp_Top, strSort, mp_timeout, l_strStoredSQL, Solo_Colonne_Usate);
                        }
                        else
                        {
