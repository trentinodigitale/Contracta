const LoadMenuSX = (data, saveGroups) => {
    ...
    if (dataParsed?.length === 0) {
        ...
        emptyDiv.innerHTML = emptyStateDrawer("Nessun gruppo trovato");
        menusx.appendChild(emptyDiv);
    }
    if (saveGroups) {
...
const littleTrashFavoriteClick = (id) => {
    let url = `/api/${apiVersion}/UserHistory/DeleteFavorite`;
    let params = {
        "Id": `${id}`,
        "Link": "",
        "Title": "",
...
const LoadRecentActivity = (data, saveRecentActivity) => {
    ...
    for (let i = 0; i < dataParsed.length; i++) {
        let { id, link, title, breadcrumb, date, isFavorite } = dataParsed[i];

        if (title == "Home") {
            continue;
        }

        if (title == "Offerta Indicativa") {
            title = "Offerta - Richiesta di abilitazione al Sistema Dinamico di Acquisizione";
        }

        if (title == "CONFORMITA_MICROLOTTI_OFF") {
            title = "Conformità Lotto";
        }
...
const HandleButtonBookmarksClick = (e) => {
    closeDrawer();
    openDrawer(``, false, "Preferiti", "Gestione dei Preferiti", false, false, false, false)
...
const LoadBookmarks = (data, saveBookmarks) => {
    ...
    if (dataParsed?.length === 0) {
        let emptyDiv = document.createElement("div");
        emptyDiv.setAttribute("style", "height: 30vh")
        emptyDiv.innerHTML = emptyStateDrawer("Nessun preferito trovato");
        containerRightArea.appendChild(emptyDiv);
    }
...
const HandleButtonUserAreaClick = (e) => {
    ...
    //                                    <div class="card-body">
    //                                        <div class="col-12 flex">
    //                                                <i class="fa fa-user-o me-3 iconBackgroundGreyRounded" aria-hidden="true" style="color: var(--main-header-menu-color)"></i><h5>${UserNome}</h5>
...
const HandleButtonUserUtilityClick = (e) => {
    ...
    //            if (value != "") {
    //                toolbarButtons
    //                    += `<a id="${id}" href="${url}" onclick="${onClick}" class="me-2"> <button type="button" aria-haspopup="true" aria-expanded="false" class="btn btn-link primary-button" style="border-radius:0; line-height:0;" title="${title}"> <span>${value} </span> </button> </a>`
    //            }
    //        }
    ...
    EprocRequest("GET", url, null, null, (err, data) => {
        ...
        openDrawer(`<div class="col-12">
                                    <div class="card">
                                        <div class="card-body">
                                            <div class="col-12 flex">
                                                    <i class="fa fa-user-o me-3 iconBackgroundGreyRounded" aria-hidden="true" style="color: var(--main-header-menu-color)"></i>
                                                    <h5>${LocalHtmlEntities(UserNome)}</h5>
                                                    ...
                                                    <a id="userLogout" href="#logout" onclick="Javascript:try{ CloseAllSub( 'TOOLBAR_HOMELIGHT' ); }catch(e){};NewDocumentAndReset( 'CHANGE_PWD');return false;" class="link_logout toRight">
                                                        ...
                                                        <span>
                                                        Cambio Password
                                                            <i class="fa fa-key me-3" aria-hidden="true"></i>
                                                        </span>
                                                        </button>
                                                    </a>
                                                    <a id="userLogout" href="#logout" onclick="logout();" class="link_logout">
...
const HandleButtonWorkspaceClick = (e) => {
    ...
    //                                    <div class="card-body">
    //                                        <div class="col-12 flex">
    //                                                <i class="fa fa-building-o me-3 iconBackgroundGreyRounded" aria-hidden="true" style="color: var(--main-header-menu-color)"></i><h5>${AziRagioneSociale}</h5>
    //                                        </div>
    //                                        <div class="col-12">
...
const HandleButtonLinkManualiClick = () => {
    let linkManuali = document.getElementById("buttonLinkManuali").getAttribute("link-manuali");
    window.open(linkManuali, '_blank');
}
...
let buttonLinkManuali = document.getElementById("buttonLinkManuali");
if (buttonLinkManuali) {
    buttonLinkManuali.onclick = HandleButtonLinkManualiClick;
}

function LocalHtmlEntities(str)
{
    return String(str)
        .replace(/&/g, '&amp;')
        .replace(/"/g, '&quot;')
