...
const insertWidgetBase = (element, widget) => {
    ...
    if (cta) {
        ...
        if (ctaParsed.length > 0) {
            footer = `
                <div class="footer">
                    <div class="justify-content-end row gx-0">`;
...
const insertWidgetList = (element, widget) => {
    ...
    let restOfBodyParsed = [];
    let rows = "";
    const chunkOf = 100;
    let bigList = bodyParsed.length > chunkOf;
    if (bigList) {
        restOfBodyParsed = bodyParsed.slice(chunkOf, bodyParsed.length);
        bodyParsed = bodyParsed.slice(0, chunkOf);
    }

    rows = calculateRows(bodyParsed);
    ...
    if (bodyElement && bigList) {
        bodyElement.setAttribute("chunkTimes", 0);
    }
    ...
    if (bigList) {
        bodyElement.addEventListener("scroll", (e) => {
            if (bodyElement.scrollTop + bodyElement.getBoundingClientRect().height === bodyElement.scrollHeight) {
                let chunkTimes = bodyElement.getAttribute("chunkTimes");
                bodyElement.setAttribute("chunkTimes", parseInt(chunkTimes) + 1);
                rows = calculateRows(restOfBodyParsed.slice(chunkTimes * chunkOf, chunkTimes * chunkOf + chunkOf ));
...
const calculateRows = (bodyParsed) => {
    let rows = "";
...
const loadGroupsDashboard = (widgetContainer) => {
    EprocRequest("GET", `/api/${apiVersion}/Widget/GroupsWidget`, null, null, (err, data) => {
        ...
        if (data.status != "OK") {
            ...
            if (widgetContainer?.childElementCount == 0 && data.result.length == 0) {
                widgetContainer.classList.add("access_div_splash_masterpage");
            } else {
                widgetContainer.classList.remove("access_div_splash_masterpage");
            }
        }
    })
}
