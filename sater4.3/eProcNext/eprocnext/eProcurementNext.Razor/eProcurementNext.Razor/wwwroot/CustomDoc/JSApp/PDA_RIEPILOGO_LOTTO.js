...
/* CODICE PER GLI E_FORMS / CAN29 */

function downloadXmlCAN29()
{
	ExecDocProcess('XML_CAN29_LOTTO,RIEPILOGO_LOTTO,,NO_MSG');
}

function afterProcess(param) 
{
	if ( param == 'XML_CAN29_LOTTO' )
	{
...
{
	//La generazione dell'xml can29 è stata effettuata dal processo XML_CAN29_LOTTO ed il codice ha inserito il risultato associato alla procedura/bando di gara
	ExecFunction('../../eforms/can29.asp?id=' + getObjValue('IdBando') + '&idpfu=' + idpfuUtenteCollegato + '&operation=download', '_blank', '');
}

/* FINE CODICE PER GLI E_FORMS / CAN29 */


// Per GGAP: apro l'URL fornito da GGAP per completare il lavoro su GGAP
function OpenGgapPage()
{
	...
	// var idDocBando = getObj('IdBando').value;
	// var idDocBandoParam = 'idDocBando=' + idDocBando;

	var idMicroLotti = getObj('IDDOC').value;
	var idMicroLottiParam = 'idMicroLotti=' + idMicroLotti;

	var targetLocationParam = 'targetLocation=Aggiudicazione';

	// var url = "/SimogGgapApi/Gara/GetGgapLandingPage?" + idPfuParam + "&" + idRichiestaCigParam + "&" + idAziParam + "&" + idDocBandoParam + "&" + idMicroLottiParam + "&" + targetLocationParam;
	var url = "/SimogGgapApi/Gara/GetGgapLandingPage?" + idPfuParam + "&" + idRichiestaCigParam + "&" + idAziParam + "&" + idMicroLottiParam + "&" + targetLocationParam;
...
function OpenArt36( objGrid , Row , c )
{
    MakeDocFrom( 'PDA_ART_36#800,800#LOTTO#' + getObjValue( 'LISTA_BUSTEGrid_idRow_' + Row  ) );
}