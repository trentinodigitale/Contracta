...
function OnChangeProcedura(o) {
	...
	if (getObjValue('ProceduraGara') == '15583') {
		...
		getObj("SocietaInHouse").closest("td").style.display = '';
		...
		getObj("SocietaInHouse").closest("td").style.display = 'none';
...
function OnChangeLotti(o) {
	if (getObjValue('Divisione_lotti') != '' && getObjValue('Divisione_lotti') != '0' && getObjValue('Divisione_lotti') != '2') {
		...
		try {
			...
			getObj("Complex").closest("td").style.display = '';
		...
		try {
			...
			getObj("Complex").closest("td").style.display = 'none';
...
function LocSaveDoc() {
	...
	/* ALMENO PER ORA IL REGIME ALLEGERITO NON DEVE PIU' ESSERE OBBLIGATORIO   <-- requisito tornato valido con le schede P1_20 e P2_20 */
	try {
	...
	//Il campo "regime allegerito" è obbligatorio a meno delle procedure di "Affidamento diretto" e "Richiesta Preventivo"
	...
	if (!(valProceduraGara == '' || valProceduraGara == '15583' || valProceduraGara == '15479') && objRegAllVal == '') // || valProceduraGara == '15477' ) && objRegAllVal == '' )
	{
...
function OnChangeTipoAppalto(obj) {
	...
	if (
		...
		if (getObj('TipoProceduraCaratteristica').value != 'Cottimo') {
			...
			getObj("Concessione").closest("td").style.display = '';
			...
			getObj("Concessione").closest("td").style.display = 'none';
		}
	}
	else {
		//-- nascondi
...
function manageRegimeAllegerito() {
	...
	var myRegAllCaption = document.getElementById('cap_RegimeAllegerito')
	...
	if (objCapRegAll) {
		...
		//Il campo ( regime allegerito ) non dovrà essere visibile  nel caso in cui il tipo procedura sia: Affidamento diretto, Negoziata o Ristretta  <--- condizioni non più valide

		//Il campo ( regime allegerito ) non dovrà essere visibile  nel caso in cui il tipo procedura sia: Affidamento diretto o Richiesta Preventivo
		//if (valProceduraGara == '' || valProceduraGara == '15583' || valProceduraGara == '15478' || valProceduraGara == '15477') {
		if (valProceduraGara == '' || valProceduraGara == '15583' || valProceduraGara == '15479') {
			objRegAll.value = '';
