  ...
  */
  ...
  if (DOCUMENT_READONLY_LOCAL == '0') {
    ...
    if (strTipoBando != '') 
	{
	  ...
	  if (getObj('RTESTATA_PRODOTTI_MODEL_TipoBandoScelta'))
	  {
  ...
  //aggiunta tooltip per pcp_iniziativeNonSoddisfacenti
  $("#cap_pcp_iniziativeNonSoddisfacenti, #pcp_iniziativeNonSoddisfacenti").attr("title", "Nessuna delle iniziative disponibili presso i soggetti aggregatori di riferimento ha caratteristiche in grado di soddisfare i fabbisogni di questa stazione appaltante?");
  
  //se affidamento diretto metto visibile il campo SocietaInHouse, in tutti gli altri casi lo setto non visibile
  try 
  {
	  if ( getObjValue( 'ProceduraGara' ) == '15583' )
	  {
		  setVisibility(getObj('cap_SocietaInHouse'), '');
		  setVisibility(getObj('SocietaInHouse'), '');        
	  }
	  else
	  {
		  try {
			getObj('SocietaInHouse').value='no';
			setVisibility(getObj('cap_SocietaInHouse'), 'none');
			setVisibility(getObj('SocietaInHouse'), 'none');
		  }
		  catch (e){}
	  }
...
function OnClickProdotti(obj) 
{
  ...
  //SE IL CAMPO RTESTATA_PRODOTTI_MODEL_Ambito NON ESISTE ALLORA LA SEZIONE TESTATA_PRODOTTI NON EDITABILE
  var ObjAmbito = getObj('RTESTATA_PRODOTTI_MODEL_Ambito');
  
  if (ObjAmbito == undefined )
		DOCUMENT_READONLY = '1' 
...
function ControlliSend(param, param2, chiamante) {
  ...
  try 
  {
	...
	{
		var checkAppalto_PNRR_PNC = false;
		
		if ( isDocReadOnly )
		{
			if ( getObjValue('Appalto_PNRR_PNC') == '1' )
				checkAppalto_PNRR_PNC = true;
		}
		else
		{
			checkAppalto_PNRR_PNC = getObj('Appalto_PNRR_PNC').checked;
		}

      //recupero il valore della spunta
      if ( checkAppalto_PNRR_PNC ) 
	  {
			...
			if ( isDocReadOnly )
			{
				valAppalto_PNRR = getExtraAttrib('val_Appalto_PNRR', 'value')
				valAppalto_PNC = getExtraAttrib('val_Appalto_PNC', 'value')
				motivazionePNRR = getObj('Motivazione_Appalto_Verde_V').innerHTML.replace("&nbsp;", "");;
				motivazionePNC = getObjValue('Motivazione_Appalto_PNC_V').innerHTML.replace("&nbsp;", "");;
			}
			else
			{
				valAppalto_PNRR = getObjValue('Appalto_PNRR');
				valAppalto_PNC = getObjValue('Appalto_PNC');
				motivazionePNRR = getObjValue('Motivazione_Appalto_PNRR');
				motivazionePNC = getObjValue('Motivazione_Appalto_PNC');
			}
		...
		(		
          ...
          ( valAppalto_PNRR == '' && valAppalto_PNC == '')
          ||
          ( valAppalto_PNRR == 'si' && motivazionePNRR == '')
          ||
          ( valAppalto_PNRR == '' && motivazionePNRR != '')
          ||
          ( valAppalto_PNC == 'si' && motivazionePNC == '')
          ||
          ( valAppalto_PNC == '' && motivazionePNC != '')
        )
  ...
  if (getObjValue('importoBaseAsta') != 0.0) {
	...
	else
	{
		...
		if ( getExtraAttrib('val_TipoIVA', 'value') == '') 
		{
			//getObj('TipoIVA').focus();
		  DMessageBox('../', 'In presenza di un valore per "Importo Base Asta" e\' necessario selezionare un valore per il campo "Iva"', 'Attenzione', 1, 400, 300);
		  return -1;
  ...
  } else //-- per i restanti casi
  {
    ...
    if (TPC != 'AffidamentoSemplificato' && TPC != 'RDO' )
...
function PrintAndSend(param, param2) {
  ...
  //se doc editablie e se attiva la PCP e sto facendo INVIO all'approvazione
  //controllo i campi della PCP
  if ( isDocReadOnly == 0 && hasPCP() && getObj('Controlla_Campi_PCP').value =='1' )
  {  
...
function UpdateModelloBando() {
  ...
  if ( docReadonly == '0' )
  {  
	  //SE DOC EDITABILE MA IL CAMPO RTESTATA_PRODOTTI_MODEL_Ambito NON ESISTE ALLORA LA SEZIONE TESTATA_PRODOTTI NON EDITABILE
	  var ObjAmbito = getObj('RTESTATA_PRODOTTI_MODEL_Ambito');
	  if (ObjAmbito == undefined )
	  {	  
		DMessageBox('../', 'Documento in sola lettura', 'Attenzione', 1, 400, 300);
...
function checkDataScadenzaOfferta() {
	...
	{
		if ( getObjValue('ProceduraGara') == '15583' ) //-- affidamento diretto
		{
			if (CheckData('DataScadenzaOfferta', Riferimento, 'Compilare Data Scadenza Offerta', 'Data Scadenza Offerta non deve essere nel passato', '') == -1) return -1;
		}
		else
		{
...
function afterProcess(param) {
	...
	//Se il processo di controlli base è stato completato con successo ( innescato dal conferma appalto per pcp )
...
function openGEO_simog() 
{
  ...
  //se il campo CriterioAggiudicazioneGara NON E' una combo ESCO SUBITO perchè vuol dire non editabile (INVITO DELLA BIFASICA)
  if ( getObj('CriterioAggiudicazioneGara').type != 'select-one')
		return;
...
function onChangeAppalto_PNRR()
{
	...
	{	
		...
		var TipoScheda = getObjValue('pcp_TipoScheda');		
		var schedeAppalto_PNRR = ['AD4', 'AD5' , 'AD3'];
...
function PCP_ConfermaAppalto() 
{
	...
	//se doc editabile chiamo il controllo dei campi della PCP
	//e il caso di quando e in approvazione
	if ( isDocReadOnly == '0' )
	{
		if ( PCP_obbligatoryFields(true) == -1 )
		{
...
function PCP_ConfermaAppalto_End() {
  ...
  //se tipo scheda P1_16 oppure P1_19 aggiungo il messaggio InfoTED altrimenti nulla
  TipoScheda = getObjValue('pcp_TipoScheda');
  
  if (TipoScheda == 'P1_16' || TipoScheda == 'P1_19')
  {
	DMessageBoxWithAction('../', 'Conferma appalto eseguito correttamente', 'Informazione', 1, 400, 300, 'InfoTED');
  }
  else
  {
...
function InfoTED() {
	DMessageBox('../', 'Per finalizzare correttamente la pubblicazione su TED è necessario cliccare sul pubblica avviso le prossime 24h', 'Informazione', 1, 400, 300);
}
...
function PCP_showOrHideFields() {
  ...
  */
  ...
  //Richiamo la funzione per settare l'obbligatorietà dei campi
  onChangeAppalto_PNRR();
  ...
  //SE SI TRATTA DI UNA SCHEDA P7_2 ALLORA NASCONDO L'AREA Indirizzo dei documenti di gara (BT-15) SE PRESENTE
  var TipoScheda = getObjValue('pcp_TipoScheda');
  var HideInfoInvio_P7_2;
  HideInfoInvio_P7_2 = false;
  
  if (TipoScheda == 'P7_2')
  {	
	//SE PRESENTE IL CAMPO 
	if ( getObj('cn16_CallForTendersDocumentReference_ExternalRef') )
	{
		//SE GARA NON IN LAvORAZIONE SE NON VALORIZZATO LO NASCONDO
		var DOCUMENT_READONLY = isDocumentReadonly();
		if ( DOCUMENT_READONLY == 0 )
		{	
			if ( getObj('cn16_CallForTendersDocumentReference_ExternalRef').value == '' )
				HideInfoInvio_P7_2 = true;
		}else
		{	
			//SVUOTO E NASCONDO
			getObj('cn16_CallForTendersDocumentReference_ExternalRef').value = '';
			HideInfoInvio_P7_2 = true;
			
		}
		
		//NASCONDO IL CAMPO e LA LABEL DI RAGGRUPPAMENTO
		if ( HideInfoInvio_P7_2 )
		{	
			getObj("cap_cn16_CallForTendersDocumentReference_ExternalRef").closest("td").parentElement.closest("td").style.display = 'none';
			getObj("cap_Static4").closest("td").parentElement.closest("tr").style.display = 'none';
		}
	}		
  }
}
...
function PCP_obbligatoryFields(clicked ) 
{
	...
	{	
		...
		if ( getObj('pcp_VersioneScheda') ) 
		{
			...
			{	
				//BVEP-7407  (refuso, il campo non va aggiunto agli obbligatori in quanto deve essere nascosto)
    ...
    if(TipoScheda == 'P2_16')
		listaCampiObblig = 'CATEGORIE_MERC@DESC_LUOGO_ISTAT@Appalto_PNRR@pcp_CodiceCentroDiCosto@pcp_MotivoUrgenza@pcp_ContrattiDisposizioniParticolari@pcp_PrestazioniComprese@pcp_ServizioPubblicoLocale@MOTIVO_COLLEGAMENTO@pcp_PrevedeRipetizioniCompl@pcp_lavoroOAcquistoPrevistoInProgrammazione@pcp_PrevedeRipetizioniOpzioni@pcp_Categoria@cn16_CallForTendersDocumentReference_ExternalRef';

	
    if (TipoScheda == 'P7_1_3')
	{
        listaCampiObblig = 'CATEGORIE_MERC@DESC_LUOGO_ISTAT@Appalto_PNRR@DataTermineQuesiti_V@pcp_CodiceCentroDiCosto@pcp_Categoria@pcp_ServizioPubblicoLocale@pcp_PrestazioniComprese@pcp_saNonSoggettaObblighi24Dicembre2015@pcp_CondizioniNegoziata@MOTIVAZIONE_CIG@TIPO_FINANZIAMENTO@pcp_ImportoFinanziamento@pcp_iniziativeNonSoddisfacenti@pcp_ContrattiDisposizioniParticolari@pcp_lavoroOAcquistoPrevistoInProgrammazione';
	...
	if (TipoScheda == 'P1_19')
	{	
		listaCampiObblig ='pcp_Categoria@pcp_CodiceCentroDiCosto@Appalto_PNRR@MOTIVO_COLLEGAMENTO@pcp_ServizioPubblicoLocale@pcp_ImportoFinanziamento@TIPO_FINANZIAMENTO@pcp_PrevedeRipetizioniOpzioni@pcp_lavoroOAcquistoPrevistoInProgrammazione@pcp_ripetizioniEConsegneComplementari@cn16_FundingProgramCode_eu_funded@cn16_TendererRequirementTypeCode_reserved_proc@cn16_ExecutionRequirementCode_reserved_execution@cn16_CallForTendersDocumentReference_DocumentType@cn16_CallForTendersDocumentReference_ExternalRef@cn16_OrgRicorso_Name@cn16_OrgRicorso_ElectronicMail@cn16_OrgRicorso_CompanyID@cn16_OrgRicorso_CityName@cn16_OrgRicorso_Telephone@cn16_OrgRicorso_codnuts@cn16_OrgRicorso_cap';
	
		//aggiungo alla lista degli obblig della P1_19 il campo pcp_ModalitaAcquisizione se la gara è TipoAppaltoGara forniture oppure servizi
		
		if ( getObj('TipoAppaltoGara').value == '1' || getObj('TipoAppaltoGara').value == '3' )
		{	
			listaCampiObblig = listaCampiObblig  + '@pcp_ModalitaAcquisizione' ;
		}
  ...
  if (TipoScheda == 'P2_19')
	{	
		listaCampiObblig ='pcp_Categoria@pcp_CodiceCentroDiCosto@MOTIVO_COLLEGAMENTO@Appalto_PNRR@pcp_ServizioPubblicoLocale@pcp_ImportoFinanziamento@TIPO_FINANZIAMENTO@pcp_PrevedeRipetizioniOpzioni@pcp_lavoroOAcquistoPrevistoInProgrammazione@pcp_ripetizioniEConsegneComplementari@cn16_CallForTendersDocumentReference_ExternalRef@CATEGORIE_MERC';
	
		//aggiungo alla lista degli obblig della P2_19 il campo pcp_ModalitaAcquisizione se la gara è TipoAppaltoGara forniture oppure servizi
  ...
  if (TipoScheda == 'P2_20')
	{	
		listaCampiObblig ='pcp_Categoria@pcp_PrevedeRipetizioniCompl@pcp_CodiceCentroDiCosto@MOTIVO_COLLEGAMENTO@Appalto_PNRR@pcp_ServizioPubblicoLocale@pcp_ImportoFinanziamento@TIPO_FINANZIAMENTO@pcp_PrevedeRipetizioniOpzioni@pcp_lavoroOAcquistoPrevistoInProgrammazione@cn16_CallForTendersDocumentReference_ExternalRef@CATEGORIE_MERC';
	
		//aggiungo alla lista degli obblig della P2_20 il campo pcp_ModalitaAcquisizione se la gara è TipoAppaltoGara forniture oppure servizi
	  ...
	  if (TipoScheda == 'P1_20')
	{	
		listaCampiObblig ='pcp_Categoria@pcp_PrevedeRipetizioniCompl@pcp_CodiceCentroDiCosto@MOTIVO_COLLEGAMENTO@Appalto_PNRR@pcp_ServizioPubblicoLocale@pcp_ImportoFinanziamento@TIPO_FINANZIAMENTO@pcp_PrevedeRipetizioniOpzioni@pcp_lavoroOAcquistoPrevistoInProgrammazione@cn16_CallForTendersDocumentReference_ExternalRef@CATEGORIE_MERC@pcp_ContrattiDisposizioniParticolari@pcp_MotivoUrgenza@cn16_TendererRequirementTypeCode_reserved_proc@cn16_FundingProgramCode_eu_funded@cn16_CallForTendersDocumentReference_DocumentType@cn16_OrgRicorso_Name@cn16_OrgRicorso_CompanyID@cn16_OrgRicorso_CityName@cn16_OrgRicorso_ElectronicMail@cn16_OrgRicorso_Telephone@cn16_OrgRicorso_codnuts@cn16_OrgRicorso_cap';
	
		//aggiungo alla lista degli obblig della P1_20 il campo pcp_ModalitaAcquisizione se la gara è TipoAppaltoGara forniture oppure servizi
	...
	//SE SCHEDA AD3 controllo solo CUP kpf 580667
	if( TipoScheda == 'AD3' )
	{
		if(getObjValue('Appalto_PNRR') == 'si')
		{
			listaCampiObblig = listaCampiObblig  + '@CUP' ;
		}
	}
  ...
  if (clicked) 
  {
    if (bFoundObblig) {
	  ...
	  //if ( chiamante == 'PrintAndSend' )  
		return -1 
	  
    }
    else {
      
	  //chiamo il processo che poi innesca il conferma appalto solo se non lo sto chiamando dalla funzione PrintAndSend
...
function PCP_CodiceCentroDiCosto(Ente) {
	...
	{	
		...
		if(ajax)
		{
			...
			ajax.open("GET", pathRoot + "../WebApiFramework/api/ConfermaAppalto/recuperaCDC?idDoc=" + iddoc + '&idAzi=' + idAzi +'&nocache=' + nocache, false);
