function Controllo_Valori() 
{
	...
	//var IdPfu = getObj( 'IdPfu' ).value;
	var IdPfu = getObj( 'IdPfuLog' ).value;
	var Fornitore = getObj( 'Fornitore' ).value;
	var datalogda = getObj( 'datalogda' ).value;
	var dataloga = getObj( 'dataloga' ).value;
	var Fascicolo = getObj( 'Fascicolo' ).value;

	
	if (IdPfu ==  '' && Fornitore == '' && Fascicolo == '')
		{
			DMessageBox( '../ctl_library/' , 'Almeno uno dei campi "Utente", "Fornitore" oppure "Fascicolo" deve essere compilato' , 'Attenzione' , 1 , 400 , 300 ); 
			return false;		
		}
	if (datalogda == '' || dataloga == '' )
		{
			
			DMessageBox( '../ctl_library/' , 'Compilare i campi "Giorno Lavorativo da" e "Giorno Lavorativo a" correttamente' , 'Attenzione' , 1 , 400 , 300 ); 
			return false;
