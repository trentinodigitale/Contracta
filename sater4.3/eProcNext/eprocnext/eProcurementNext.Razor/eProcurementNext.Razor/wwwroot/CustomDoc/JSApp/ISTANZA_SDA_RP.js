...
function SetInitField() {
    ...
    //Se lo stato � avvalorato ed � diverso da italia
...
function DISPLAY_FIRMA_OnLoad() {
    try {
        //uso questo metodo al posto di afterprocess in quanto non disponibile parti del documento, cos� faccio l'operazione a documento catricato
...
function controlli(param) {
    if (getObj('DOCUMENT_READONLY').value != '1') {
        ...
        if (codiceStato != '' && codiceStato != 'M-1-11-ITA') {
            for (i = 0; i < NumControlliEsteri; i++) {
                //se l'azienda non aveva la PIVA allora � facoltativo altrimenti obbligatorio
...
function OnClickProdotti(obj) {
    ...
    if (Stato == '') {
        DMessageBox('../', 'Prima di procedere con l\'importazione dei prodotti � necessario effettuare un salvataggio del documento', 'Attenzione', 1, 400, 300);
    }
    else {
