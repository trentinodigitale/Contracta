...
$(document).ready(function () {
    if (getObj('DOCUMENT_READONLY')) {
        var isDocumentReadOnly = getObj('DOCUMENT_READONLY').value;
    }
    if (isDocumentReadOnly === '1') {
        $(".Fld_Not_Essential").removeClass("Fld_Not_Essential");
    }
...
function OnLoadPage() {
    ...
    //-- se lo stato funzionale è quello di creazione della RDA deve creare il modello da associare alla gestione
    ...
    if (getObj('attivoSimog')) {
        //Se non è attivo il simog  oppure siamo sul primo giro delle gare ( Avviso - Negoziata oppure se Bando - Ristretta )
    ...
    if (getObjValue('ProceduraGara') == '15478' && getObjValue('TipoBandoGara') == '4') //-- Negoziata - Avviso con risposta
    {
        ...
        try{ setVisibility(getObj('cap_DataAperturaOfferte').offsetParent.offsetParent, 'none');}catch(e){}
    ...
    */
    ...
    if (DOCUMENT_READONLY_LOCAL == '0') {
		...
		// Funzioni chiamate per avere i campi come nel wizard
		
		SET_FIELD_OnChangeTipoSceltaContraente(this);
		SET_FIELD_OnChangeProcedura(this);
		SET_FIELD_OnChangeLotti(this);
		SET_FIELD_OnChangeCriterio(this);
		/*
        {
            FilterFieldsBasedOnTipoSceltaContraente();

            // TODO:
            // OnChangeLotti(this);
            FilterBasedOnDivisioneLotti();

            FilterBasedOnProcedura();
            ...
            // {
            //   FilterDom('TipoAppaltoGara', 'TipoAppaltoGara', getObjValue('TipoAppaltoGara'), 'SQL_WHERE=  DMV_COD <> \'4\'', '', 'OnChangeTipoAppalto(this)');
            //   // FilterDom('TipoAppaltoGara', 'TipoAppaltoGara', getObjValue('TipoAppaltoGara'), 'SQL_WHERE=  DMV_COD <> \'4\'', '', 'OnChangeTipoAppalto(this)');
    ...
    if (DOCUMENT_READONLY_LOCAL == '0') {
        // TODO: commentato dopo aver introdotto la logica sui criteri che si comporta come nel wizard
        //set_Complex();
    ...
    //allineo come larghezza il campo "QuestionariAmministrativo" a "PresenzaDGUE"
...
function CheckControlloSuperamentoImportoGara() {
    ...
    if (!(
        (proceduragara == '15476' && TipoBandoGara == '2')
        ||
        (proceduragara == '15477' && TipoBandoGara == '3')
        ||
        (proceduragara == '15478' && TipoBandoGara == '3')
        ||
        !TipoProcedura == 'RDO'
    )) {
        try {
...
function onChangeCalcoloSoglia(obj) 
{
    try 
	{
		...
		// {
			...
			// SelectreadOnly('ModalitaAnomalia_ECO',false);
			// ReadOnlyObj('ScontoDiRiferimento_V',false);
...
// function onChangeMetodoDiCalcolo(obj) {
	// if (getObj('METODO_DI_CALCOLO_ANOMALIA').value == 'Metodo C') {
		// ReadOnlyObj('ScontoDiRiferimento_V',false);
		// // if (getObj('ScontoDiRiferimento_V').value == '') {
			// // DMessageBox('../', 'Il campo sconto di riferimento è obbligatorio', 'Attenzione', 1, 400, 300);
			// // return;
		// // }	
	// } else {
		// getObj('ScontoDiRiferimento_V').value = '';
	...
	// if (getObj('TipoSoglia').value == 'sotto' && getObj('Appalto_PNRR_PNC').checked == false) {
		// if (getObj('CriterioAggiudicazioneGara').value == '15531'&& getObj('TipoAppaltoGara').value == '1') {	
			// // Se offerta anomala è pari a Valutazione allora bisogna scegliere solo il metodo di calcolo A
			// if (getObj('OffAnomale').value == '16310') {
				// FilterDom( 'METODO_DI_CALCOLO_ANOMALIA' ,  'METODO_DI_CALCOLO_ANOMALIA' , 'No' , 'SQL_WHERE=  DMV_Cod = \'Metodo A\' ' , '' , 'controlliCombinatorieCalcoloAnomalie();');
				// getObj('METODO_DI_CALCOLO_ANOMALIA').value = 'Metodo A';
			// }

			// // Se offerta anomala è pari a Manuale allora accetta solo l'opzione vuota in metodo di calcolo
			// if (getObj('OffAnomale').value == '16311') {
				// FilterDom( 'METODO_DI_CALCOLO_ANOMALIA' ,  'METODO_DI_CALCOLO_ANOMALIA' , 'No' , 'SQL_WHERE=  DMV_Cod = \'\' ', '' , 'onChangeMetodoDiCalcolo();');
			// }	
		// }
		
		// if (getObj('CriterioAggiudicazioneGara').value == '15532') {	
			// if (getObj('OffAnomale').value == '16310') {
				// FilterDom( 'METODO_DI_CALCOLO_ANOMALIA' ,  'METODO_DI_CALCOLO_ANOMALIA' , 'No' , 'SQL_WHERE=  DMV_Cod = \'Metodo 4/5\' ' , '' , 'controlliCombinatorieCalcoloAnomalie();');
				// getObj('METODO_DI_CALCOLO_ANOMALIA').value = 'Metodo 4/5';
			// }
			// // Se offerta anomala è pari a Manuale allora accetta solo l'opzione vuota in metodo di calcolo
			// if (getObj('OffAnomale').value == '16311') {
				// FilterDom( 'METODO_DI_CALCOLO_ANOMALIA' ,  'METODO_DI_CALCOLO_ANOMALIA' , 'No' , 'SQL_WHERE=  DMV_Cod = \'\' ', '' , 'onChangeMetodoDiCalcolo();');
	...
	// if (getObj('TipoSoglia').value == 'sopra' && getObj('Appalto_PNRR_PNC').checked == false) {
	...
	// if (getObj('TipoSoglia').value != '' && getObj('Appalto_PNRR_PNC').checked == true) {
	
		// // Il campo sconto di riferimento lo rendo non editabile e lo svuoto
		// getObj('ScontoDiRiferimento_V').value = '';
		// ReadOnlyObj('ScontoDiRiferimento_V',true);
		
		// // Se il criterio di aggiudicazione gara è PPB o OEPV allora verifichiamo i due scenari
		// if (getObj('CriterioAggiudicazioneGara').value == '15532' || getObj('CriterioAggiudicazioneGara').value == '15531') {	
			// // Se PPB e offerta anomala diversa da Manuale allora scegliere il metodo di calcolo A
			// if (getObj('CriterioAggiudicazioneGara').value == '15531') {			
				// FilterDom( 'METODO_DI_CALCOLO_ANOMALIA' ,  'METODO_DI_CALCOLO_ANOMALIA' , 'No' , 'SQL_WHERE=  DMV_Cod = \'Metodo A\' ' , '' , 'controlliCombinatorieCalcoloAnomalie();');	
				// getObj('METODO_DI_CALCOLO_ANOMALIA').value = 'Metodo A';
			// // Altrimenti se OEPV scegliere offerta anomala di tipo Valutazione con metodo di calcolo 4/5
			// } else {
				// if (getObj('OffAnomale').value == '16310') {	
					// FilterDom( 'METODO_DI_CALCOLO_ANOMALIA' ,  'METODO_DI_CALCOLO_ANOMALIA' , 'No' , 'SQL_WHERE=  DMV_Cod = \'Metodo 4/5\' ' , '' , 'controlliCombinatorieCalcoloAnomalie();');
					// getObj('METODO_DI_CALCOLO_ANOMALIA').value = 'Metodo 4/5';
...
// function controlliCombinatorieCalcoloAnomalie(obj) {
	...
	// if (getObj('TipoSoglia').value == 'sotto' && getObj('Appalto_PNRR_PNC').checked == false) {
		...
		// if (getObj('CriterioAggiudicazioneGara').value == '15531'&& (getObj('TipoAppaltoGara').value == '3' || getObj('TipoAppaltoGara').value == '2')) {	
			// FilterDom( 'OffAnomale' ,  'OffAnomale' , '16309' , 'SQL_WHERE=  tdrCodice = \'16309\' ' , '' , 'controlliCombinatorieCalcoloAnomalie();');
			// FilterDom( 'METODO_DI_CALCOLO_ANOMALIA' ,  'METODO_DI_CALCOLO_ANOMALIA' , 'Metodo 4/5' , 'SQL_WHERE=  DMV_Cod <> \'Metodo 4/5\' ' , '' , 'onChangeMetodoDiCalcolo();');
		// }
		...
		// if (getObj('CriterioAggiudicazioneGara').value == '15531'&& getObj('TipoAppaltoGara').value == '1') {	
			// FilterDom( 'OffAnomale' ,  'OffAnomale' , '' , 'SQL_WHERE=  tdrCodice in ( \'16310\',\'16311\' )' , '' , 'onChangeMetodoDiCalcolo();');				
		// }
	...
	// if (getObj('TipoSoglia').value == 'sopra' && getObj('Appalto_PNRR_PNC').checked == false) {
		...
		// // Se il criterio di aggiudicazione è OEPV allora accetterà solo offerta anomala di tipo Valutazione o Manuale
		// if (getObj('CriterioAggiudicazioneGara').value == '15532') {
			// FilterDom( 'OffAnomale' ,  'OffAnomale' , '16311' , 'SQL_WHERE=  tdrCodice = \'16311\'', '' , 'onChangeMetodoDiCalcolo();');				
		// }
	// }
	
	// // Se siamo nel caso di una sopra/sotto soglia e in Appalto PNRR PNC allora verifichiamo 2 possibili scenari
	// if (getObj('TipoSoglia').value != '' && getObj('Appalto_PNRR_PNC').checked == true) {
		...
		// if (getObj('CriterioAggiudicazioneGara').value == '15532' || getObj('CriterioAggiudicazioneGara').value == '15531') {	
			// // Se PPB e offerta anomala diversa da Manuale allora scegliere il metodo di calcolo A
			// if (getObj('CriterioAggiudicazioneGara').value == '15531') {
				// FilterDom( 'OffAnomale' ,  'OffAnomale' , '' , 'SQL_WHERE=  tdrCodice in ( \'16309\',\'16310\' )' , '' , 'onChangeMetodoDiCalcolo();');				
			// // Altrimenti se OEPV scegliere offerta anomala di tipo Valutazione con metodo di calcolo 4/5
			// } else {
				// if (getObj('OffAnomale').value != '16310') {	
					// FilterDom( 'OffAnomale' ,  'OffAnomale' , '16310' , 'SQL_WHERE=  tdrCodice in ( \'16310\' )' , '' , 'onChangeMetodoDiCalcolo();');	
					// FilterDom( 'METODO_DI_CALCOLO_ANOMALIA' ,  'METODO_DI_CALCOLO_ANOMALIA' , 'Metodo 4/5' , 'SQL_WHERE=  DMV_Cod = \'Metodo 4/5\' ' , '' , 'onChangeMetodoDiCalcolo();');				
				// }
			// }
		// }
...
function verifyModalitaDiCalcoloAnomalia()
{
	...
	var tipoSoglia = getObj('TipoSoglia').value;
	var tipoAppaltoGara = getObj('TipoAppaltoGara').value;
	var criterioAggiudicazioneGara = getObj('CriterioAggiudicazioneGara').value;
	var calcoloAnomalia = getObj('CalcoloAnomalia').value;
...
function onChangeRichiestaSimog() {
    ...
    if (getObj('docRichiestaCig')) {
        ...
        if (docRichiestaCig == '1' && RichiestaCigSimog == 'no') {
            ...
            //se alla gara associato un pregara allora visualizzo un messaggio diverso perch?err?olo disassociata la ricehista cig dalla gara
            //e non verr?nnullata la richiesta CIG al SIMOG
...
function onChangeUserRUP() {
    ...
    // //sui campi gerarchici non è implemtnata la funziuone UpdateFieldVisual
...
function OnChangeCriterioAggiudicazioneGara() {
    ...
    if ((getObjValue('CriterioAggiudicazioneGara') == '15532') || (getObjValue('CriterioAggiudicazioneGara') == '25532')) //-- vantaggiosa or costo fisso
	{
		...
		} catch (e) { getObj( 'Conformita' ).value = 'No'; }
	...
	{		
		...
		} catch (e) { getObj( 'Conformita' ).value = ''; }
	}
...
function Handle_Attrib_MODULO_APPALTO_PNRR_PNC(nCheckValori) {
	...
	//-- Il cliente ha richiesto di nascondere sempre i campi relativi al PNRR/PNC
	Modulo_Attivo = 'no';

    //se il modulo attivo vado a nascondere i campi a seconda della spunta
    if (Modulo_Attivo == 'yes') {
        if (HideCampi == 'yes') {
...
// function CambiaTipoAppalto(thisObj) {
...
function AllineaQuestionario_PresenzaDGUE() {
    if (getObj('PresenzaQuestionario')) {
        ...
        var strWidth = $("#cap_PresenzaDGUE").parents("table:first").parents("td:first").width();
        // console.log("larghezza iniziale PresenzaDGUE = " + strWidth);

        if (!strWidth)
            strWidth = 400;
        // console.log("larghezza impostata PresenzaQuestionario = " + strWidth);
...
function OnChangeTipoAppalto(thisObj) {
    ExecDocProcess('SAVE,NUOVO_TEMPLATE_GARA,,NO_MSG');
}

function OnChangeConcessione(thisObj) {
    ExecDocProcess('SAVE,NUOVO_TEMPLATE_GARA,,NO_MSG');
}

function OnChangeProcedura(thisObj) {
	
	
	var AFFIDAMENTO_DIRETTO_DUE_FASI = getObjValue('AFFIDAMENTO_DIRETTO_DUE_FASI')

	// Se Tipo di Procedura non è "Negoziata" e se il campo/attributo Caratteristica è visibile => nascondi campo Caratteristica.
	if (getObjValue('ProceduraGara') != '15478'  ) // 15478 == Negoziata
	{
        getObj('TipoProceduraCaratteristica').value = '';
	}
    

	if (getObjValue('ProceduraGara') == '15478') // ProceduraGara==Negoziata
    {
...
function OnChangeProceduraCaratteristica(thisObj) {
	
	OnChangeProcedura(thisObj);
    //ExecDocProcess('SAVE,NUOVO_TEMPLATE_GARA,,NO_MSG');
}

function OnChangeTipoBandoGara(thisObj) {
	
	
	if ( getObjValue( 'TipoBandoGara' ) == '3' && getObjValue( 'ProceduraGara' ) == '15477' )
	{
	}
	else
	{
...
function OnChangeLotti(thisObj) {
...
// Chiamata all'interno della funzione OnLoad
function FilterFieldsBasedOnTipoSceltaContraente() {
    if (getObj('TipoSceltaContraente') && getObjValue('TipoSceltaContraente') != 'ACCORDOQUADRO') {
        FilterDom('TipoAppaltoGara', 'TipoAppaltoGara', getObjValue('TipoAppaltoGara'), 'SQL_WHERE=  DMV_COD <> \'4\'', '', 'OnChangeTipoAppalto(this)');
        ...
        FilterDom('TipoBandoGara', 'TipoBandoGara', getObjValue('TipoBandoGara'), '', '', 'OnChangeTipoBandoGara(this)'); // tutto
        //-- SelectreadOnly('TipoBandoGara', false);

        FilterDom('Divisione_lotti', 'Divisione_lotti', getObjValue('Divisione_lotti'), '', '', 'onChangeDivisione_lotti( this );'); // tutto
    }
    else {
		/* --- 
		...
		*/
        FilterDom('TipoAppaltoGara', 'TipoAppaltoGara', getObjValue('TipoAppaltoGara'), 'SQL_WHERE=  DMV_COD <> \'4\' ', '', 'OnChangeTipoAppalto(this)'); // filtra per AltraTipologia
        // TODO:
        ...
        // TODO:
        if (getObjValue('ProceduraGara') === '15478' && getObj('TipoProceduraCaratteristica').offsetParent !== null) {
            getObj('TipoProceduraCaratteristica').value = '';
            setVisibility(getObj('cap_TipoProceduraCaratteristica'), 'None');
            setVisibility(getObj('TipoProceduraCaratteristica'), 'None');
        }

        FilterDom('Divisione_lotti', 'Divisione_lotti', getObjValue('Divisione_lotti'), 'SQL_WHERE=  DMV_COD <> \'0\' ', '', 'onChangeDivisione_lotti( this );'); // filtra per AltraTipologia
    }

    if (getObjValue('TipoProceduraCaratteristica') && getObjValue('TipoProceduraCaratteristica') !== null)
        var tipoProceduraCaratteristica = getObjValue('TipoProceduraCaratteristica');
    else
        var tipoProceduraCaratteristica = "";

    FilterDom('TipoProceduraCaratteristica', 'TipoProceduraCaratteristica', tipoProceduraCaratteristica, 'SQL_WHERE= dmv_cod = \'RDO\' ', '', 'OnChangeProceduraCaratteristica(this);'); // solo Rdo
}

// Chiamata all'interno della funzione OnLoad
function FilterBasedOnDivisioneLotti() {
    // If Divisione_lotti === 'Lotti Multivoci'
    if (getObjValue('Divisione_lotti') != '' && getObjValue('Divisione_lotti') != '0' && getObjValue('Divisione_lotti') != '2') {
        //sul modello della RDO Complex non presente
        try {
...
// Chiamata all'interno della funzione OnLoad
function FilterBasedOnTipoAppaltoGara() {
    ...
    if (
        ...
        // TODO:
        // OnChangeConcessione(thisObj);
        setVisibility(getObj('cap_Concessione'), 'none');
        setVisibility(getObj('Concessione'), 'none');
    }
}

// Al cambiamento dei campi "Tipo di Procedura" (ProceduraGara) e Caratteristica (TipoProceduraCaratteristica)
function FilterBasedOnProcedura() {
    var AFFIDAMENTO_DIRETTO_DUE_FASI = getObjValue('AFFIDAMENTO_DIRETTO_DUE_FASI');
    ...
    if (getObjValue('TipoSceltaContraente') != 'ACCORDOQUADRO') {
        ...
        if (getObjValue('ProceduraGara') == '15476' || getObjValue('ProceduraGara') == '15477' || getObjValue('ProceduraGara') == '') // Aperta o Ristretta
        {
            ...
            FilterDom('TipoBandoGara', 'TipoBandoGara', '2', 'SQL_WHERE=  dmv_cod = \'2\' ', '', 'OnChangeTipoBandoGara(this)'); // solo bando

            FilterDom('Divisione_lotti', 'Divisione_lotti', getObjValue('Divisione_lotti'), '', '', 'onChangeDivisione_lotti( this );'); // come inizio

            if (getObjValue('Concessione') == 'si')
                FilterDom('CriterioAggiudicazioneGara', 'CriterioAggiudicazioneGara', getObjValue('CriterioAggiudicazioneGara'), 'SQL_WHERE=DMV_COD <> \'\' ', '', 'OnChangeCriterio_2(this);'); //-- RIMUOVE il filtro
            else
                FilterDom('CriterioAggiudicazioneGara', 'CriterioAggiudicazioneGara', getObjValue('CriterioAggiudicazioneGara'), 'SQL_WHERE=DMV_COD <> \'16291\' ', '', 'OnChangeCriterio_2(this);'); //-- filtro il prezzo più alto
        ...
        else if (getObjValue('ProceduraGara') == '15583' || getObjValue('ProceduraGara') == '15479') // Affidamento Diretto  o Richiesta Preventivo
        {
            if (AFFIDAMENTO_DIRETTO_DUE_FASI == '0' || getObjValue('ProceduraGara') == '15479') {
                ...
                FilterDom('TipoBandoGara', 'TipoBandoGara', '3', 'SQL_WHERE=  tdrcodice = \'3\' ', '', 'OnChangeTipoBandoGara(this)'); // solo invito
                //-- SelectreadOnly('TipoBandoGara', true);
            }
            else {
                //con affidamento diretto a tre fasi
                if (AFFIDAMENTO_DIRETTO_DUE_FASI == '1') {
                    FilterDom('TipoBandoGara', 'TipoBandoGara', getObjValue('TipoBandoGara'), 'SQL_WHERE=  tdrcodice in (\'3\',\'4\',\'5\') ', '', 'OnChangeTipoBandoGara(this)'); //-- solo invito e avviso/avvisi
                    //-- SelectreadOnly('TipoBandoGara', false);
                }
            }

            FilterDom('CriterioAggiudicazioneGara', 'CriterioAggiudicazioneGara', '15531', 'SQL_WHERE=DMV_COD = \'15531\' ', '', 'OnChangeCriterio_2(this);'); // SOLO Prezzo più basso
            //-- SelectreadOnly('CriterioAggiudicazioneGara', true);
        ...
        else if (getObjValue('ProceduraGara') == '15478') // ProceduraGara==Negoziata
        {
            ...
            if (getObj('TipoProceduraCaratteristica').offsetParent === null) {
                ...
                FilterDom('TipoBandoGara', 'TipoBandoGara', getObjValue('TipoBandoGara'), 'SQL_WHERE=  tdrcodice in (\'1\',\'3\') ', '', 'OnChangeTipoBandoGara(this)'); // Avviso + Invito
                FilterDom('CriterioAggiudicazioneGara', 'CriterioAggiudicazioneGara', getObjValue('CriterioAggiudicazioneGara'), 'SQL_WHERE=DMV_COD <> \'16291\' ', '', 'OnChangeCriterio_2(this);'); // filtro il prezzo più alto
				try{
            ...
            if (getObjValue('TipoProceduraCaratteristica') === 'RDO') {
                FilterDom('TipoAppaltoGara', 'TipoAppaltoGara', getObjValue('TipoAppaltoGara'), 'SQL_WHERE=  DMV_COD in (\'1\', \'3\') ', '', 'OnChangeTipoAppalto(this)');
                getObj('TipoBandoGara').value = '3'; // 3==Invito
                // getObj('TipoBandoGara').disabled = true;
                //-- SelectreadOnly('TipoBandoGara', true);
                FilterDom('Divisione_lotti', 'Divisione_lotti', getObjValue('Divisione_lotti'), 'SQL_WHERE=  DMV_Cod in (\'0\',\'2\') ', '', 'onChangeDivisione_lotti( this );'); // non "Lotti Multivoci"
            ...
            FilterDom('TipoBandoGara', 'TipoBandoGara', getObjValue('TipoBandoGara'), 'SQL_WHERE=  tdrcodice not in (\'2\',\'4\',\'5\') ', '', 'OnChangeTipoBandoGara(this)');
            FilterDom('CriterioAggiudicazioneGara', 'CriterioAggiudicazioneGara', getObjValue('CriterioAggiudicazioneGara'), 'SQL_WHERE=DMV_COD <> \'16291\' ', '', 'OnChangeCriterio_2(this);'); // filtro il prezzo più alto
            FilterDom('Divisione_lotti', 'Divisione_lotti', getObjValue('Divisione_lotti'), '', '', 'onChangeDivisione_lotti( this );'); // come inizio
...
// // Al cambiamento del campo Concessione
// function OnChangeConcessione(thisObj) {
...
function OnChangeCriterio_2(thisObj) {
	if ((getObjValue('CriterioAggiudicazioneGara') == '15532') || (getObjValue('CriterioAggiudicazioneGara') == '25532')) //-- vantaggiosa or costo fisso
	{
		try{
			FilterDom('Conformita', 'Conformita', 'No', 'SQL_WHERE=  DMV_COD = \'No\' ', '', ''); //-- solo no
...
function SET_FIELD_OnChangeTipoSceltaContraente(o) 
{
	if (getObjValue('TipoSceltaContraente') != 'ACCORDOQUADRO') 
	{
		FilterDom('TipoProceduraCaratteristica', 'TipoProceduraCaratteristica', getObjValue('TipoProceduraCaratteristica'), 'SQL_WHERE=  DMV_COD =\'RDO\' ', '', 'OnChangeProceduraCaratteristica(this);'); // filtra per AltraTipologia


		FilterDom('TipoAppaltoGara', 'TipoAppaltoGara', getObjValue('TipoAppaltoGara'), 'SQL_WHERE=  DMV_COD <> \'4\'', '', '');
    
		//-- commentato - getObj('ProceduraGara').value = '';
		// "Concorso di Idee" === 15586,   "Concorso di Progettazione" === 15587

		FilterDom('ProceduraGara', 'ProceduraGara', getObjValue('ProceduraGara'), 'SQL_WHERE=  tdrcodice not in ( \'15585\', \'15586\', \'15587\' )', '', 'OnChangeProcedura(this)', 'DT');
		//-- commentato - SelectreadOnly('ProceduraGara', false);

		//-- commentato - getObj('TipoBandoGara').value = '';
		FilterDom('TipoBandoGara', 'TipoBandoGara', getObjValue('TipoBandoGara'), '', '', 'OnChangeTipoBandoGara(this);'); // tutto
		//-- commentato - SelectreadOnly('TipoBandoGara', false);
  ...
  else 
  {
		...
		//-- commentato - if (getObj('CriterioAggiudicazioneGara').classList.contains('readonly')) // Il campo 'CriterioAggiudicazioneGara' è readonly (cioè contiene la classe che lo rende solo lettura)?
		//-- commentato - SelectreadOnly('CriterioAggiudicazioneGara', false); // Non è più in sola lettura.
    
		//-- commentato - if (getObj('CriterioFormulazioneOfferte').classList.contains('readonly')) // Il campo 'CriterioFormulazioneOfferte' è readonly (cioè contiene la classe che lo rende solo lettura)?
		//-- commentato - SelectreadOnly('CriterioFormulazioneOfferte', false); // Non è più in sola lettura.
		//-- commentato - if (getObj('Conformita').classList.contains('readonly')) // Il campo 'Conformita' è readonly (cioè contiene la classe che lo rende solo lettura)?
		//-- commentato - SelectreadOnly('Conformita', false); // Non è più in sola lettura.

		FilterDom('TipoAppaltoGara', 'TipoAppaltoGara', getObjValue('TipoAppaltoGara'), 'SQL_WHERE=  DMV_COD <> \'4\' ', '', ''); // filtra per AltraTipologia
		getObj('ProceduraGara').value = '15476'; // ProceduraGara==Aperta
		//-- commentato - SelectreadOnly('ProceduraGara', true);
		...
		//-- commentato - SelectreadOnly('TipoBandoGara', true);

		//-- commentato - getObj('TipoProceduraCaratteristica').value = '';
		//-- commentato -setVisibility(getObj('cap_TipoProceduraCaratteristica'), 'None');
		//-- commentato -setVisibility(getObj('TipoProceduraCaratteristica'), 'None');
...
function SET_FIELD_OnChangeProcedura(o) 
{

	var AFFIDAMENTO_DIRETTO_DUE_FASI = getObjValue('AFFIDAMENTO_DIRETTO_DUE_FASI');

	// Se Tipo di Procedura non è "Negoziata" e se il campo/attributo Caratteristica è visibile => nascondi campo Caratteristica.
	if (getObjValue('ProceduraGara') != '15478' ) //-- commentato -&& getObj('TipoProceduraCaratteristica').offsetParent !== null) // 15478 == Negoziata
	{
		//-- commentato -FilterDom('TipoAppaltoGara', 'TipoAppaltoGara', getObjValue('TipoAppaltoGara'), 'SQL_WHERE=  DMV_COD <> \'4\'', '', '');
  ...
  if (getObjValue('TipoSceltaContraente') != 'ACCORDOQUADRO') 
  {
    ...
    if (getObjValue('ProceduraGara') == '15476' || getObjValue('ProceduraGara') == '15477' || getObjValue('ProceduraGara') == '') // Aperta o Ristretta
    {
		...
		{
			FilterDom('CriterioAggiudicazioneGara', 'CriterioAggiudicazioneGara', getObjValue('CriterioAggiudicazioneGara'), 'SQL_WHERE=DMV_COD <> \'16291\' ', '', 'OnChangeCriterio_2(this);'); //-- filtro il prezzo pi? alto
		}

		//getObj('TipoProceduraCaratteristica').value='';
		//setVisibility(getObj('cap_TipoProceduraCaratteristica'), 'none');
		//setVisibility(getObj('TipoProceduraCaratteristica'), 'none');
		SET_FIELD_OnChangeCriterio();
    }
    else if (getObjValue('ProceduraGara') == '15583' || getObjValue('ProceduraGara') == '15479') // Affidamento Diretto  o Richiesta Preventivo
    {
		if (AFFIDAMENTO_DIRETTO_DUE_FASI == '0' || getObjValue('ProceduraGara') == '15479') {
		...
		else 
		{
			//con affidamento diretto a tre fasi
			if (AFFIDAMENTO_DIRETTO_DUE_FASI == '1') {
				FilterDom('TipoBandoGara', 'TipoBandoGara', getObjValue('TipoBandoGara'), 'SQL_WHERE=  tdrcodice in (\'3\',\'4\',\'5\') ', '', 'OnChangeTipoBandoGara(this);'); //-- solo invito
				//-- commentato -SelectreadOnly('TipoBandoGara', false);
			}
		}

		FilterDom('CriterioAggiudicazioneGara', 'CriterioAggiudicazioneGara', '15531', 'SQL_WHERE=DMV_COD = \'15531\' ', '', 'OnChangeCriterio_2(this);'); // SOLO Prezzo più basso
		//-- commentato -SelectreadOnly('CriterioAggiudicazioneGara', true);

		try{
			FilterDom('Conformita', 'Conformita', 'No', 'SQL_WHERE=  DMV_COD = \'No\' ', '', ''); //-- solo no
			//-- commentato -SelectreadOnly('Conformita', true);
		}catch(e) 
		{
			getObj( 'Conformita' ).value = 'No'
		}
    ...
    else if (getObjValue('ProceduraGara') == '15478') // ProceduraGara==Negoziata
    {
      // Se il campo/attributo Caratteristica non è visibile => rendi visibile campo Caratteristica.
        ...
        //-- commentato -getObj('TipoBandoGara').value = '3'; // 3==Invito
        // getObj('TipoBandoGara').disabled = true;
        //-- commentato -SelectreadOnly('TipoBandoGara', true);
...
function SET_FIELD_OnChangeLotti(o) {
  // If Divisione_lotti === 'Lotti Multivoci'
  if (getObjValue('Divisione_lotti') != '' && getObjValue('Divisione_lotti') != '0' && getObjValue('Divisione_lotti') != '2') {
    //sul modello della RDO Complex non presente
    try {
  ...
  //-- commentato -HideConformita();
}


function SET_FIELD_OnChangeCriterio(o) {
	if ((getObjValue('CriterioAggiudicazioneGara') == '15532') || (getObjValue('CriterioAggiudicazioneGara') == '25532')) //-- vantaggiosa or costo fisso
	{
