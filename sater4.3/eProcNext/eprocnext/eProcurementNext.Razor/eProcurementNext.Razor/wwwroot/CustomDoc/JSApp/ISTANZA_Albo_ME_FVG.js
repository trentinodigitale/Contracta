...
function DISPLAY_FIRMA_OnLoad() {
	...
	if (getObjValue('PresenzaDGUE') == 'si') {
	   ...
	   {
	   document.getElementById('CompilaDGUE').disabled = true; 
	   document.getElementById('CompilaDGUE').className ="CompilaDGUEdisabled";
	   }*/
	}
...
function controlli(param) {
	if (getObj('DOCUMENT_READONLY').value != '1') {
		...
		//controllo per il punto c) devono essere compilati "n."  e  "di" oppure " o altro" 
		//alert(getObj('numero_iscrizione'));
		if (getObj('numero_iscrizione') !== undefined) {
			TxtOK('numero_iscrizione');
			TxtOK('sede_iscrizione');
			TxtOK('altro_iscrizione');

			ValueNumIscriz = trim(getObj('numero_iscrizione').value);
			ValueSedeIscriz = trim(getObj('sede_iscrizione').value);
			if (ValueNumIscriz != '' || ValueSedeIscriz != '') {
				if (ValueNumIscriz == '') { err = 1; TxtErr('numero_iscrizione'); }
				if (ValueSedeIscriz == '') { err = 1; TxtErr('sede_iscrizione'); }
			}
			else {
				//controllo che valorizzato altro_iscrizione
				if (trim(getObj('altro_iscrizione').value) == '') { err = 1; TxtErr('altro_iscrizione'); }
			}
		}


		//se presente campo "altro_SettoreCCNL" verifico se il campo "SettoriCCNL" contiene ###XX### (Contratti diversi o nessun contratto)
		if (getObj('altro_SettoreCCNL') !== undefined) {
			TxtOK('altro_SettoreCCNL');
			strValueSettore = trim(getObj('SettoriCCNL').value);

			if (strValueSettore.indexOf('###XX###') > -1) {
				if (trim(getObj('altro_SettoreCCNL').value) == '') {
					err = 1;
					TxtErr('altro_SettoreCCNL');
				}
			}
...
function OnChange_SettoreCCNL(objSettore) {
	strValueSettore = trim(getObj('SettoriCCNL').value);

	if (strValueSettore.indexOf('###XX###') < 0) {
		getObj('altro_SettoreCCNL').value = '';
...
function OnChangeLocalita(strNameLocalita) {
	//LocalitaRapLeg    RapLeg
	//ResidenzaRapLeg   ResidenzaRapLeg
	//LOCALITALEG		non gestito
	var fieldName = strNameLocalita
	if (strNameLocalita == 'LocalitaRapLeg')
		fieldName = "RapLeg";

	//alert(strNameLocalita);

	strValue = trim(getObj(strNameLocalita).value);

	//alert(strValue);

	//verifico se esiste una sola corrispondenza
	...
	if (ajax) {
		...
		ajax.open("GET", '../../ctl_library/functions/infoNodoGeo.asp?fldname=descrizionelocalita&descrizione=' + escape(strValue), false);
		//output nella forma : 1###COD_COMUNE
		ajax.send(null);
			...
			if (ajax.status == 200) {
				...
				if (ajax.responseText != '') {
					...
					if (res.substring(0, 2) == '1#') {
						...
						impostaLocalita(codLoc, fieldName);
