	...
	try
	{
		...
		if(  getObjValue( 'StatoFunzionale' ) == 'InLavorazione'  )
		{
			...
			if ( filtro != '')
			{
				...
				SetProperty( getObj('ClasseIscriz_Aggiunte'),'filter','SQL_WHERE=DMV_COD not in ( select * from dbo.split(\'' + filtro + '\',\'###\'))');
...
{
		var classi_sel=getObjValue('ClasseIscriz_Aggiunte');
        ...
        if (ajax) 
		{
            ajax.open("GET", '../../customdoc/CONTROLLO_CLASSE_ALTRI_BANDI.asp?IDDOC=' + getObj('LinkedDoc').value  + '&classi_sel=' + encodeURIComponent(classi_sel) + '&nocache=' + nocache  , false);
