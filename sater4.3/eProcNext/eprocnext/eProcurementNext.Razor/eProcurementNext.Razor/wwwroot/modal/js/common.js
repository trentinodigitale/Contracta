  ...
  links.addEvent('click',function(ev){           
   ...
   Modal.show(target);
	...
	show:function(lnk){
		...
		$('overlay').setStyle('height',Window.getSize().scrollSize.y+'px');
		$('mb_box').addClass(Modal.cssClass).setStyle('height',(Modal.height-35)+'px');
		...
		Modal.fx.start({
			...
			'left':[coord.left,(Window.getWidth()/2)],
			'top':[coord.top,((Window.getHeight()/2)+Window.getSize().scroll.y)],
			'opacity':[0,1]
		});
	...
	hide: function(){
		Modal.fx.start({
			...
			'left':[(Window.getWidth()/2),Modal.startPosition.x],
			'top':[((Window.getHeight()/2)+Window.getSize().scroll.y),Modal.startPosition.y],
			'opacity':[1,0]
		});
...
window.addEvent('domready',function(){
	...
	accordion = new Accordion('h3.atStart', 'div.atStart', {
		...
		onActive: function(toggler, element)
		{
			...
			toggler.setAttribute("class","toggler atStart tabSelected");
			element.setAttribute("class","element atStart elemSelected");
		},
		onBackground: function(toggler, element){
			toggler.setStyle('color', '');
			toggler.setAttribute("class","toggler atStart");
			element.setAttribute("class","element atStart");
		}
	}, $('accordion'));	
	...
	var registerform = $('FormCens');
		...
		*/
		registerform.addEvent('submit',function(ev){
			...
			var rb =$('privacyagree');
			if(!rb.checked){
				rb.getParent().setStyles({fontWeight:'bold',color:'#BF0B27'});
				ev.preventDefault();
