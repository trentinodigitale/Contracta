...
//estende il controllo del CF anche alle CUC
function ControllaCF_PG_Extended( obj )
{
	var r = ControllaCF_PG_sub_CUC(obj.value)
	if ( r != '')
	{
		AF_Alert( r );
		obj.value = '';
...
function ControllaCF_PG_sub_CUC( pi )
{
    if( pi == '' )  return '';
	
	//controllo su CUC con CF fittizio di 14 cratteri che comincia con "CFAVCP-" e un alfanumerio a seguire
	if (pi.length === 14 )
	{
		check = "CFAVCP-";
		if( pi.indexOf( check ) != 0 )
				return "IL Codice Fiscale non inizia con \'CFAVCP-\'.";
		validi = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";
		var to_check= pi.substring(7);
		for( i = 0; i < 11; i++ )
		{
			if( validi.indexOf( to_check.charAt(i) ) == -1 )
				return "IL Codice Fiscale contiene un carattere non valido. I caratteri validi sono gli alfanumerici.";
		}
		return '';
	}
	//controllo su cf
	...
	{
		return "La lunghezza del Codice Fiscale non e\' corretto: la sua lunghezza dovrebbe essere o 14 o 11 caratteri.";
	}
}
...
function ValidaNumPercentuale ( obj )
{
	var controllo=obj.value;   

	// un numero compreso tra 0 e 100
	if ( controllo.search('^([0-9]|([1-9][0-9])|100)$') < 0 ) 
	{
		AF_Alert( 'Errore_Numeri_percentuale' );
		setTimeout(function() { document.getElementById(obj.id).focus(); }, 10);
		obj.value='';
	}
...
function Validate_Field_URL ( obj ) {
  ...
  var ValoreField=obj.value;  

  // Se l'url Ã?Â¨ diverso da vuoto si va a testare rispetto all'espressione regolare
  if (ValoreField != '' && !urlPattern.test(ValoreField)) 
  {
    obj.value = ''; //svuoto il campo considerato non valido
    //DMessageBox('../', 'URL non valido. Non rispetta il formato richiesto', 'Attenzione', 1, 400, 300);
	AF_Alert('URL non valido. Non rispetta il formato richiesto');
