...
namespace eProcurementNext.RegistroImprese
{
	...
	public class ClasseAdrierClient : IParixClient
	{
                        ...
                        foreach (XmlNode nodoN in listaNodi)
						{
                            ...
                            foreach (XmlNode nodoX in m_nodelist)
							{
								...
								if (ParixClient.Utils.getXPathValue(nodoX, "FLAG_SEDE") == "SI")
								{
                                    ...
                                    if (String.IsNullOrEmpty(cciaaAnag) || (String.IsNullOrEmpty(cciaaAnag) == false && nodoX.SelectSingleNode("CCIAA").InnerText.ToUpper() == cciaaAnag.ToUpper()))
                                    ...
                                    if (String.IsNullOrEmpty(cciaaAnag) == false && nodoX.SelectSingleNode("CCIAA").InnerText.ToUpper() != cciaaAnag.ToUpper()) { 
                                        continue;
                                    }else{
                                        break;
                                    }
		...
		private void popolaDatiMancantiDaIDAZI()
		{
			...
			var db = new ParixClient.DB_old(connectionString);
			var db2 = new ParixClient.DB_old(connectionString);
