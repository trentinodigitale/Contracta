...
namespace eProcurementNext.Razor.ViewComponents
{
    public class BreadcrumbViewComponent : ViewComponent
    {
        ...
        private System.Text.StringBuilder GetHtmlCode(dynamic pathRoot, string firstElement, string durataSessione)
        {
            ...
            _out.Write(@"<div class=""me-5 span_footer_datetime containerInfoSessionServerHours"">");
            _out.Write(@"<div style=""text-align: end;"">");
            _out.Write($@"<span class=""me-2 fw-bold"">{ApplicationCommon.CNV("Ora Server", _session)}:</span><span id=""DateTime""></span>");

            _out.Write(@"</div>");

            _out.Write(@"<div style=""text-align: end;"">");

            _out.Write($@"<span class=""ms-3 me-2 fw-bold"">{ApplicationCommon.CNV("Tempo stimato di sessione rimanente", _session)}:</span><span id=""tempo_di_sessione"">{CStr(durataSessione)}:00 m</span>");
            _out.Write(@"</div>");
