...
namespace eProcurementNext.Razor.Pages.CTL_LIBRARY
{
    public class xlxsModel
    {
        ...
        public void Page_Load(HttpContext HttpContext, EprocResponse htmlToReturn, eProcurementNext.Session.ISession _session)
        {
            ...
            GENERAFOGLIODOMINI = CStr(GetParamURL(strQueryString, "GENERAFOGLIODOMINI")).ToUpper();
            ...
            bool bAutoCol;
			//'recupero questo parametro che indica se il viewer usa una griglia con tutte le colonne del record set o meno
			if (Strings.UCase(CStr(GetParamURL(strQueryString, "AUTO_COL"))) == "YES")
			{
				bAutoCol = true;
			}
			else
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(MODEL.Trim()))
                {
                    ...
                    if (rsColonne is not null && rsColonne.RecordCount > 0)
                    {
                        ...
                        {
                            wsDomini = wb.Worksheets.Add("Domini");
                        ...
                        do
                        {
                            ...
                            if ((dztType == 4 || dztType == 5 || dztType == 8) && GENERAFOGLIODOMINI.ToUpper() == "YES")
                            {
                                ...
                                ValidationRow(wsDomini, indCol, dtz_name, strVisualValue, ref numeroRigheProcessate);

                                //'mi creo la validazione per le celle
                                string cellStartDomini = CStr(ws.Cell(1, indCol));
                                string lettera = cellStartDomini.Replace("1", "");
                                string rangeValue = $"=Domini!${lettera}$2:${lettera}${numeroRigheProcessate}";


                                string cell = CStr(ws.Cell(1, indCol));
                                string letteraWs = cell.Replace("1", "");

                                string rangeValidationWs = $"{letteraWs}2:{letteraWs}1048576";

                                IXLRange range = ws.Range(rangeValidationWs);
                                //IXLDataValidation v = ws.CreateDataValidation();

                                ////v.ClearRanges();
                                //v.AddRange(range);
                                //v.ShowErrorMessage = true;
                                //v.ErrorStyle = XLErrorStyle.Warning;
                                //v.ErrorTitle = "An invalid value was entered";
                                //v.ErrorMessage = "Selezionare il valore dalla lista";
                                //v.IgnoreBlanks = false;
                                ////v.InCellDropdown = true;
                                ////v.Value = rangeValue;
                                //v.AllowedValues = XLAllowedValues.List;
                                ////v.List(rangeValidationWs, true); 
                                ////ws.DataValidations.Add(v);
                                ///

                                //ws.Column(indCol).SetDataValidation().List(range, true);
                                //ws.Column(indCol).SetDataValidation().ShowErrorMessage = true;
                                //ws.Column(indCol).SetDataValidation().ErrorMessage = "Selezionare il valore dalla lista";
                                //ws.Column(indCol).SetDataValidation().ErrorTitle = "An invalid value was entered";
                                //ws.Column(indCol).SetDataValidation().IgnoreBlanks = false;
                                //ws.Column(indCol).SetDataValidation().AllowedValues = XLAllowedValues.List;
                                //ws.Column(indCol).SetDataValidation().Value = rangeValue;

                                var dv1 = ws.Range(rangeValidationWs).CreateDataValidation();
                                dv1.List(range, true);
                                dv1.ShowErrorMessage = true;
                                dv1.ErrorMessage = "Selezionare il valore dalla lista";
                                dv1.ErrorTitle = "An invalid value was entered";
                                dv1.IgnoreBlanks = false;
                                dv1.AllowedValues = XLAllowedValues.List;
                                dv1.Value = rangeValue;
                            ...
                            for (m = 1; m <= resSplit.Length - 1; m++)
                            {
                                ...
                                if (!IsDbNull(rsDati[resSplit[m]]))
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        //-- KPF per i numeri non faccio nulla perchè coerenti con il tipo colonna ritornato
                                        if (resSplitType[m] == "2")
										{
                                            ws.Cell(indRow, indCol).Value = rsDati[resSplit[m]];
										}
                                        else
                                        //rimango i tipi stringa
										{
											if (!resSplitFormat[m].ToUpper().Contains("H"))
                                            {
                                                ws.Cell(indRow, indCol).Value = TruncateMessage(CStr(rsDati[resSplit[m]]), 32764);  //32767 è il limite di caratteri supportati da una cella di Excel
