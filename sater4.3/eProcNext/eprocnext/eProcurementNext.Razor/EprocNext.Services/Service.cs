...
namespace eProcurementNext.Services
{
    ...
    {
		private DateTime DataUltimaPuliziaServices; 
        ...
        public void ExecuteThread_sub(dynamic[,] m, int j, string connectionString, int threadIndex)
        {
			...
			if (DateDiff("s", DataUltimaPuliziaServices, DateTime.Now) > CInt(tempoGarbageCollector))
			{
				parCollection.Clear();
				...
				DataUltimaPuliziaServices = DateTime.Now;
			}
		}
