...
{
    public abstract class AbsNoSqlAction<TIn, TOut, TContext> : INoSqlHashValidator<TContext>, INoSqlAction<TOut,TIn>, INoSqlActionAsync<TOut, TIn> where TContext : class, INoSqlCollection
    {
        ...
        {
            if (param is INoSqlTenantSecurityCollection securityParam && !(Session is null))
            {
                string messageToSign = $"{param._id}{Session.TenantId}{Session.AccountId}{Session.SecretHashKey}";
        ...
        public void CalculateHash(TContext obj)
        {
            ...
            {
                string messageToSign = $"{obj._id}{Session?.TenantId}{Session?.AccountId}{Session?.SecretHashKey}";
