...
namespace eProcurementNext.DashBoard
{
    public static class Basic
    {
            ...
            if (string.IsNullOrEmpty(strRegExp))
            {
                ...
                switch (tipo)
                {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        _isValid = isValidaSqlFilter(strValue, strConnectionString);
                        return _isValid;
                    case 4:
        ...
        public static bool isValidaSqlFilter(string strFilter, string strConnectionString = null)
        {
            ...
            if (strFilter != "")
            {
                ...
                if (strConnectionString != null && strConnectionString.Length > 0)
                {
                    ...
                    try
                    {
                        strSql = $@"select value from CTL_CHECK_SECURITY with(nolock) where tipo = 'KEYS_TO_BLOCK_IN_FILTER' and '{strTempFilter.Replace("'", "''")}' like '% ' + value + ' %'";
                ...
                if (!bTabella)
                {
                    ...
                    if (strTempFilter.Contains(" UPDATE ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" DELETE ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" DROP ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" ALTER ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" UNION ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" EXEC ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" EXECUTE ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" CREATE ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" TRUNCATE ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" KILL ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" INTERSECT ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" AUTHORIZATION ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" SHUTDOWN ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" SCHEMA ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" DENY ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" GO ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" GOTO ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" WAIT ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" WAITFOR ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" DBCC ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" OPENDATASOURCE ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" OPENQUERY ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" OPENROWSET ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" OPENXML ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" RESTORE ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" WHILE ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" SYSOBJECTS ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" SYSDATABASES ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" SYS.OBJECTS ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" SYS.DATABASES ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" SYSCOLUMNS ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" SYS.COLUMNS ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" INFORMATION_SCHEMA ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" SYSUSERS ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" DECLARE ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" DB_NAME ", StringComparison.Ordinal)) test = false;
                    if (strTempFilter.Contains(" DB_ID ", StringComparison.Ordinal)) test = false;
                }
                else
                {
