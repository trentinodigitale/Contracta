...
//getObj('RTIPOLOGIA_INCARICOGrid_' + 1 + '_SelRow' ).checked = false;
function controllo_attivitaSelRow(obj)
{
	//alert(obj.id);
	if( getObj(obj.id).checked == true && (Get_CTL_PARAMETRI('ISTANZA_AlboProf_TIPOLOGIA_INCARICO','TIPOLOGIA_INCARICO','Selezione_Fasce','Univoca',-1) != 'Entrambi') )
    {
       getObj(obj.id+'1').checked = false;
	  
    }
	//CONTROLLO CHE NON SIANO CHECKED OLTRE 5 attivita
	var NRTIPOLOGIA_INCARICOGrid = GetProperty( getObj('TIPOLOGIA_INCARICOGrid') , 'numrow');
	var cont=0;
	for( i = 0 ; i <= NRTIPOLOGIA_INCARICOGrid ; i++ )
	{
			try
			{
				if( getObj( 'RTIPOLOGIA_INCARICOGrid_' + i + '_SelRow' ).checked == true  || getObj( 'RTIPOLOGIA_INCARICOGrid_' + i + '_SelRow1' ).checked == true  )
				{
					cont=cont+1;
				}
		   }catch(e)	  {	  }
	...
	if ( cont > 5)
	{
		 DMessageBox( '../' , 'Contrassegnare un minimo di 1 sino ad un massimo di 5 attivita' , 'Attenzione' , 1 , 400 , 300 );
		 return -1;
	}
...
function controllo_attivitaSelRow1(obj)
{
	
	if( getObj(obj.id).checked == true && (Get_CTL_PARAMETRI('ISTANZA_AlboProf_TIPOLOGIA_INCARICO','TIPOLOGIA_INCARICO','Selezione_Fasce','Univoca',-1) != 'Entrambi') )
    {
       getObj(obj.id.substring(0,obj.id.length-1)).checked = false;
...
function controllo_attivita_send(grid , r , c)
{
    ...
    if(  Number( NRTIPOLOGIA_INCARICOGrid ) >= 0 )
    {
	...
	if ( Number(NRFATTURATO_Grid)+1 != Number(cont) )
	{
		DMessageBox( '../' , 'Per ogni attivita deve essere inserito il Fatturato al punto J' , 'Attenzione' , 1 , 400 , 300 );
		 return -1;
	}
...
function set_tipologia_incarico_grid()
{
    ...
    if(  Number( NRTIPOLOGIA_INCARICOGrid ) >= 0 )
    {
		 ...
		 for( i = 0 ; i <= NRTIPOLOGIA_INCARICOGrid ; i++ )
		 {
				try
				{
					if( getObjValue( 'val_RTIPOLOGIA_INCARICOGrid_' + i + '_Folder_Node' ) == '1'  )
					{
						getObj('RTIPOLOGIA_INCARICOGrid_'+ i +'_SelRow').style.display='none';
						getObj('RTIPOLOGIA_INCARICOGrid_'+ i +'_SelRow1').style.display='none';
					}
			   }catch(e)	  {	  }
		}
    }
