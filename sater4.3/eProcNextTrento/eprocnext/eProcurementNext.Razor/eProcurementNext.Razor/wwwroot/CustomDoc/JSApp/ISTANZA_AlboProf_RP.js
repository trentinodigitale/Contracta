...
var LstAttrib = [
  ...
  //'EmailRapLeg',
  'CittaEntrate',
  'data_abilitazione_professione',
  'OrdineProfessionale',
  'ProvinciaOrdProfessionale',
  'NumeroOrdProfessionale',
  'TitoloProfessionale',
  'AnnoOrdProfessionale'
];
...
function controllo_attivita(grid, r, c) {
  ...
  if (Number(NRTIPOLOGIA_INCARICOGrid) >= 0) {
    ...
    for (i = 0; i <= NRTIPOLOGIA_INCARICOGrid; i++) {
      try {
        if (getObj('RTIPOLOGIA_INCARICOGrid_' + i + '_SelRow').checked == true) {
          cont = cont + 1;
        }
      } catch (e) { }
    }
  ...
  if (cont == 0) {
    DMessageBox('../', 'Contrassegnare almeno una casella nella scelta delle attivita professionali', 'Attenzione', 1, 400, 300);
    return -1;
  }
