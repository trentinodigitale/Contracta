...
function InvioOfferta(param) 
{
    ...
    //alert(param);
	//la chiamata a ControlliOfferta() commentata perchè la coerenza della RTI viene controllata prima di firmare ogni busta
...
function HideCestinoProdotti() 
{
    ...
    try 
	{
		...
		//per i complex lascio le funzionalità sul cestino solo se non è in corso la firma del lotto
		else
		{
			 for (i = 0; i < PRODOTTIGrid_EndRow + 1; i++) 
			{
...
function DisplaySection(obj) 
{
    ...
    if (getObj('CriterioAggiudicazioneGara')) 
	{
		...
		try{
            //-- le gare al prezzo più alto o concessione non devono mostrare il ribasso
        ...
        if (Divisione_lotti != '0') 
		{
			...
			*/
			...
			//-- SE PER TUTTA LA COLONNA NON è NECESSARIA LA BUSTA TECNICA RIMUOVO LA COLONNA
		...
		/*
		//Enrico - questa gestione non serve perchè presente nella stored ed anche più completa
		else {
			
			//SE GARA NON A LOTTI
			
			//questa gestione non serve perchè presente nella stored ed anche più completa
			try{
        ...
        try {
            if (getObjValue('ProceduraGara') == '15477' && getObjValue('TipoBandoGara') == '2') {
				try{
					...
					//DocDisplayFolder('BUSTA_ECONOMICA', 'none');
					//DocDisplayFolder('LISTA_LOTTI', 'none');
					//questo non serve perchè apro la domanda per una ristretta-bando e mai una offerta
		...
		*/
		...
		//innescate le funzioni per gestire i bottoni di genera pdf perchè la variabile DOCUMENT_READONLY non è ancora disponibile al caricamento delle varie sezioni 
  ...
  if (getObjValue('PresenzaQuestionario') === 'si') {
	...
	//e quando lo stesso è pronto per invio
...
function OpenEconomica(objGrid, Row, c) 
{
	...
	*/
	...
	{
		//Se la griglia dei prodotti è stata modificata senza salvare
...
function UploadBusteFirmate() 
{
	...
	if ( ControlloFirmaBuste )
	{
		//Se richiesta la verifica solo per la bontà hash
		if (ControlloFirmaBuste.value == 'no')
		{
	 ...
	 try
	 {
		...
		else
		{
			...
			else
			{
				...
				{
					//CORREZIONE: prima il warning sui prodotti non usciva mai perchè se c'è solo il warning, finiremmo comunque l'if dell'OK che sovrascrive la precedente icona di warning.
			...
			try 
			{
				...
				else
				{
					...
					{	
						...
						{
							if ( ControlloFirmaBuste_RESULT == 1 && $("#F3_SIGN_ATTACH_V tr:first-child").html().indexOf('sign_not_ok.png') > 0  ) //Se non è richiesta la firma buste e non ho messo un file firmato
							{
					...
					try
					{
						...
						{
							if ( ControlloFirmaBuste_RESULT == 1 && $("#F1_SIGN_ATTACH_V tr:first-child").html().indexOf('sign_not_ok.png') > 0  ) //Se non è richiesta la firma buste e non ho messo un file firmato
							{
...
function My_Set_Change_Document() 
{
	 ...
	 {	 
		...
		//ed ildocumento è pronto per invio cambio key ml
		if ( objClickInvio != undefined )
