...
function DisplaySection(obj) 
{
	...
	if ( bVisualMessageDataSuperata == 1) {
			...
			AF_Alert('i termini di presentazione della Manifestazione di interesse sono superati');
...
function afterProcess( param )
{
	...
	if ( param == 'SEND' )
	{
		...
		if( getObjValue( "StatoFunzionale" ) == 'Inviato' )
		{
			...
			var ML_text = 'Invio Manifestazione di Interesse eseguito correttamente';
			
			//nel caso di documento di risposta per l'affidamento a due fasi cambio frase
			var StrTipoBandoGara = getObj('TipoBandoGara').value;
			if ( StrTipoBandoGara == '4' || StrTipoBandoGara == '5'  )
			{	
				ML_text = 'Risposta avviso eseguito correttamente';
			}
