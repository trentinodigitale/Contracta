...
function DISPLAY_FIRMA_OnLoad()
{
	...
	if ( (Stato=='Saved' || Stato=="") && IdpfuInCharge == idpfuUtenteCollegato  && getObjValue('SIGN_LOCK') =='0')
		{
	...
	if ( getObjValue('SIGN_ATTACH') != '' && Stato != 'Sended' && statoFunzionale != 'Variato' && getQSParam('COMMAND') != 'PROCESS' )
	{
		...
		//DMessageBox( '../' , 'Per effettuare l\'invio dell\'istanza cliccare sul comando "Invia" in alto sul documento' , 'Attenzione' , 1 , 400 , 300 );
		return;
	}
...
if (getObj('DOCUMENT_READONLY').value != '1' )
{
	var err = 0;
		...
		if (getObjValue('F2_SIGN_ATTACH') == ""  )
		{
			err = 1;
			TxtErr( 'Cell_F2_SIGN_ATTACH' );
		}
		else
		 {
					 
					 TxtOK( 'Cell_F2_SIGN_ATTACH' );
		 }
		...
		if(  Number( numeroRighe ) < 0 )
		{
			 ...
			 alert('Avvalorare almeno una riga  nella sezione di dettaglio CESSATI');
		...
		if(  Number( NRPOSIZIONI_INAILGrid ) >= 0 )
		{
		 ...
		 for( i = 0 ; i <= NRPOSIZIONI_INAILGrid ; i++ )
		 {
			try
			{
			   ...
			   /*
				if( getObjValue( 'RPOSIZIONI_INAILGrid_' + i + '_IndirizzoINAIL' ) == '' )
				{
					 err = 1;
					 TxtErr( 'RPOSIZIONI_INAILGrid_' + i + '_IndirizzoINAIL' );
				}
				else
				 {
					 
					 TxtOK( 'RPOSIZIONI_INAILGrid_' + i + '_IndirizzoINAIL' );
				}
			   
			   
			   
				if( getObjValue( 'RPOSIZIONI_INAILGrid_' + i + '_TelefonoINAIL' ) == '' )
				{
					 err = 1;
					 TxtErr( 'RPOSIZIONI_INAILGrid_' + i + '_TelefonoINAIL' );
				}
				else
				 {
					 
					 TxtOK( 'RPOSIZIONI_INAILGrid_' + i + '_TelefonoINAIL' );
				}
			   
			   
			   
				if( getObjValue( 'RPOSIZIONI_INAILGrid_' + i + '_FaxINAIL' ) == '' )
				{
					 err = 1;
					 TxtErr( 'RPOSIZIONI_INAILGrid_' + i + '_FaxINAIL' );
				}
				else
				 {
					 
					 TxtOK( 'RPOSIZIONI_INAILGrid_' + i + '_FaxINAIL' );
				}   
			   */
		   }catch(e)	  {	  }
		}
		...
		if(  Number( NRPOSIZIONI_CASSAEDILEGrid ) >= 0 )
		{
		 ...
		 for( i = 0 ; i <= NRPOSIZIONI_CASSAEDILEGrid ; i++ )
		 {
			try
			{
			   ...
			   /* if( getObjValue( 'RPOSIZIONI_CASSAEDILEGrid_' + i + '_IndirizzoEdile' ) == '' )
				{
					 err = 1;
					 TxtErr( 'RPOSIZIONI_CASSAEDILEGrid_' + i + '_IndirizzoEdile' );
				}
				else
				 {
					 
					 TxtOK( 'RPOSIZIONI_CASSAEDILEGrid_' + i + '_IndirizzoEdile' );
				}
			   
			   
			   
				if( getObjValue( 'RPOSIZIONI_CASSAEDILEGrid_' + i + '_TelefonoEdile' ) == '' )
				{
					 err = 1;
					 TxtErr( 'RPOSIZIONI_CASSAEDILEGrid_' + i + '_TelefonoEdile' );
				}
				else
				 {
					 
					 TxtOK( 'RPOSIZIONI_CASSAEDILEGrid_' + i + '_TelefonoEdile' );
				}
			   
			   
			   
				if( getObjValue( 'RPOSIZIONI_CASSAEDILEGrid_' + i + '_FaxEdile' ) == '' )
				{
					 err = 1;
					 TxtErr( 'RPOSIZIONI_CASSAEDILEGrid_' + i + '_FaxEdile' );
				}
				else
				  {
					 
					  TxtOK( 'RPOSIZIONI_CASSAEDILEGrid_' + i + '_FaxEdile' );
				}   
			   */
		   }catch(e)	  {	  }
		}
		...
		if ( getObj( 'CheckReati1' ).checked == false &&  getObj( 'CheckReati2' ).checked == false && getObj( 'Checkcessati2' ).checked == true )
			{
			 err = 1;
			  TxtErr( 'CheckReati1' );
			  TxtErr( 'CheckReati2' );
		...
		if ( getObj( 'CheckObblighi1' ).checked == false &&  getObj( 'CheckObblighi2' ).checked == false )
		  ...
		  */
			...
			if ( getObj( 'Checkcessati1' ).checked == true &&  getObj( 'Checkcessati2' ).checked == true )
			{
			 err = 1;
			  TxtErr( 'Checkcessati1' );
			  TxtErr( 'Checkcessati2' );
			...
			if ( getObjValue('Numerodipendenti') != "0"  && getObjValue('SettoriCCNL') == "")
			{
				err = 1;
				TxtErr( 'SettoriCCNL' );
			}
...
function allegaPattoDiIntegrita()
{
	var idDoc;
	idDoc = getObjValue('IDDOC');
	
	ExecFunctionCenterDoc('../functions/field/uploadattachsigned.asp?TABLE=CTL_DOC_SIGN&IDDOC=' + idDoc + '&OPERATION=INSERTSIGN&NO_REFRESH_PARENT=YES&IDENTITY=IdHeader&AREA=F2&DOMAIN=FileExtention&FORMAT=#AllegaFirma#600,400');
    //MySaveDoc();
