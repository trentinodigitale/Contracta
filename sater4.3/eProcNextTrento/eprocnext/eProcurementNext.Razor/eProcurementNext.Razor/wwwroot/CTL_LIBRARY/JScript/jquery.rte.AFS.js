...
function setVisibility(target, s_value_property){	
	var node = target;
	eval('node.style.display = \'' + s_value_property + "'" );
}
...
jQuery.fn.rte = function(css_url, media_url) {
    ...
    function enableDesignMode(textarea) {
        ...
        iframe.style['height']= textarea.css('height');  // -- calcolare ALTEZZA
        
        if(textarea.attr('class'))
            iframe.className = 'FRM_' + textarea.attr('class');   //-- aggiunto un discriminante
        if(textarea.attr('id'))
            iframe.id = 'FRM_' + textarea.attr('id') ;
        if(textarea.attr('name'))
            iframe.title = 'FRM_' + textarea.attr('name');
        ...
        tryEnableDesignMode(iframe, doc, function() {
            ...
            //textarea.remove();    //-- nasconde il controllo
            //setVisibility( textarea , 'none' );
    ...
    function toolbar(iframe) {   
	    ...
	    try
		{
			//if(getObj('TipoDoc').value=='MAIL_TEMPLATE') 
			if(getObj('TYPEDOC').value=='MAIL_TEMPLATE')
			{
			link="<a href='#' class='link'><img src='"+media_url+"link.gif' title='link' alt='link' /></a>"		
			}
		}
		catch (e) {}
        ...
        var tb = $("<div class='rte-toolbar' id='toolbar-"+iframe.title+"'><div>\
            ...
            <!--/p>\
            <p-->\
                <a href='#' class='bold'><img src='"+media_url+"bold.gif' title='bold' alt='bold' /></a>\
                <a href='#' class='italic'><img src='"+media_url+"italic.gif' title='italic' alt='italic' /></a>\
                <a href='#' class='underline'><img src='"+media_url+"underline.gif' title='underline' alt='underline' /></a>\
            <!--/p>\
            <p-->\
                <a href='#' class='unorderedlist'><img src='"+media_url+"unordered.gif' title='unordered list' alt='unordered list' /></a>\
                <a href='#' class='insertorderedlist'><img src='"+media_url+"insertorderedlist.gif' title='ordered list' alt='ordered list' /></a>\
                <a href='#' class='image'></a>"+ link +"\
        ...
        $('.link', tb).click(function() {
			...
			DOC_NewDocumentFrom( 'ADD_LINK_MAIL_TEMPLATE#MAIL_TEMPLATE,' + getObj('PrevDoc').value + '#600,300##&UpdateParent=no' );
		});
...
function CloseRTE()
{
        ...
        $('.FRM_RTE').each( function(){
			
            try{ RTE_Close(this , false); }catch(e){};
...
function RTE_Close(iframe, submit) {	
		  
    var content = iframe.contentWindow.document.getElementsByTagName("body")[0].innerHTML;
	
    var id = iframe.id.substring(4) ;	
    var textareaOld = getObj( id );
	...
	{
		textareaOld.innerHTML = t.innerHTML;
