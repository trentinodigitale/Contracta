...
var openDrawer = (
    ...
    if (removeDrawerTitle) {
        drawerTitle.style = `
            ...
            border: none;`
        drawerTitle.querySelector("button").classList.remove("mr-4");
    ...
    if (expandable) {
        ...
        newButton.querySelector("i").classList.add("fa-external-link");
...
var closeDrawer = () => { 
    ...
    drawerTitle.style = ``
    drawerTitle.querySelector("button").classList.remove("mr-4");
    drawerTitle.querySelector("button").classList.add("mr-4");
    ...
    const setListeners = (div) => {
        ...
        document.addEventListener('mousemove', function (e) {
            if (curCol) {
                ...
                applyWidthToCol(curCol, curColWidth, diffX, nxtCol, nxtColWidth);
                //applyWidthToCol(curCol, curColWidth, diffX, null, null);
...
const applyWidthToCol = (_curCol, _curColWidth, _diffX, _nxtCol, _nxtColWidth) => {
...
const applyWidthAndEllipsis = (elem, width, padding, index) => {
    ...
    if (elem.nodeName.toLowerCase() === "img" ||
        ...
        elem.classList.contains("DateFaseII")) {
        return;
    }
...
const initializeResizableGrids = () => {
    ...
    for (let i = 0; i < listOfGrids.length; i++) {
        ...
        listOfGrids[i].addEventListener("mouseenter", () => { resizableGrid(listOfGrids[i]) })
        let firstTR = listOfGrids[i].getElementsByTagName("tr")[0];
        let nCols = firstTR?.childElementCount;
        for (let colIndex = 0; colIndex < nCols; colIndex++) {
            let width = (EprocCache.getItem(`${listOfGrids[i].id}_${colIndex}`))
            if (!!width) {
