...
function Onload_Process () 
{
	...
	try
	{
		...
		//Se il modello è dedicato alle procedure di tipo RDO non avremo il giro contratto_gara con relativo perfezionamento del fornitore
	...
	if ( getObj('VersioneLinkedDoc').value == 'BANDO_SEMPLIFICATO' || getObj('VersioneLinkedDoc').value == 'BANDO_GARA' )
	{
		...
		try
		{
			//Conformità lista - 'MOD_ConfLista'
...
function OnChangeconformita()
{
	...
	//se ritorna -1 non è stato selezionato Ex-Post allora vengono nascoste le colonne Conformità Lista e Conformita Dett
...
function hideTipoFile(riga)
{
	...
	if (testoSelezionato.substring(0, 8) != 'Attach -')
	{
		...
		//FilterDom(  'RMODELLIGrid_' + riga + '_TipoFile' , 'TipoFile' , '', '' , '' , '');//se non è un allegato settiamo i tipifile a vuoto
...
function nascondiQTFormule()
{
	...
	//Se tutte le righe hanno la quantità vuota
	if ( valore == '' )
	{
