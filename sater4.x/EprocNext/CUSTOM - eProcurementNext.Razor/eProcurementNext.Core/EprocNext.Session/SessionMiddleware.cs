...
namespace eProcurementNext.Session
{
    public class SessionMiddleware
    {
        ...
        public async Task Invoke(HttpContext httpContext, eProcurementNext.Session.ISession _session)
        {
            ...
            string path = CommonModule.Basic.NormalizeUrlSlashes(httpContext.Request.Path.ToString());
            if (path != httpContext.Request.Path.ToString())
            {
        ...
        public static void LoadSession(HttpContext ctx, ISession session)
        {
            ...
            ctx.Response.Cookies.Append(Cookie_Anon_Name, tokenGenerated);
