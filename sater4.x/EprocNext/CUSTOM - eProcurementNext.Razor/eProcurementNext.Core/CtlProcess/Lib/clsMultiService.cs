...
namespace eProcurementNext.CtlProcess
{
    internal class ClsMultiService : ProcessBase
    {
        ...
        public override ELAB_RET_CODE Elaborate(string strDocType, dynamic strDocKey, long lIdPfu, string strParam, ref string strDescrRetCode, dynamic? vIdMp = null, dynamic? connection = null, SqlTransaction? transaction = null, int timeout = -1)
        {
            ...
            try
            {
                ...
                if (GetParameters(strParam, ref strDescrRetCode))
                {
                    ...
                    if (rs.RecordCount > 0 && Len(Trim(strAvanzamento)) == 0)
                    {
                    ...
                    while (!rs.EOF && bOK)
                    {
                        ...
                        //-- Se il servizio ennesimo sul quale stiamo iterando non  stato ancora eseguito
                        if (MidVb6(strAvanzamento, numeroServizio, 1) == "0")
                        {
                            ...
                            try
                            {
                                ...
                                outputProcess = objProc.Elaborate(strDocType, strDocKey, lIdPfu, relValOutput, ref strDescrRetCode, vIdMp, connection);
                            }
                            catch
                            {
                            ...
                            aggiornaSentinella(strAvanzamento, strDocKey, strTable, colDocKey, dseID, ref cnLocal);
        ...
        private string stuff(string original, int position, string newValue)
        {
            ...
            if (Len(Trim(original)) > 0 && position > 0 && position <= Len(original))
            {
        ...
        private void aggiornaSentinella(string sentinella, dynamic? strDocKey, string strTable, string colDocKey, string dseID, ref SqlConnection cnLocal)
        {
            var sqlParams = new Dictionary<string, object?>();
            sqlParams.Add("@strDocKey", CLng(strDocKey));
            sqlParams.Add("@dseID", dseID);
            ...
            cdf.Execute(CStr(strSql), cnLocal.ConnectionString, cnLocal, iTimeout, sqlParams);
