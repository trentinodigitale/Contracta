...
namespace eProcurementNext.PDND
{
    public class PDNDClient
    {
        ...
        //private readonly string SignatureCertificatePassword;
        //private readonly string MutualSSLCertificate;
        //private readonly string MutualSSLCertificatePassword;
        ...
        private PCPHeader header;
        private PCPPayload payload;
        private PCPPayloadWithData payloadWithData;
        private PCPEservice Eservice;
        //private readonly EProcurementNext.Session.ISession Session;
        ...
        public PDNDClient(IConfiguration configuration, PCPPayloadWithData Pload) //ISession _session, 
        {
            ...
            url = Configuration.GetSection("PDND_info").GetValue<string>("url");
            aud = Configuration.GetSection("PDND_info").GetValue<string>("aud");
            clientId = Configuration.GetSection("PDND_info").GetValue<string>("clientId");
            PemPrivateKey = Configuration.GetSection("PDND_info").GetValue<string>("keyPos");
            SignatureCertificate = Configuration.GetSection("PDND_info").GetValue<string>("certPos");
        ...
        public string composeComplementaryJWT(PCPPayloadWithData pLoad, string audience = null)
        {
            string headerJson = serializeHeader();
            byte[] headerByte = System.Text.Encoding.UTF8.GetBytes(headerJson);
            string payloadJson = serializePayload(pLoad, audience); // serializePayload(pload); // : 
        ...
        public string composeJWT(string serializedPayLoad, string hashedJwt)
        {
            string headerJson = serializeHeader();
            byte[] headerByte = System.Text.Encoding.UTF8.GetBytes(headerJson);
            ...
            payload.digest = dig;
            string payloadJson = serializePayload(payload); // serializePayload(pload); // : 
            byte[] payLoadByte = System.Text.Encoding.UTF8.GetBytes(payloadJson);
            try
            {
                //string jwtHeaderBase64 = Convert.ToBase64String(headerByte);
        ...
        public string composeJWT(PCPPayload pLoad, string audience = null)
        {
            string headerJson = serializeHeader();
            byte[] headerByte = System.Text.Encoding.UTF8.GetBytes(headerJson);
            ...
            string payloadJson = serializeEmptyPayLoad(pLoad, audience);
            byte[] payLoadByte = System.Text.Encoding.UTF8.GetBytes(payloadJson);
        ...
        {
            header.kid = Configuration.GetSection("PDND_info").GetValue<string>("kid");
            return JsonSerializer.Serialize(header);
        }
        public string serializePayload(PCPPayloadWithData pLoad, string audience = null)
        {
            pLoad.iss = clientId;
            pLoad.sub = clientId;
            if (!string.IsNullOrEmpty(audience))
        ...
        public PCPPayload composePayLoad(PCPPayloadWithData pLoadWithData)
        {
        ...
        public string serializePayload(PCPPayloadWithHash pLoad)
        {
            pLoad.iss = clientId;
            pLoad.sub = clientId;
            pLoad.aud = aud;
        ...
        public string serializeEmptyPayLoad(PCPPayload pLoad, string audience = null)
        {
            pLoad.iss = clientId;
            pLoad.sub = clientId;
            if (!string.IsNullOrEmpty(audience))
        ...
        public async Task<string> PDNDRequest(string url, string jwt, HttpMethod method = null, Dictionary<string, string> parametri = null, string? receivedVoucher = null, string? jwsForAgid = null, bool serviceRequest = false)
        {
            ...
            using (HttpClient httpClient = new HttpClient())
            {
                ...
                //byte[] jwsForAgidByte = System.Text.Encoding.UTF8.GetBytes(jwsForAgid);
                //string jwsForAgidBase64 = Convert.ToBase64String(jwsForAgidByte);
