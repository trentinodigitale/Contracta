    ...
    public partial class Session : ISession
    {
        ...
        public void Save()
        {
            ...
            foreach (var bson in this._toUpdateProperties)
            {
                ...
                else if(bson.Value != null && bson.Value.GetType() == typeof(System.DBNull))
                {
                    tempDict.Add(bson.Key, null);
                }
                else if(bson.Value != null && bson.Value.GetType() == typeof(System.Object[,]))
                {
                    for(int i = 0; i < ((System.Object[,])bson.Value).GetUpperBound(0) + 1; i++)
                    {
                        for(int j = 0; j < ((System.Object[,]) bson.Value).GetUpperBound(1) + 1; j++)
                        {
                            if (((System.Object[,])bson.Value)[i,j] is System.DBNull)
                            {
                                ((System.Object[,])bson.Value)[i, j] = null;
                            }
                        }
                    }
                }
