...
function OnLoadPage() {
	...
	*/
	...
	try {
		if (DOCUMENT_READONLY_LOCAL == '0' && getObj("FaseConcorso").value != 'seconda') {
			filtraRupProponente();
...
function onchangeEnteProponente() {
	if (getObj("FaseConcorso").value != 'seconda') {
		filtraRupProponente();
	}
}
...
function EsportaOfferteInXLSX() {
	...
	*/
	...
	ExecFunction('../../CTL_Library/accessBarrier.asp?goto=xlsx.aspx&TitoloFile=Risposte&FILTER=linkeddoc%3D' + getObjValue('IDDOC') + '&TIPODOC=' + TipoDoc + '&MODEL=BANDO_CONCORSO_LISTA_OFFERTEGriglia&VIEW=BANDO_CONCORSO_LISTA_OFFERTE&HIDECOL=FNZ_OPEN,Name' + extraHideCol + '&Sort=DataInvio%20asc&IDDOC=' + getObjValue('IDDOC'), '_blank', '');
}
...
function EstraiPartecipantiConcorso() {
	var extraHideCol = '';
	var TipoDoc = 'RISPOSTA_CONCORSO';

	ExecFunction('../../CTL_Library/accessBarrier.asp?goto=xlsx.aspx&TitoloFile=PartecipantiConcorso&FILTER=linkeddoc%3D' + getObjValue('IDDOC') + '&TIPODOC=' + TipoDoc + '&MODEL=BANDO_CONCORSO_LISTA_PARTECIPANTIGriglia&VIEW=VIEW_PARTECIPANTI_BANDO_CONCORSO&HIDECOL=&Sort=aziRagioneSociale%20asc&IDDOC=' + getObjValue('IDDOC'), '_blank', '');

}

function SetConcorsoInDueFasi() {
	//Se il concorso è a due fasi e siamo nella prima fase
	if (getObj('FaseConcorso').value == 'prima') {
		//nascondo le 2 colonne per cancellare e copiare la riga. nella prima fase ci deve essrere un solo record
		ShowCol('CRITERI', 'FNZ_DEL', 'none');
		ShowCol('CRITERI', 'FNZ_COPY', 'none');
	}
	else {
		//nascondo la colonna DataPrevistaAvvioSecondaFase
		$("#cap_DataPrevistaAvvioSecondaFase").parents("table:first").css({ "display": "none" });
	}

	//Se mi trovo nella seconda fase del concorso a 2 fasi
	if (getObj('FaseConcorso').value != 'seconda') {
		getObj('DOCUMENTAZIONE_RICHIESTA_PRIMAFASE').style.display = 'none';
	}
}



function InfoTec_DefinizionePremi_griglia_AFTER_COMMAND(param) {

	//alert( 'InfoTec_DefinizionePremi_AFTER_COMMAND' );
	//var numrighe = GetProperty(getObj('PRODOTTIGrid'), 'numrow');
	//alert(numrighe);
	//if ( getObj('RPRODOTTIGrid_' + numrighe + '_CODICE_REGIONALE').value != '' && param == 'ADDFROM')
	ExecDocProcess('SET_CLASSIFICA_PREMI,BANDO_CONCORSO,,NO_MSG');
