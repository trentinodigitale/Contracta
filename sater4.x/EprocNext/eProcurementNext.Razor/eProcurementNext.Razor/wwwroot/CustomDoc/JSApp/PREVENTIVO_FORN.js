...
function LocDetailMakeTotal( Section , obj )
{
    ...
    var im =  Number( getObj( r + '_RDP_Importo' ).value ).toFixed(6);
    var PercSconto = Number( getObj( r + '_PercSconto' ).value ).toFixed(6);
    
    SetNumericValue( r + '_Importo' , ( qt * im ) - ((qt * im * PercSconto ) / 100 ));
...
function PRODOTTI_OnLoad()
{
    ...
    try
    {
        ...
        while( getObj( 'R' + r + '_RDP_Qt' ) != undefined && r < 1000 )
        {
            qt =  Number( getObj( 'R' + r + '_RDP_Qt' ).value ).toFixed(6);
            im =  Number( getObj( 'R' + r + '_RDP_Importo' ).value ).toFixed(6);

            SetNumericValue( 'R' + r + '_Importo' , qt * im );
            r++;
        }
    }
    catch(e){alert( r );}
