...
function OpenDettaglioGareCal( objGrid , Row , c )
{
	...
	var strTable = 'DASHBOARD_VIEW_GARE_DETTAGLI_CAL' ;
	var strModelloGriglia = 'DASHBOARD_VIEW_GARE_DETTAGLI_CALGriglia';
	try{ var hiddenViewerCurFilter = getObj('hiddenViewerCurFilter').value; } catch(e){};


	if ( idpfucollegato != '' )
	{
		strTable = 'DASHBOARD_VIEW_GARE_ENTE_DETTAGLI_CAL&HIDE_COL=FNZ_OPEN' ;
		strFilter = strFilter + encodeURIComponent(' and idpfu=' + idpfucollegato) ;
	}
	else if ( hiddenViewerCurFilter.indexOf('AZI_Ente') >= 0 )	
	{
		strTable = 'DASHBOARD_VIEW_GARE_ENTE_DETTAGLI_CAL_CONF_SISTEMA';
	}
	...
	var strURL='DASHBOARD/Viewer.asp?ModelloFiltro=DASHBOARD_VIEW_GARE_DETTAGLI_CALInfo&MODGriglia=' + strModelloGriglia + '&AreaFiltro=no&AreaInfo=no&TOOLBAR=DASHBOARD_VIEW_GARE_DETTAGLI_CAL_TOOLBAR&Table=' + strTable + '&JSCRIPT=&IDENTITY=ID&DOCUMENT=P&PATHTOOLBAR=../customdoc/&AreaAdd=no&CaptionNoML=yes&Caption=' + Caption + '&Height=50,100*,0&numRowForPag=20&Sort=DataRiferimentoCompleta&SortOrder=asc&Exit=si&' + strFilter ;
