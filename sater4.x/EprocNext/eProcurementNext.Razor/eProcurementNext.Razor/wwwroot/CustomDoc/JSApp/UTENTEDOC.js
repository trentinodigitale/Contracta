	...
	if ( nRet == 0 ){
		...
		*/
		...
		if (getObjValue('Nazione') != "M-1-11-ITA" && getObjValue('Nazione') != "" )  {
			...
			var ML_text = 'Codice fiscale non coerente con nome e cognome';
		
			DMessageBox( '../' , ML_text , Title , 1 , 400 , 300 );
			return;	
		}
	}else{
...
function RUOLI_AFTER_COMMAND ( param )
{
...
function ONLoad()
{
	...
	if ( getObjValue('DOCUMENT_READONLY') == '0' )
 	{
		try	{filtroResponsabile();	}catch(e){}
		try	{filtroPlant( );	}catch(e){}
	...
	try
	{
		ProfiloAlbo = getObjValue( 'ProfiloAlbo' );
	...
	if ( aziProfili.indexOf( 'P' ) == -1 )
	{
		...
		DocDisplayFolder(  'AREA_VAL'   ,'none' );
	}
	
	if ( ProfiloAlbo != 1 ) // && getObjValue('Destinatario_User') != '0' )
	{		
		DocDisplayFolder(  'AREA_VAL'   ,'none' );
	}
	...
	//filtro i ruoFiltraRuolili
	FiltraRuoli();		

	//Rimuovo il cestino se sono su una riga con ruolo responsabile peppol
	rimuoviCestinoRuolo();
	
	
	//se si tratta di un OE nascondo il campo della struttura di appartenenza
	if ( aziProfili.indexOf( 'S' ) != -1 )
	{
		try{$("#cap_Plant").parents("table:first").css({"display": "none"});}catch(e){}
	}
...
function FiltraRuoli()
{
	var aziProfili = getObjValue( 'aziProfili' );
	var i = 0;
	var filter = 'SQL_WHERE= dmv_cod in ( SELECT dmv_cod from UserRole_MLNG where ML_LNG=\'I\' and DMV_Cod <> \'RESPONSABILE_PEPPOL\' and DMV_Deleted =0 and ( dbo.checkCarattereValido(DMV_Father,\'' + aziProfili + '\')=1 or dmv_father = \'1\') ) or DMV_Cod = \'BASE\' ';
	//alert(filter);
	...
	{	
		...
		for( i=0; i < 1 ; i++ )
		//for( i=0; i < RRUOLIGrid_EndRow+100 ; i++ )
		{
		  
			
			//FilterDom(  'RRUOLIGrid_' + i + '_UserRole', 'UserRole' , getObjValue( 'RRUOLIGrid_' + i + '_UserRole' ), filter , 'RUOLIGrid_' + i , '' );
			
			try
			{
				FilterDomFirstRowCol(  'RRUOLIGrid_' + i + '_UserRole' , 'UserRole' , getObjValue( 'RRUOLIGrid_' + i + '_UserRole' ), filter , 'RUOLIGrid_' + i  , '');
			}
			catch(e)
			{
			}
...
function rimuoviCestinoRuolo()
{
	var numRowRUOLI = GetProperty( getObj('RUOLIGrid') , 'numrow');
	
	if(  Number( numRowRUOLI ) >= 0 )
	{
		
		
		for( i = 0 ; i <= numRowRUOLI ; i++ )
		{
			try
			{
				var nomeCampoGrid = 'val_RRUOLIGrid_' + i + '_UserRole';
				
				var ruolo = GetProperty( getObj(nomeCampoGrid) , 'value');
		
				if ( ruolo == 'RESPONSABILE_PEPPOL' )
				{
					getObj('RRUOLIGrid_' + i + '_FNZ_DEL').innerHTML = '';
				}
...
function filtroPlant()
{
		...
		{ 
			...
			filter = 'dmv_level >= 3 and idaz in ( ' + getObj('Azienda').value + ' )' ;
			getObj('Plant_extraAttrib').value= 'strformat#=#DT#@#filter#=#SQL_WHERE= ' + filter + '#@#multivalue#=#0';
