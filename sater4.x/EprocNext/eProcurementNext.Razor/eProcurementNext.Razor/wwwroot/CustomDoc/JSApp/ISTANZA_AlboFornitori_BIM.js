...
'IndirizzoEntrate',
'PEC_Entrate'//,
/*'IAF_altra_classificazione2',
'campo_scopo_certificato',
'IAF_altra_classificazione',
'norme_di_riferimento',
'tipoogia_conformita',
'sistema_gestione_ambientale',
'settore_ambito_candidatura',
'campo_applicazione_certificato'*/
];
...
function GeneraPDF ()
{
	...
	PrintPdfSign('URL=/report/prn_' + getObj('TYPEDOC').value + '.ASP?SIGN=YES&PROCESS=ISTANZA@@@VERIFICHE_PRE_PDF&VIEW_FOOTER_HEADER=ISTANZA_AlboForn_HF_Stampe');
...
function LocalPrintPdf( param )
{
    ...
    param = '/report/prn_' + getObj('TYPEDOC').value + '.ASP?&VIEW_FOOTER_HEADER=ISTANZA_AlboForn_HF_Stampe'
    if( Stato == '' ) 
    {
...
function controlli (param)
{
	...
	{	
		...
		//se è stato checked il punto m2 aggiungo 3 nuovi attributi all'array dei campi obblig
...
function CampiNotEdit()
{
	...
	//abilito i campi del punto p se il secondo check del punto m è attivo
	if ( getObj('check_art_80_assunzioni_2').checked == true )
	{
...
function OnChange_Categoria_Merc()
{
	ExecDocProcess( 'ADD_NEW,POSIZIONI_FATTURARO_INCARICHI_FORN,,NO_MSG');
}
