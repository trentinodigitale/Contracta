...
function OnLoadPage()
{
	...
	FilterDominio(); //-- attributi criteri di valutazione
    ...
    try {
	 ...
	 //nascondere le colonne del drag & drop se il documento è chiuso
	try
	{
		
		ShowCol( 'DOCUMENTAZIONE' , 'FNZ_DRAG' , 'none' );	
		ShowCol( 'DOCUMENTAZIONE' , 'FNZ_COPY' , 'none' );
		ShowCol( 'DOCUMENTAZIONE' , 'FNZ_UPD' , 'none' );			
...
function filtraRupProponente()
{
	...
	if ( EnteProponente == enteappaltante ) //se coincidono stesso filtro presente sul RUP anche su RUP proponente ed il campo è bloccato
	{
		...
		FilterDom( 'RupProponente' , 'RupProponente' , getObj('val_UserRUP_extraAttrib').value.split('#=#')[1] , filter ,'', '','TD','','no');
...
function OnClickProdotti( obj )
{
    ...
    if ( DOCUMENT_READONLY == "1" )
        ...
        ImportExcel( 'CAPTION_ROW=yes&TITLE=Upload Excel&TABLE=CTL_Import&FIELD=RTESTATA_PRODOTTI_MODEL_Allegato&SHEET=0&PARAM=posizionale&PROCESS=LOAD_PRODOTTI,BANDO_SEMPLIFICATO&OWNER_FIELD=Idpfu&OPERATION=INSERT#new#400,300' );
}
...
function ChangeImpAppalto( obj )
{
    ...
    SetNumericValue( 'importoBaseAsta' , Oneri + importoBaseAsta2 );
...
function FilterDominio() {
    ...
    if (strVersione != '' && statFunz == 'InLavorazione') 
	{
...
function onChangeUserRUP()
{
	var EnteProponente=getObjValue('EnteProponente').split('#')[0];	
	var enteappaltante=getObjValue('Azienda');
	if ( EnteProponente == enteappaltante ) //se coincidono valorizzo RupProponente con lo stesso valore
	{
...
{
	//PER LE GARE CHE NON SONO economicamente vantaggiose 15532
