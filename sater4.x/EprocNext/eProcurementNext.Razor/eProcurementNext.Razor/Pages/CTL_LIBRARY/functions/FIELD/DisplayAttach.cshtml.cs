        ...
        public static void InsertInTable(dynamic lIdPfu, dynamic att_hash, CommonDbFunctions cdf)
        {
            ...
            if (!string.IsNullOrEmpty(CStr(lIdPfu)))
            {
                ...
                if (rs.RecordCount > 0)
                {
                    ...
                    cdf.Execute("update CTL_ATTACH_READ set ATR_DataInsert=GETDATE() where atr_hash='" + CStr(att_hash) + "' and atr_idpfu=" + CStr(lIdPfu), ApplicationCommon.Application.ConnectionString);
                }
                else
                {
                    cdf.Execute("insert into CTL_ATTACH_READ (atr_hash,atr_idpfu, ATR_DataInsert) values ('" + CStr(att_hash) + "'," + CStr(lIdPfu) + ", GETDATE())", ApplicationCommon.Application.ConnectionString);
